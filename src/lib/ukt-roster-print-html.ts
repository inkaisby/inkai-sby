...
export type UktRosterPrintPaper = "A4" | "F4";
export type UktRosterPrintOrientation = "portrait" | "landscape";

export type UktRosterPrintData = {
  periodTitle: string;
  dojoLabel: string;
  participantCount: number;
  showRantingColumn: boolean;
  rows: UktRosterPrintRow[];
  origin: string;
  printedAt: string;
  sekretariatAddress?: string;
  paper?: UktRosterPrintPaper;
  orientation?: UktRosterPrintOrientation;
};

export function uktRosterPageSizeCss(
  paper: UktRosterPrintPaper = "A4",
  orientation: UktRosterPrintOrientation = "landscape",
): string {
  if (paper === "F4") {
...
export function buildUktRosterPrintHtml(data: UktRosterPrintData): string {
...
<head>
  <meta charset="utf-8" />
  <title>Daftar Peserta UKT</title>
  <style>
...
<body>
  <div class="page">
    ...
    <h1 class="doc-title">Daftar Peserta Ujian Kenaikan Tingkat</h1>
    <div class="meta">
...
export function printUktRosterDocument(data: UktRosterPrintData): void {
  openHtmlPrintWindow(buildUktRosterPrintHtml(data));
}
