...
/** Scroll ke entri saat deep-link ?slug=. */
export function ArticleScrollTarget({ targetId }: { targetId: string | null }) {
  useEffect(() => {
    if (!targetId) return;
    const el = document.getElementById(`artikel-entry-${targetId}`);
    if (!el) return;
