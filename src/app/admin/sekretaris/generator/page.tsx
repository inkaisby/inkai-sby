  ...
  return (
    <div className="space-y-6 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-6 rounded-3xl shadow-xl">
        <div>
          <h1 className="text-2xl font-black text-white tracking-tight flex items-center gap-2">
            <FileSpreadsheet className="w-6 h-6 text-red-500" /> Generator & Template Surat PDF
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Editor Form Parameters (5 Cols) */}
        <div className="lg:col-span-6 bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-5 text-xs">
          <h2 className="font-bold text-white text-base border-b border-slate-800 pb-3 flex items-center justify-between">
            <span>📝 Parameter & Editor Isi Surat</span>
            <span className="text-xs text-red-400 font-mono">Format: {kategori}</span>
          </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              />
              {nomorSurat === "AUTO" && (
                <div className="text-[11px] text-red-400 mt-1">Preview: {nextNumberPreview}</div>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-slate-400 mb-1 font-medium">Tempat Ditetapkan</label>
              <input
                type="text"
                value={ditetapkanDi}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3 bg-slate-950 rounded-2xl border border-slate-800">
            <div>
              <label className="block text-slate-400 mb-1 font-medium">Ukuran Kertas Print</label>
              <select
                value={paperSize}
            ...
            <div>
              <label className="block text-slate-400 mb-1 font-medium">Mode Tanda Tangan</label>
              <select
                value={signatureMode}
          ...
          <div className="space-y-2">
            <label className="block text-slate-400 font-medium">WYSIWYG Format & Style Toolbar</label>
            <div className="flex flex-wrap items-center gap-2 p-2 bg-slate-950 border border-slate-800 rounded-2xl">
            ...
            <textarea
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-3 text-white font-sans"
          ...
          <div className="space-y-3 pt-3 border-t border-slate-800">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-white text-xs flex items-center gap-1.5">
                <Sparkles className="w-4 h-4 text-amber-400" /> Interactive Table Panitia / Personel ({tableRows.length})
              </h3>

              <button
                type="button"
                onClick={() => addTableRow()}
                className="inline-flex items-center gap-1 px-3 py-1 rounded-xl bg-red-600/20 text-red-300 hover:bg-red-600/30 border border-red-500/30 text-xs font-bold transition"
              >
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-amber-500/30 rounded-xl pl-8 pr-3 py-1.5 text-white text-xs placeholder-slate-500 focus:outline-none focus:border-amber-400"
              />
              {memberSuggestions.length > 0 && (
                <div className="absolute top-full left-0 right-0 z-20 mt-1 bg-slate-950 border border-slate-800 rounded-xl shadow-2xl divide-y divide-slate-800 overflow-hidden">
                  {memberSuggestions.map((m) => (
                    <button
                      ...
                      className="w-full p-2 text-left hover:bg-slate-800 transition flex items-center justify-between text-xs"
                    >
                      <span className="font-bold text-white">{m.fullName}</span>
                      <span className="text-[10px] text-slate-400">{m.currentRank} ({m.dojo?.name})</span>
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
              {tableRows.map((row, i) => (
                <div key={i} className="flex items-center gap-2 bg-slate-950 p-2 rounded-xl border border-slate-800">
                  <span className="text-slate-500 font-mono w-5 text-center">{i + 1}</span>
                  <input
                    type="text"
                    value={row.NAMA}
                    placeholder="Nama Lengkap"
                    onChange={(e) => updateTableRow(i, "NAMA", e.target.value)}
                    className="flex-1 bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1 text-white font-semibold"
                  />
                  <input
                    type="text"
                    value={row.JABATAN}
                    ...
                    className="flex-1 bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1 text-slate-300"
                  />
                  <button
                    type="button"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-3 border-t border-slate-800">
            <div>
              <label className="block text-slate-400 mb-1 font-medium">Nama Ketua TTD</label>
              <input
                type="text"
                value={ketuaName}
            ...
            <div>
              <label className="block text-slate-400 mb-1 font-medium">Nama Sekretaris TTD</label>
              <input
                type="text"
        ...
        <div className="lg:col-span-6 bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col items-center justify-between">
          <div className="w-full flex items-center justify-between pb-3 border-b border-slate-800 mb-4">
            <h2 className="font-bold text-white text-sm flex items-center gap-2">
              <Printer className="w-4 h-4 text-red-400" /> Pratinjau Live PDF Cetak ({paperSize})
            </h2>
          ...
          <div className="w-full flex-1 bg-slate-950 rounded-2xl p-4 overflow-auto flex justify-center border border-slate-800">
