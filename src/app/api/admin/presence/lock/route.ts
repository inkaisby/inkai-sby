...
import { lockUser } from "@/lib/security/session-control";
...
export async function POST(request: Request) {
  ...
  if (await isCurrentlyImpersonating()) {
    return NextResponse.json(
      { error: "Tidak dapat mengunci akun saat mode ambil alih" },
      { status: 403 },
    );
  }

  const rlKey = `presence:lock:${authResult.user.id}`;
  ...
  if (userId === authResult.user.id) {
    return NextResponse.json(
      { error: "Tidak dapat mengunci akun sendiri" },
      { status: 400 },
    );
  }
  ...
  if (typeof body.until === "string" && body.until.trim()) {
    ...
    if (Number.isNaN(parsed.getTime()) || parsed.getTime() <= Date.now()) {
      return NextResponse.json(
        { error: "until harus tanggal masa depan yang valid" },
        { status: 400 },
      );
    }
    until = parsed;
  ...
  await lockUser(target.id, until);

  writeLocalAuditLog({
    userId: authResult.user.id,
    email: authResult.user.email,
    action: "SECURITY_SESSION_LOCK",
    details: `targetUserId=${target.id} targetEmail=${target.email} until=${until?.toISOString() ?? "indefinite"} reason=${reason.slice(0, 200)}`,
  ...
  return NextResponse.json({
    success: true,
    message: `Akun ${target.fullName || target.email} dikunci`,
  });
}
