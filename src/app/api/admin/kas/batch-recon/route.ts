...
export async function POST(request: Request) {
  ...
  try {
    const body = (await request.json().catch(() => null)) as {
      ids?: string[];
      reconStatus?: "matched" | "open";
    } | null;

    const ids = body?.ids;
    const reconStatus = body?.reconStatus === "matched" ? "matched" : "open";
    ...
    return NextResponse.json({ success: true, updated: result.count, reconStatus });
  } catch (error) {
    if (error instanceof KasPeriodLockedError) {
      return NextResponse.json({ error: error.message }, { status: 409 });
    }
    const msg = error instanceof Error ? error.message : "Gagal memperbarui status rekonsiliasi";
