...
| Kwitansi Pembayaran | Nav **Kwitansi** `/admin/kwitansi` + **Arsip** `/admin/kwitansi/arsip` di bawah Keuangan; logo INKAI + kop Cabang Surabaya + cap LUNAS; mode Per Penerima vs Rangkuman; edit arsip via `?no=` memuat data kwitansi beserta **100% rincian item barang/penerima**; simpan arsip memperbarui data eksisting berdasarkan nomor dokumen (in-place update tanpa duplikat) + **sinkronisasi otomatis update nominal/tanggal ke Jurnal Kas** (kecuali jenis iuran); **hapus kwitansi di Arsip otomatis menghapus entri Kas terkait** (`DELETE /api/admin/kas?sourceType=kwitansi&sourceId=`); pembersihan duplikat otomatis di Arsip; export CSV rekap; nomor KW/NP; TTD; `openHtmlPrintWindow`; grant + preset bendahara + Kas |
