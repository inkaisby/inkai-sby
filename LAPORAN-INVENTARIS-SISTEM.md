...
| 14 September 2026 | **Fitur Drag & Drop Reordering Baris Kas (`src/lib/kas-store.ts`, `POST /api/admin/kas/reorder`, `KasLedgerClient.tsx`):** Menambahkan field `sortOrder` pada model `KasEntry` (`prisma/schema.prisma`), endpoint API `reorder`, serta handle geser `GripVertical` di UI tabel Kas; mendukung pengubahan urutan baris kas secara interaktif (drag & drop) seperti spreadsheet dengan update saldo berjalan real-time dan penyimpan urutan ke database; inventaris §6/§9/§15 |
