...
| Kas | Aktif (jurnal lokal) | `/admin/kas` Prisma `KasEntry`/`KasPeriodLock`; filter periode awal–akhir; Ubah/Hapus/Pindah lokasi (1 baris + centang batch + Pindah kegiatan); **Tambah item** di header grup kegiatan; **grup kegiatan default dilipat**; **Tambah massal multi-tanggal** (append paste, tanggal per baris); sticky laptop / ringkasan kolaps HP; auto dari verifikasi bayar; manual/massal/impor CSV; cetak kop INKAI; tutup buku; rekon; grant `/admin/kas`; bukan backfill Excel |
