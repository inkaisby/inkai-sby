...
| UKT | Sidebar grup **UKT (Ujian Kenaikan Tingkat)** → **Pendaftaran** (`/admin/ukt`) + **Arsip UKT** (`/admin/ukt/arsip`); **registrants-first** (peserta terdaftar; Belum Daftar via suggest+hydrate); foto peserta di-hydrate dari `Member.photoUrl` Prisma (fallback `User.photoUrl`, Prisma menang atas string kosong/stale dari Inkai); **shell streaming**: header + semester/tahun tampil dulu, KPI/tabel Suspense menyusul; periode aktif, **sort kolom**, multi-select ranting, **filter Gabungan multi-ranting**, bayar→verifikasi cabang, sabuk target, nota, **Laporan/Rekapan** (hub tab Peserta / Hasil Ujian / Administrasi — cabang; ranting tanpa Administrasi), **hari-H**, **setoran + rekonsiliasi**, **arsip**, wizard (ujian/pejabat/snapshot biaya); **HP:** timer tetap; aksi toolbar → **Lainnya**; jadwal/setoran/KPI/alur → **Ringkasan**; cari sticky di bawah topbar+TermNav (`top-[6.75rem]` / `2xl`); saran cabang tampil **ranting** + tombol **Daftar UKT** di dropdown (hilang jika sudah terdaftar; `uktEventId` di suggest); **kolom Aksi** **Daftar UKT** (shared `EventQuickRegisterButtons`; Waiver cabang tetap); HP **Daftar UKT** dekat nama; **kolom Nama+checkbox sticky** (satu scroller; HP vertikal scroll halaman via `overflow-y-clip` pada wrapper tabel); ranting subtitle HP + kolom `sm+`; **kolom Tanggal daftar** (sort; tampil di mode ringkas); kolom default ringkas; **toolbar atas sticky**; **ikon Printer Cetak Nota** di toolbar sticky **tabel** (semua breakpoint, termasuk mode full); modal Cetak Nota **tetap terbuka** setelah Print; **ikon Maximize/Minimize full halaman in-app** (`fixed inset-0`, Esc; header Refresh+Laporan/Rekapan; tanpa auto-refresh saat maximize; FAB Tanya INKAI disembunyikan); **Kyu Lama+Baru Select Radix** (cabang/administrator) di **tabel biasa dan full** (Kyu Lama = snapshot `update_kyu`, Kyu Baru = alur Selesai; Esc pada dropdown tidak menutup full); saat full: tampilan Lengkap, tanpa kolom Dokumen; **ranting: Daftar/Batal (sebelum Menunggu Ujian)/Bayar + toolbar Laporan/Rekapan (Peserta+Hasil scoped) + Laporan WA (rincian setor), Salin/WA Undangan & Cetak Nota**; cabang: **edit NIA inline** (set_nia) + **Hapus tagihan** terpisah dari hapus pendaftaran + **Hapus pendaftaran termasuk lunas**; **Pengprov: edit NIA inline** (tanpa aksi cabang lain); **undangan portal** `/undangan/ukt/[periodId]` + **roster publik** `/ukt`; **penyelarasan URL period**: auto-detect semester/tahun dari judul event (mis. period ID Semester I yang diakses lewat URL `semester=II` akan otomatis disinkronkan ke semester/tahun kanonis event tanpa berkedip/redirect-loop); **loader registrasi Prisma**: filter status registrasi Prisma disamakan (`notIn: ["CANCELLED", "REJECTED"]`) sehingga peserta berstatus `PAID`/`CONFIRMED`/`REGISTERED` tetap dihitung dan dimuat penuh tanpa salah terdeteksi sebagai periode kosong |
...
| 5 September 2026 | **Kas Toolbar Micro-Density & Dedicated Smooth Table Scroll:** Merapatkan toolbar filter & KPI kas (`/admin/kas`) ke tinggi `h-8 text-xs` (32px) dengan penataan flex 2 baris ringkas, menghemat >55% tinggi atas sehingga 12-15+ baris data kas langsung terlihat; memperbarui kontainer scroll tabel menjadi `max-h-[calc(100vh-270px)] overflow-y-auto` dengan `sticky top-0` header agar halaman & tabel dapat digulung ke atas/bawah secara mulus tanpa terpotong; inventaris §6/§9/§15 |
