...
| `/latber` | **Pendaftaran Latber walk-in (publik):** info login NIA + Pesan; **timer batas pendaftaran** (sama komponen UKT, di atas cari); KPI status + KPI per ranting + grafik batang; cari/scan QR kartu; modal Tambah Anggota (= tab Daftar); tabel (NIA, biaya flat **Rp 45.000**); **Bayar** = QRIS percobaan + Mandiri `1400024546344` a.n. HABIBUR RAHMAN (salin rekening/nominal 45.000) + **Sudah bayar** → Menunggu Verifikasi; batal WA; `/admin/latber` tanpa sesi → sini; link undangan pendek **`/latber`** + `opengraph-image` / metadata "Latihan Bersama" |
...
| Latber walk-in publik | Aktif | `/latber` + `/api/public/latber/*`; tarif flat 45.000 (tanpa uniqueTail); KPI status+ranting+grafik; Bayar QRIS percobaan + transfer Mandiri; Sudah bayar → verifikasi admin; batal WA; poll senyap ~30s |
