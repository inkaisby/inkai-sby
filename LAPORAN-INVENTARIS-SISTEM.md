...
| 8 September 2026 | **Pengelompokan Induk Kegiatan Kas Multiranting & Tanggal (`src/lib/kas.ts`):** Meng-update `getKasBaseKegiatan` & `groupKasTable` agar seluruh transaksi kegiatan sejenis (seperti `Bayar Latber Persiapan UKT` dan `Bayar UKT II-2026` / `Bayar UKT I-2026` dari seluruh ranting & lintas tanggal) digabung secara otomatis di bawah **1 baris header induk utama** (mis. `Bayar UKT II-2026` / `Bayar Latber Persiapan UKT`), menampilkan total penjumlahan seluruh ranting/tanggal sekaligus saat terlipat, dan menampilkan seluruh rincian peserta saat dibuka (*expand*); inventaris §6/§9/§15 |
