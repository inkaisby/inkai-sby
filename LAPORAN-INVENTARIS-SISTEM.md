...
7. Alur cabang setelah bayar: **Verifikasi** → **Menunggu Ujian** → isi **Kyu Baru** → otomatis **Lulus** + **Selesai**. Kolom **Kyu Baru** kosong sampai cabang mengisi (bukan dari nama kategori tagihan/sabuk target); snapshot `registeredRank` tanpa pemisah = Kyu Lama. Apply Kyu Baru menulis Prisma `Member.currentRank` + `EventRegistration.registeredRank` (`lama || baru`) + Inkai (selaras `set_rank`).
