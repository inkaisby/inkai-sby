...
| Sekretariat | Grup **Sekretariat** (`/admin/sekretaris`): **Dashboard Sekretariat** (`/admin/sekretaris`), **Surat & Persuratan** (`/admin/sekretaris/surat`), **Generator Surat** (`/admin/sekretaris/generator`), **Dokumen & SK** (`/admin/sekretaris/dokumen`), & **Notulen Rapat** (`/admin/sekretaris/rapat`); 100% mendukung dual mode toggle terang & gelap (`dark:`); penomoran otomatis `[No]/INKAI-KOTA.SBY/[Kode]/[Bulan-Romawi]/[Tahun]`, generator cetak A4/F4 presisi 1:1 Kop FORKI (sebelah kiri header kop) & INKAI (sebelah kanan header kop; tertanam terintegrasi Base64 persisten), **Stempel Resmi INKAI Kota Surabaya** (stempel keorganisasian resmi berlogo sabuk INKAI Kota Surabaya terpasang otomatis pada tanda tangan digital/sistem), templat **Surat Tugas**, **Surat Undangan**, **Surat Keterangan**, **Rekomendasi**, **SK Pengurus**, dan **Tab Resume Rapat / Notulensi** + **Impor Otomatis Notulensi Rapat Tersimpan**, **Editor Visual WYSIWYG Langsung In-Place**, **Toolbar Toolkit Lengkap**, **Pratinjau Live PDF Cetak Kertas Utuh Tanpa Terpotong** (margin kertas A4/F4 presisi 16mm di layar + bingkai responsif proporsional dengan kontrol zoom +/- & mode layar penuh 100% utuh tanpa teks terpotong di tepi), TTD Digital/Manual/Draw, QR Code verifikasi publik (`/v/surat/[id]`), delivery ke portal & notifikasi anggota (`/dashboard/dokumen`), kearsipan SK pengurus dengan alert kadaluarsa, & notulensi rapat dengan action items & salin format WA; **Navigasi Sidebar Dinamis Single-Active**: perbaikan penentuan link aktif di `SidebarNavGroup` menggunakan algoritma *dynamic longest-prefix matching* pada link bersaudara sehingga ketika berada di sub-halaman seperti Generator Surat (`/admin/sekretaris/generator`), hanya menu spesifik tersebut yang tersorot merah aktif (mencegah indikator ganda pada Dashboard parent `/admin/sekretaris`) |
