...
10. Cetak nota memakai tabel biaya sabuk bulat; **tanpa kode unik** (+1…999). Nomor nota memuat semester (`UKT/SBY/{RANTING}/I|II/{tahun}`); modal **daftar centang ranting** (satu / beberapa / semua via Pilih semua) — bila >1 tercentang: `RANTING : GABUNGAN ({nama…})` + nomor `UKT/SBY/GABUNGAN/{smt}/{tahun}` + agregat peserta/sabuk/komisi/TOTAL setor ke cabang; satu ranting = nota per ranting seperti semula. Pejabat (Bidang Ujian / Bendahara) dari **period-meta per periode**, fallback **Pengaturan → Kebijakan**. **Nota + Laporan WA rincian setor** hanya peserta **tagihan pembayaran** (`WAITING_VERIFICATION` atau lunas) — **Belum Bayar** (setelah hapus tagihan / belum Bayar UKT) tidak masuk total setor; tab Peserta tetap semua terdaftar.
10a. **Laporan WA ranting:** daftar peserta + **rincian pembayaran** (Subtotal A sabuk − Komisi Ranting = TOTAL disetor ke cabang), selaras Nota (`isUktPaymentDocumentRow`). WA cabang bisa memilih: **ringkas** (Total Ranting / kyu) atau **rinci per ranting** (peserta + rincian setor) tanpa ganti akun; rekap sabuk mengikuti **Kyu Lama**.
