...
| UKT | Sidebar grup **UKT (Ujian Kenaikan Tingkat)** → **Pendaftaran** (`/admin/ukt`) + **Arsip UKT** (`/admin/ukt/arsip`); **registrants-first** (peserta terdaftar; Belum Daftar via suggest+hydrate); **shell streaming**: header + semester/tahun tampil dulu, KPI/tabel Suspense menyusul; periode aktif, **sort kolom**, multi-select ranting, **filter Gabungan multi-ranting**, bayar→verifikasi cabang, sabuk target, nota, **Laporan** (hub tab Peserta / Hasil Ujian / Administrasi — cabang; ranting tanpa Administrasi), **hari-H**, **setoran + rekonsiliasi**, **arsip**, wizard (ujian/pejabat/snapshot biaya); **toolbar atas sticky**; **ranting: Daftar/Batal (sebelum Menunggu Ujian)/Bayar + toolbar Laporan (Peserta+Hasil scoped) + Laporan WA (rincian setor), Salin/WA Undangan & Cetak Nota**; cabang: **edit NIA inline** (set_nia) + **Hapus tagihan** terpisah dari hapus pendaftaran + **Hapus pendaftaran termasuk lunas**; **Pengprov: edit NIA inline** (tanpa aksi cabang lain); **undangan portal** `/undangan/ukt/[periodId]` |
...
10b2. **Hub Laporan UKT** (tombol **Laporan** di toolbar, Pendaftaran + Arsip): modal ber-tab `UktReportsHubModal` — **Peserta** (export daftar: ranting/CSV/PDF/Print; floating Export multi-select juga membuka tab ini), **Hasil Ujian** (rekap Pengda Excel/PDF/Print + pejabat/penguji/TTD), **Administrasi** (Perincian setor Pengprov — **cabang saja**; ranting tidak melihat tab ini). Default tab **Hasil Ujian** jika sudah ada Kyu Baru, selain itu **Peserta**.
10b2b. **Tab Administrasi** (cabang): Bagian I = peserta **lunas** × tarif **Pengprov** (`pengprovBeltFees` di period-meta, default 150/150/150/155/195 — **terpisah** dari `beltFees` Nota); abaikan filter ranting UI (agregat semua ranting); Bagian II buku + Biaya Penguji manual; Print/PDF.
10b3. **Tab Hasil Ujian** (cabang **dan** ranting scoped): Excel 2 sheet format Pengda (`SMT. I/II` + `LEMBAR TTD`) via `POST /api/admin/ukt/rekap-hasil`; PDF/Print HTML A4 landscape 2 halaman (kop logo INKAI kiri); hanya peserta yang sudah punya **Kyu Baru**; Kyu Lama dari snapshot daftar; urut ranting + Kyu 10→1; pejabat TTD + daftar **penguji** (bukan pelatih) dari period-meta/template/org; CATATAN sabuk `SABUK X = N` rapat; nama penguji di kolom kanan CATATAN; gambar TTD opsional dari URL upload `ukt-ttd`.
...
13. Toolbar cabang: **Buat Periode**, Hari-H, **Laporan** (hub Peserta/Hasil/Administrasi), Laporan WA, **Salin Undangan** / **WA Undangan**, Cetak Nota, Biaya Sabuk, Arsip (tombol terpisah). Toolbar ranting: **Laporan** (Peserta+Hasil scoped) + **Laporan WA** + **Salin/WA Undangan** + **Cetak Nota** (manual). Sama di **Arsip UKT**.
...
| Latber walk-in publik | Aktif | `/latber` + `/api/public/latber/*`; tarif flat 45.000 (tanpa uniqueTail); KPI status+ranting+grafik; Bayar QRIS percobaan + transfer Mandiri; Sudah bayar → verifikasi admin; batal WA |
...
| Export rekap UKT | Aktif | Hub **Laporan**: tab Peserta (Print/PDF/CSV + ranting + pratinjau); tab Hasil Ujian Excel/PDF/Print Pengda (pejabat+penguji+TTD opsional, CATATAN rapat); tab Administrasi setor Pengprov (cabang) |
