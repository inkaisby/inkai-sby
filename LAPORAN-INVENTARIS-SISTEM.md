...
11h. **Load Pendaftaran UKT:** URL canonical di-sync lewat `resolveUktAdminPeriodId` (events + period-meta saja) **sebelum** fetch berat; **registrants-first** (skip pool members + billing global + attendance dump 800); **slim-when-empty** (0 peserta: skip 4 prefix exam/waiver/attendance/deposit + self-reg Prisma + allowlist regs); meta hanya dari prefix `ukt-period-meta:` (tanpa GET per-id); fees dari snapshot bila ada; dojos Prisma scoped; Inkai dashboard `retries:0`/8s; wave-2 hanya detail event bila period URL ≠ terpilih; `dojoGroups` lazy `GET /api/admin/ukt/dojo-groups`; `UktDashboard` `ssr:false` + dialog print/export/exam/add dynamic; refresh snapshot membawa self-reg meta + append peserta baru; ranting allowlist kosong = fail-closed.
