...
| 7 September 2026 | **Penyembunyian Form & Nominal Iuran pada Pengecualian Iuran (`/dashboard/iuran`):** Meng-update `IuranPage` (`src/app/dashboard/iuran/page.tsx`) dan `IuranListClient` (`src/components/member/IuranListClient.tsx`) dengan prop `isExempt`. Ketika pengecualian iuran aktif (`allowEventWithoutDues = true`), seluruh komponen pembayaran disembunyikan (formulir *Laporkan setor periode*, nominal `Rp 50.000`, pemilih tanggal bayar, dan tombol *Laporkan setor ke ranting*); tagihan belum lunas diganti dengan badge hijau **"Pengecualian iuran"**; inventaris §5/§9/§15 |
