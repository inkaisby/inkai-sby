...
- **Canvas TTD Digital Interactive** (`SignatureCanvasModal`): Modul tanda tangan digital interaktif untuk **Ketua** dan **Sekretaris**. Mendukung goresan pen beresolusi tinggi, pilihan warna tinta (Biru Tua/Hitam/Merah/Hijau), ketebalan pen, mode sentuh touchscreen/tablet (`touch-action: none`), upload file gambar TTD, reset canvas, dan **penyimpanan permanen `localStorage`** (`INKAI_TTD_KETUA` & `INKAI_TTD_SEKRETARIS`). Gambar TTD tersimpan secara otomatis dan dapat dipakai seterusnya pada surat keluar, kwitansi, nota, resume rapat, dll.
...
- **Ranting Footer List**: Daftar 33 Ranting Aktif Cabang Surabaya di bagian footer `surat-print-html.ts` menggunakan Flexbox responsif (`.footer-ranting-label` & `.footer-ranting-list`) dengan penanda nomor tebal.
