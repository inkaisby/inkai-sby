...
11b. **Rekonsiliasi setoran** (kartu di UI **cabang**): tabel Ranting / Peserta / Lunas / Total / Status setor via `buildUktDepositReconciliation`; kolom **Keterangan** = rincian peserta `Belum Bayar: n, Menunggu Ujian: m` (Lunas = Menunggu Ujian; bukan duplikat teks Status setor); **Total tagihan** menghitung nominal **net** disetorkan ke cabang (Subtotal A kotor − total CASHBACK ranting peserta lunas) sehingga nominal pada tabel Status Setoran UKT selaras 100% dengan **TOTAL** pada NOTA PEMBAYARAN UJIAN KENAIKAN TINGKAT; status setoran (diterima/reset) **hanya diubah cabang** — `PATCH /api/admin/ukt/deposit` menolak non-cabang (`canEditKyuBaru`) fail-closed; ranting tidak lagi menandai setor dari halaman UKT (diganti alur nota + verifikasi cabang). **Tulis Prisma-first** (`AppSetting` key `ukt-deposit:{eventId}:{dojoId}`) + best-effort Inkai PUT — sukses UI tidak bergantung JWT Inkai (hindari toast *Invalid or expired token*); baca deposit di dashboard/table merge Prisma atas prefix Inkai; UI update `depositMap` lokal dari response.
