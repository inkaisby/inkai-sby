...
| `/ukt` | **Roster peserta UKT publik** (tanpa login, `noindex`): periode non-arsip term berjalan; kolom No/Foto/NIA/Nama/Kyu Lama/Kyu Baru/Ranting/Status; foto dari `Member.photoUrl` Prisma (fallback `User.photoUrl`, URL kosong diabaikan; **retry tanpa kolom** bila P2022); periode dipisah dari load peserta (`loadError` ≠ “tidak ada periode”); KPI + cari filter-only + ringkasan HP; poll senyap ~30s; **bukan** daftar UKT |
...
5. **Ranting** — aksi baris: **Daftar UKT**, **Terima**/**Tolak** (mandiri), **Batal UKT** (hanya **Belum Bayar** & **Menunggu Verifikasi** — setelah cabang verifikasi / **Menunggu Ujian+** hanya cabang), **Bayar UKT** (= ajukan ke cabang → **Menunggu Verifikasi**; untuk jalur daftar ranting). Setelah daftar ranting: **Belum Bayar**. Notifikasi otomatis daftar/batal/Bayar/Terima.
...
| Cadangan produksi (ops) | Runbook | (1) Supabase Dashboard → project **inkai-db** → Database → Backups / PITR sesuai plan; (2) opsional `pg_dump` berkala ke storage terenkripsi di luar repo; (3) restore drill ke project/staging terpisah — verifikasi `count(Member)` + login backend; **jangan** restore langsung menimpa produksi; (4) `db:local:dump-from-prod` hanya untuk debug lokal, **bukan** pengganti backup |
...
| Drift skema Prisma | Proses | Setiap ubah `prisma/schema.prisma` wajib `prisma/sql/*.sql` (ADD IF NOT EXISTS) + apply ke **DB yang sama dengan `DATABASE_URL` app (Vercel)** sebelum/bersamaan deploy field baru — `db:check-drift` lokal **bukan** probe produksi; `prisma/sql/member-photo.sql` menambah + backfill `Member.photoUrl`; **JWT refresh / impersonasi tidak select `Member.photoUrl`** (foto dari `User.photoUrl`) + fail-open saat P2022/pool; roster `/ukt` retry tanpa kolom + `loadError`; **`set_msh` / `set_dues_exemption` / sync `set_nia` / create anggota / `set_name`/`set_rank`/`set_dojo`/`set_dues`/`set_documents`/`set_photo`** pakai `update`+`select` (hindari RETURNING kolom drift) + `prismaUserFacingError`; toast admin 401 = **Sesi berakhir** (bukan `Unauthorized`) |
