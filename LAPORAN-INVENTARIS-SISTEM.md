...
10b4. **Pejabat & TTD**: period-meta (`pengdaKetua`/`mshKetua`/`ketuaCabangName`/`bidangUjianName` + titles + `pengujiNames` + `*SignUrl`); autocomplete `GET /api/admin/ukt/ttd-suggest` (akun + Hitam/DAN); prefill Pengprov `Province.headName` Jatim + ketua struktur/kebijakan + konstanta MSH; template reusable `appSetting` `ukt.ttd-template` via `GET/PATCH /api/admin/ukt/ttd-template` (cabang); pad coret di layar → upload; Simpan/cetak persist otomatis ke period-meta.
