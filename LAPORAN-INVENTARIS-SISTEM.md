...
11d. **Fokus periode aktif:** resolusi mengutamakan non-arsip; judul kanonis `UKT Semester {I|II}-{tahun}`; buat periode baru mengarsipkan term yang sudah tutup; sidebar **UKT → Pendaftaran / Arsip UKT** (bukan dropdown campuran); anggota hanya melihat periode aktif. Arsipkan dari Pendaftaran mengarahkan ke Arsip; buka arsip mengembalikan ke Pendaftaran. **Pendaftaran & Arsip UKT** = **registrants-first** (tanpa pool 250/500 anggota): `allRows` = peserta terdaftar; **Belum Daftar** via cari nama → `GET /api/admin/ukt/suggest?uktEventId=` + hydrate `GET /api/admin/ukt/members?memberId=&periodId=` (stub lalu **Daftar UKT** di dropdown pencarian atau di baris tabel; tombol dropdown **hilang** jika sudah registrasi aktif); CANCELLED/REJECTED tetap boleh daftar ulang. Tolak/batal (`CANCELLED`/`REJECTED`) tidak di-ghost sebagai Belum Bayar.
...
10. **Kelola Anggota** — dropdown pencarian: **Daftar UKT** / **Daftar Latber** ke periode aktif (`active-registration-periods`); tombol hanya jika belum registrasi aktif (tanpa teks Sudah UKT/Latber).
