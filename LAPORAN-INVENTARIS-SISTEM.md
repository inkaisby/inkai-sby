...
| Kwitansi Pembayaran | Wireframe grup nav **Pembuatan** `/admin/kwitansi` + **Arsip** `/admin/kwitansi/arsip` (dummy); jenis Iuran/Prestasi/Pengeluaran/Lainnya; mode A (centang menyaring jumlah/pratinjau/daftar/cetak) + B batch multi-halaman; Periode/Nama; tab preserve state; draft `sessionStorage`; nomor KW/NP lokal berurutan; cetak resmi (tanpa watermark DRAFT); TTD jelas; cetak padat; `openHtmlPrintWindow` 1× (shared UKT/Latber); `Member.signatureUrl`; arsip `PaymentReceipt` **fase 2**; grant + soft-backfill + preset bendahara |
...
| Nav admin | Campuran | Top-level: Iuran, **Kwitansi Pembayaran** (grup Pembuatan + Arsip), Event, Absensi; **UKT** + **Latihan Bersama** sebagai grup (Pendaftaran + Arsip); grup: Keanggotaan / Konten / Sistem + badge unread pesan |
