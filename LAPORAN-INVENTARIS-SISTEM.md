...
| Nominal UKT & Latber | Flat (Tanpa kode unik) | Frontend & backend tidak menulis `uniqueTail`; `postKasFromUktPaid` & `postKasFromLatberPaid` memotong ekor unik ke ribuan (`amount - (amount % 1000)`); `allocateLatberUniqueTail` mengembalikan `uniqueTail: null` & flat fee; script `backfill-kas-flat-fee.ts` menormalisasi data historis `Billing` & `KasEntry`. |
