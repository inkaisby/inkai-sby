...
| Nominal UKT & Latber | Flat (Tanpa kode unik) | Frontend & backend tidak menulis `uniqueTail`; `postKasFromUktPaid` & `postKasFromLatberPaid` memotong ekor unik ke ribuan (`amount - (amount % 1000)`); `allocateLatberUniqueTail` mengembalikan `uniqueTail: null` & flat fee; verifikasi tagihan Latber (`billing/[id]`) otomatis mencatat **Komisi Ranting Rp 5.000** ke Kas Ranting; script `backfill-kas-flat-fee.ts` menormalisasi data historis `Billing`, `KasEntry`, dan menyinkronkan komisi Latber yang belum tercatat. |
