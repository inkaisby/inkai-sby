...
10a. **Laporan WA ranting:** daftar peserta terdaftar (isUktWaRosterRow) diurut **Kyu Lama** (Kyu 10→1, lalu Dan 1→10, lalu nama) + **rincian A/B/C/TOTAL (A+B−C)** selaras Nota untuk **A** (uang dari isUktNotaRow, B=0 di WA) + **C = jumlah peserta daftar WA × komisi** (bukan hanya baris nota) + pecahan Sudah lunas / Belum lunas + **rekening Bendahara Cabang** (`resolveUktWaBendaharaPayment`: profil `org.branch.profile` bila nomor terisi, else fallback Mandiri `LATBER_PAYMENT`; tanpa instruksi Latber Rp45rb; jarak enter antar blok rincian). WA cabang: **ringkas** (Pelaksanaan + *Jumlah UKT* + pecahan Lunas/Belum lunas rupiah + tempat/lokasi + *N Ranting* dengan Lunas/pecahan + jumlah kyu; **tanpa** rekening) atau **rinci per ranting** (sama + rekening); picker = semua ranting di tabel (bukan hanya Menunggu Verifikasi/lunas); tombol **Salin laporan** (clipboard); rekap sabuk **Kyu Lama**.
