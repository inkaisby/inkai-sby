...
| 25 Agustus 2026 | **Fix Rekapitulasi Setoran Kas UKT Net & Komisi Latber:** `aggregateKasByDojo` (`src/lib/kas.ts`) & `postKasFromUktPaid` (`src/lib/kas-hooks.ts`) kini secara otomatis menghitung nominal **Net UKT** (Subtotal A kotor − Komisi Ranting Rp 50.000/orang) serta **Komisi Latber** (Rp 5.000/peserta), sehingga pada modal **Rekapitulasi Setoran Masuk Per Ranting** (`/admin/kas`), Ranting **FORTRESS** mencatat **UKT (Lunas) = Rp 2.725.000,-**, **Komisi UKT = Rp 550.000,-**, dan **Komisi Latber = Rp 10.000,-**, 100% utuh dan selaras di seluruh 7 kolom modal kas; inventaris §9.3c/§15 |
