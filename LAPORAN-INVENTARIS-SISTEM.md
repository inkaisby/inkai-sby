...
11h. **Load Pendaftaran UKT:** satu fetch berat `fetchUktDashboardData` (tanpa light-resolve redirect ganda); URL sync hanya via `resolveUktAdminCanonicalRedirect` (tidak strip `period` saat API gagal / canonical null); filter event `isUktAdminEventTitle` (exclude Latber); `fetchUktEventsCached` memakai cache key `ukt-events-list-v4` dan kompatibel dengan payload cache lama berbentuk array saat deploy; **fallback Prisma `Event`** bila Inkai `/v1/events` gagal/kosong (selaras `/ukt` publik) + meta `AppSetting` `ukt-period-meta:`; **registrants-first** termasuk seed Prisma regs saat detail Inkai null (cabang); **slim-when-empty**; fees snapshot; dojos Prisma scoped; Inkai `retries:0`/8s; `dojoGroups` lazy; dashboard SSR lewat client re-export; refresh snapshot; ranting allowlist kosong = fail-closed.
