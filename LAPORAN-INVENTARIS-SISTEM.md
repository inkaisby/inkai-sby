...
7. **Link undangan** `/undangan/latber/[periodId]` → redirect ke `/latber?period=…` (walk-in publik); Salin/WA di toolbar admin tetap memakai URL undangan (kompatibilitas); snapshot `latber-invite:{periodId}` / sync lama boleh tetap ada, tidak lagi dipakai route publik.
...
| Undangan portal Latber | Aktif (redirect) | `/undangan/latber/[periodId]` → `/latber?period=…`; Salin/WA di toolbar admin tetap URL undangan |
...
/api/admin/latber/registrations/[id]  PATCH terima/tolak mandiri (terima tanpa lapor bayar → billing PENDING; konfirmasi bayar → WAITING_VERIFICATION), submit verifikasi, mark_paid; notif anggota+cabang; DELETE batal + tagihan (tanpa gate arsip)
