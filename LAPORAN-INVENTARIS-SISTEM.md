...
| `/ukt` | **Roster peserta UKT publik** (tanpa login, `noindex`): periode non-arsip term berjalan; kolom No/Foto/NIA/Nama/**Tanggal daftar**/Kyu Lama/Kyu Baru/Ranting/Status; **kolom Nama sticky** saat scroll horizontal (HP); foto dari `Member.photoUrl` Prisma via `memberPhotoSelect` (fallback `User.photoUrl`; skip kolom bila drift); periode dipisah dari load peserta (`loadError` ≠ “tidak ada periode”); **sticky bar** (`top-14`/`sm:top-16`): timer compact HP + cari + **filter centang multi-ranting** (`RantingCheckboxFilter`); KPI status mengikuti filter; kartu per ranting klik = toggle filter; ringkasan HP; poll senyap ~30s; **bukan** daftar UKT |
...
14b. **Roster publik** `/ukt`: tanpa login; periode non-arsip; **timer batas pendaftaran** (payload `registrationCloseAt`); KPI + cari filter (nama/NIA/ranting) + ringkasan HP; **kolom Tanggal daftar**; poll senyap ~30s (sama `/latber`); `GET /api/public/ukt/registrants`; `robots: noindex`.
...
| Roster publik UKT | Aktif | `/ukt` + `GET /api/public/ukt/registrants`; KPI/cari/ringkasan HP; poll ~30s; noindex; **periode tetap** jika load peserta gagal (`loadError`); cache hanya payload sukses; **kolom Tanggal daftar** |
