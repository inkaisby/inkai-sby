...
| Kas | Grup **Keuangan**: **Kas** `/admin/kas` + **Kwitansi** `/admin/kwitansi` + **Arsip** `/admin/kwitansi/arsip`; buku kas 7 kolom (No/Tanggal/Keterangan/Masuk/Keluar/Saldo/Kegiatan) baris datar + judul grup kegiatan (bukan rowspan); KPI + saldo bawa; Tambah / Tambah massal / impor CSV; Cetak kop logo INKAI; tutup buku bulanan (cabang); rekon Belum/Cocok; otomatis setelah verifikasi iuran/UKT/Latber (komisi Latber dua buku); kwitansi non-iuran; Prisma `KasEntry` Integer rupiah; schema `prisma db push` |
...
| Kas | Aktif (jurnal lokal) | `/admin/kas` Prisma `KasEntry`/`KasPeriodLock`; auto dari verifikasi bayar; manual/massal/impor CSV; cetak kop INKAI; tutup buku; rekon; grant `/admin/kas`; bukan backfill Excel |
...
/api/admin/kas                 GET buku kas scoped; POST Tambah/massal (sourceType manual/kwitansi)
