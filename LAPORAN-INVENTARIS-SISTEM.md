...
| `/ukt` | **Roster peserta UKT publik** (tanpa login, `noindex`): periode non-arsip term berjalan; kolom No/Foto/NIA/Nama/Kyu Lama/Kyu Baru/Ranting/Status; foto dari `Member.photoUrl` Prisma (fallback `User.photoUrl`, URL kosong diabaikan); KPI + cari filter-only + ringkasan HP; poll senyap ~30s; **bukan** daftar UKT |
...
| Roster publik UKT | Aktif | `/ukt` + `GET /api/public/ukt/registrants`; KPI/cari/ringkasan HP; poll ~30s; noindex |
...
| Drift skema Prisma | Proses | Setiap ubah `prisma/schema.prisma` wajib `prisma/sql/*.sql` (ADD IF NOT EXISTS) + apply ke produksi sebelum/bersamaan deploy field baru; `prisma/sql/member-photo.sql` menambah + backfill `Member.photoUrl`; gate lokal `npm run db:check-drift` (hanya vs `DATABASE_URL` yang dituju); **`set_msh` / `set_dues_exemption` / sync `set_nia` / create anggota / `set_name`/`set_rank`/`set_dojo`/`set_dues`/`set_documents`/`set_photo`** pakai `update`+`select` (hindari RETURNING kolom drift) + `prismaUserFacingError`; toast admin 401 = **Sesi berakhir** (bukan `Unauthorized`) |
