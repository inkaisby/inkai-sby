...
10. Cetak nota memakai tabel biaya sabuk bulat; **tanpa kode unik** (+1…999). Nomor nota memuat semester (`UKT/SBY/{RANTING}/I|II/{tahun}`); opsi **Gabungan** di modal (admin multi-ranting: ranting dikelola; cabang: semua ranting) → `UKT/SBY/GABUNGAN/{smt}/{tahun}` + agregat peserta/sabuk/komisi/TOTAL setor ke cabang; pilih ranting satuan tetap seperti semula. Pejabat (Bidang Ujian / Bendahara) dari **period-meta per periode**, fallback **Pengaturan → Kebijakan**.
