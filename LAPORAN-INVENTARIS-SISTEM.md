...
| Kas | Aktif (jurnal lokal) | `/admin/kas` Prisma `KasEntry`/`KasPeriodLock`; mode **Laporan Detail** (default, flat + inline edit semua sourceType + Maximize full halaman; saat full: Refresh + periode + Cetak di header) / **Buku** (grup lipat; Ubah semua sourceType, Hapus manual); filter periode awal–akhir; Ubah/Hapus/Pindah lokasi (1 baris + centang batch + Pindah kegiatan); **Tambah item** di header grup kegiatan; **Tambah massal multi-tanggal**; sticky laptop / ringkasan kolaps HP; auto dari verifikasi bayar; manual/massal/impor CSV; cetak kop INKAI; tutup buku; rekon; grant `/admin/kas`; bukan backfill Excel |
