...
| `/ukt` | **Roster peserta UKT publik** (tanpa login, `noindex`): periode non-arsip term berjalan; kolom **centang**/No/Foto/NIA/Nama/**Tanggal daftar**/Kyu Lama/Kyu Baru/Ranting/Status; **sort kolom** (ikon header); **kolom centang + Nama sticky** (`left-10`); foto dari `Member.photoUrl` Prisma via `memberPhotoSelect` (fallback `User.photoUrl`; skip kolom bila drift); periode dipisah dari load peserta (`loadError` ≠ “tidak ada periode”); **sticky bar** (`top-14`/`sm:top-16`): timer compact HP + cari + **filter centang multi-ranting** (`RantingCheckboxFilter`) + **Cetak** (modal pilih ranting, A4/F4, portrait/landscape; modal tetap terbuka saat Print; centang tabel opsional batasi cetak); KPI status mengikuti filter; kartu per ranting klik = toggle filter; ringkasan HP; poll senyap ~30s; **bukan** daftar UKT |
...
| `/latber` | **Pendaftaran Latber walk-in (publik):** info login NIA + Pesan; **sticky bar** (timer compact HP + cari + **filter centang multi-ranting** + **Salin WA** Cormat + **Cetak daftar peserta** (dialog centang ranting, kertas A4/F4, portrait/landscape, kolom ☐ sebelum No) + Scan/**Tambah Peserta** (stub tamu cepat)/**Tambah Anggota**/Ringkasan); KPI status mengikuti filter; KPI per ranting klik = toggle + grafik; cari/scan QR; **Tambah Anggota** = form daftar penuh; **Tambah Peserta** = nama+ranting → stub `PENDING` + daftar Latber Belum Bayar (flag `latber-guest:{id}`); tabel (NIA/**badge Tamu**, **Tanggal daftar**, flat **Rp 45.000**); **kolom Nama sticky**; badge Status (lunas hijau / **tunai teal** / belum bayar/verifikasi amber); **Bayar** QRIS+Mandiri + Sudah bayar; batal WA; **tanpa** aksi Tunai/promote (admin only); `/admin/latber` tanpa sesi → sini; OG "Latihan Bersama" |
...
| Absensi | Aktif | Streaming UI; check-in GPS multi-lokasi (auto geofence 150m + fallback 500m + override "Bukan di sini?"); **kamera live Scan Kode QR Ranting** (`BarcodeDetector` + WebRTC video feed); **Scan Kode QR Ranting mendukung scan QR dari dojo/ranting/tempat lain (multi-ranting)** — melegitimasi fisik & bypass geofence lintas ranting + **modal konfirmasi lokasi absensi** ("Apakah benar lokasi absensi di [Nama Dojo]?" dengan opsi *Ya, Lanjutkan Absen*, *Ulangi Scan*, dan *Batal*); tombol Barcode Dojo Saya disembunyikan; biometrik HP opsional (WebAuthn); QR collapsible; riwayat; 1×/hari |
...
| Modul | Fungsi |
|-------|--------|
| Beranda Admin | KPI anggota, iuran pending, event, verifikasi, **pesan unread**; aksi cepat role-aware + notifikasi; **ikon back** di topbar (kecuali beranda); **dual-role: menu Dashboard Anggota**; **topbar chip kegiatan Masih terbuka** (pulse + rotasi judul + panel); **Tanya INKAI** FAB (global, skip undangan); **kepadatan shell 14"** (`.admin-app`: padding/topbar `h-12`/sidebar `w-64` sampai `2xl`, KPI & `th`/`td` lebih rapat di `lg`–`xl`; HP tetap `h-10`) |
| Kelola Anggota | Cari **autocomplete** (nama/NIA; placeholder MSH belum filter suggest) — **ketikan memfilter tabel** (`q` → Prisma; **fallback pencarian otomatis lintas status** bila pencarian nama/NIA dengan filter status menghasilkan 0 baris); **quick-reg Daftar UKT/Latber** dari dropdown bila `canQuickReg` (periode aktif via `active-registration-periods` + **fallback Prisma** bila Inkai kosong) **dan kolom Aksi** (baris ACTIVE/PENDING; flags `eventRegistration` batch `GET /api/admin/members?uktEventId&latberEventId`; shared `EventQuickRegisterButtons`) — klik nama saran = filter tabel; saran tampil **NIA · sabuk · ranting** via hydrate Prisma; tombol event **hanya bila periode aktif & pendaftaran masih terbuka** (`isUktRegistrationOpen`/`isLatberRegistrationOpen`; sembunyi otomatis jika periode diarsip/dikunci/tutup) — jika sudah terdaftar atau pendaftaran tutup tombol **tidak tampil**, tanpa label Sudah UKT/Latber; **klik baris/kolom tampilan** → sheet detail (ranting semua kolom; cabang hanya kontrol inline yang stopPropagation); **section Kode QR / Barcode Kartu Anggota** di sheet detail (pratinjau QR, **Salin Link QR**, **Cetak Barcode** single); **Cetak Barcode Massal / Cetak Kartu** via tombol toolbar **Cetak Kartu**, bilah centang massal (`BulkDeactivateBar`) & dropdown **Export** (`MemberBarcodePrintModal`: **Ukuran KTP / CR80 Standar 85.6×54 mm** 10 kartu/A4, Printer PVC Single, 3/4 kolom, A4/Folio F4 + **Filter Cabang**, **Filter Ranting**, **Pencarian Nama/NIA Anggota**, dan **Centang Multi-Select Anggota [Pilih Semua / Hapus Pilihan]** untuk memilih/memilih beberapa anggota spesifik yang akan dicetak kartunya); kolom **No** (sembunyi HP); **Nama sticky** + **Dojo tampil semua breakpoint** (subtitle ranting di bawah nama di HP; arsip sama); **sort kolom** (NIA, Nama, Sabuk, Status, Dojo, Terdaftar — ikon naik/turun, server-side); KPI status + **Dok. kurang** + **Tanpa NIA** + **Tanpa akun** + **Duplikat NIA/NIK**; badge baris tanpa akun; bulk **Buat akun login** (retry); **auto-provision** User saat create/set NIA/bulk ber-NIA; **cabang & Pengprov edit NIA** inline di kolom (`set_nia`) + **Nama** inline (`set_name`) + **Dokumen** (tombol Ubah di kolom); **ranting edit Nama/Dokumen/Identitas (NIK/TTL/JK/alamat/kontak) di detail**; **upload Akte/BPJS** di detail; **upload foto** di header sheet (klik avatar → modal unggah drop/paste/pilih file + crop 1:1, kompres ~100 KB, `set_photo`); foto disimpan kanonis di `Member.photoUrl` agar persisten walau anggota belum punya akun, dan dicerminkan ke `User.photoUrl` bila akun tersedia; pratinjau modal + print; detail, NIA; **No. MSH** (Hitam/DAN — kolom + edit ranting/cabang `set_msh` di detail); **Terdaftar**; **edit Iuran/bln**; **pengecualian iuran (event/UKT)**; **edit Sabuk + pindah ranting** (cabang/administrator: Radix Select + `stopPropagation` di sel — hindari klik sisa native `<select>` ke tombol Kembali `/admin`) + **dropdown Dojo di sheet detail**; **ranting, cabang & administrator pusat edit identitas (NIK/TTL/JK/alamat) + kontak (email login/telepon)** via `set_identity`/`set_contact`; nonaktif/bulk; **Export** dropdown **Cetak Barcode Massal / CSV / PDF / Print / WA** (semua filter aktif, cap 2000; WA format `{n}. NAMA, Sabuk`; PDF/Print roster No+Nama+Sabuk); arsip; Prisma scoped (+ **anggota luar Surabaya / ranting arsip** tetap terlihat); **filter/switcher client-fetch** (`counts=1` hanya ganti dojo); mutasi tanpa `router.refresh`; **Input Massal** (NIA…Kyu…Ranting, isi semua Kyu/DAN, progress %, maks 50); create/bulk **sinkron NIA ke Prisma lokal**; detail: **username login dari Prisma** (bukan hint palsu); **Reset password** sementara (ranting/cabang) |
| Iuran Anggota | **Rekening koran per anggota** (tabel: No, Nama, NIA, Ranting, Iuran/bln, Status bulan, Tunggakan, Aging, **Pengecualian**); **filter + DojoContextSwitcher client-fetch** (`GET /api/admin/iuran/ledger` + `replaceState`, tanpa full RSC); klik nama → Sheet **Pengaturan / Mutasi / Pembayaran**; strip antrian verifikasi; generate bulan; export rekap CSV; deep-link `?memberId=&tab=`; lunas/verifikasi menulis jurnal **Kas** (setor ranting→cabang dua buku) |
| Kas | Grup **Keuangan**: **Kas** `/admin/kas` + **Kwitansi** `/admin/kwitansi` + **Arsip** `/admin/kwitansi/arsip`; **Modal Konfigurasi Cetak Kas Fleksibel (`handlePrint` & `handleExecutePrint` di `KasLedgerClient.tsx`)**: saat menekan tombol **Cetak** (ikon Printer), sistem menampilkan modal dialog interaktif dengan opsi pilihan bebas: (1) **Jenis Dokumen** (**📖 Buku Kas Umum Klasik** ringkasan global per kegiatan vs **📄 Laporan Detail** visual), (2) **Ukuran Kertas** (**A4** `210×297 mm` vs **F4 / Folio** `215×330 mm`), dan (3) **Orientasi Kertas** (**📱 Portrait / Tegak** vs **🖥️ Landscape / Mendatar**); `@page` CSS (`getKasPageSizeCss`) menyesuaikan ukuran kertas & margin cetak secara otomatis tanpa terpotong; **Bilah Melayang Aksi Massal & Cocok Rekening (`POST /api/admin/kas/batch-recon` & `KasLedgerClient.tsx`)**: bilah melayang (*floating action bar*) yang muncul saat centang transaksi (`selectedIds`) dilengkapi tombol **`✓ Cocok rekening (n)`** (1-klik tandai seluruh transaksi terpilih sebagai *matched* / cocok rekening bank), tombol **`🔓 Buka rekon (n)`** (kembalikan ke *open* / belum rekon jika ada item cocok yang dicentang), serta **Kalkulator Subtotal Terpilih Instant** (`selectedSubtotals`: menampilkan jumlah item, total masuk, total keluar, dan saldo bersih/net live dari baris-baris dicentang tanpa hitung kalkulator manual); **Grafik Keuangan Interaktif Multi-Mode & Analisis SWOT Cetak (`KasChartSwotPanel.tsx` & `kas-print-html.ts`)**: panel grafik visual SVG di bawah kartu KPI dengan **Filter Multi-Pilihan Kegiatan** (`uniqueKegiatanList`, multi-select dropdown & pill toggle untuk memilih satu atau beberapa kegiatan spesifik/semua), **4 Mode Visualisasi Berbeda** (📊 Tren Bulanan Bar Chart, 🍩 Proporsi Donut Chart SVG, 📈 Arus Kas Bersih Area Line SVG Chart, dan ⚖️ Distribusi & Efisiensi Kegiatan), **Filter Arah Mutasi Interaktif** (Semua / Pemasukan / Pengeluaran), serta **Mesin Analisis SWOT & Rekomendasi Strategis Keuangan Otomatis** (kalkulasi 4 Kuadran: *Strengths*, *Weaknesses*, *Opportunities*, *Threats* + Rekomendasi Strategis Komprehensif mencakup dana cadangan darurat, rekonsiliasi, unit usaha merchandise/karategi resmi, event kejuaraan/coaching privat, recurring income iuran rutin, & sponsorship/kemitraan); **Cetak Laporan Keuangan A4/F4 Landscape Output Lengkap**: cetakan laporan menyertakan Kop INKAI, **100% Seluruh Grafik Visual Keuangan & Seluruh Kegiatan** (🍩 Proporsi Donut Chart SVG, 📈 Area Line Chart Net Cash Flow SVG, dan ⚖️ Bar Chart Efisiensi Mutasi 100% Semua Kegiatan tanpa batasan slice/potongan), Matriks 4-Kuadran SWOT, Rekomendasi Strategis, dan Tabel Mutasi Detail; **Presisi Nominal Kas 1:1**: `normalizeKasDisplayAmount` menampilkan 100% nominal asli sesuai database tanpa pembulatan potongan ribuan (`amount % 1000`) sehingga angka saldo & mutasi cocok presisi 1:1 dengan Google Spreadsheet; **Hak Akses Wilayah & Scope Kas**: Hanya email Super Admin (`inkaisby@gmail.com` / `inkai.sby@gmail.com`) yang berhak melihat dan berpindah ke seluruh buku Kas wilayah (cabang & 100% semua ranting tanpa terpotong `buildBranchFilter`/`buildDojoFilter`), memiliki hak akses penuh `canWrite` (+ Tambah, Impor, Edit, Hapus, Pindah), serta memindahkan/koreksi entri kas; user admin lainnya (pengurus cabang/ranting biasa) **dibatasi secara ketat hanya pada scope wilayahnya sendiri** (apabila mencoba membuka URL dengan `scopeType` / `scopeId` milik wilayah left, server dan client otomatis me-redirect ke scope wilayahnya masing-masing); **Auto-Fill Nominal Tagihan / Setoran Ranting**: Modal **Tambah Mutasi** dilengkapi selector cepat `⚡ Ambil dari Total Tagihan / Setoran Ranting` (dropdown pilihan ranting & breakdown tagihan UKT/Latber/Iuran yang otomatis mengisi Keterangan, Kegiatan, Arah Masuk, dan Nominal) serta tombol `⚡ Ambil ke Kas` pada setiap baris tabel modal **Rekap Per Ranting** untuk menginput setoran kas 1-klik tanpa ketik angka manual; toggle **Laporan Detail** (default) vs **Buku**; Laporan Detail = flat 7 kolom seperti Excel/cetak (**LAPORAN KEUANGAN DETAIL** + badge Saldo akhir) + **edit inline** Tanggal/Keterangan/Masuk/Keluar/Kegiatan (**semua sourceType** inkl. otomatis iuran/UKT/Latber/kwitansi; bulan terbuka; `PATCH /api/admin/kas/[id]`; Hapus 🗑️ selalu tampil pada modal/tabel) + **badge sumber `manual`/`otomatis` dengan tautan audit** + **centang multi-select, hapus massal & tombol ikon Salin WA (terpilih)** + ikon **Maximize/Minimize** full halaman in-app (`fixed inset-0`, Esc; saat full: **Refresh** + **periode Dari–Sampai** + ikon **Cetak** kiri Saldo); **filter arah mutasi** (Semua/Masuk saja/Keluar saja); **Preset Tanggal Cepat** (tombol pill 1-klik: **Bulan Ini**, **Bulan Lalu**, **Semester UKT**, **Tahun Ini**, **Semua Tanggal**); **tombol Reset filter**; **badge interaktif Belum rekon** pada KPI (1-klik filter belum rekon); **tombol Salin WA** (format Laporan Kas WA rapi & terstruktur: Buku Kas, Periode, Saldo Awal, Total Masuk, Total Keluar, Surplus/Defisit, Saldo Akhir, rincian `• [MASUK]` & `• [KELUAR]`); **Rekap Per Ranting** (dialog modal ringkasan setoran UKT/Latber/Iuran/Lainnya per ranting per periode + KPI banner + **Sub-Breakdown TANPA RANTING / UMUM** + kolom **Status setor UKT** baca `ukt-deposit` periode overlap tanggal via `GET /api/admin/kas/ukt-deposit` + join `dojoId`/nama; TANPA RANTING = **—**; tautan **Buka UKT**; tanpa Terima/Reset di Kas + kalkulasi CASHBACK UKT Rp 50rb/anggota, Latber Net Rp 40rb/anggota, CASHBACK Latber Rp 5rb/anggota + **Salin Teks WA** terstruktur termasuk status + **klik baris drill-down filter** + **Daftar Seluruh Ranting Resmi**: menyertakan 100% Ranting resmi dari `officialDojoList` & `uktDepositMap` meskipun Ranting tersebut belum mencatatkan mutasi di Buku Kas pada rentang tanggal terpilih sehingga selalu dapat dicari & dipilih); **Modal Tambah Mutasi Explorer, Anti-Duplikat & Breakdown Accordion Ranting**: modal Tambah Mutasi dilengkapi **Toggle Mode Form vs ⚡ Explorer Setoran Ranting (UKT & Latber)**; di mode Explorer admin dapat melihat daftar **Total Tagihan / Setoran per Ranting** (nama ranting, nominal setoran, status setor) dengan opsi **Pilih 1 Item** (auto-fill form) atau **Centang Multi-Select Ranting** (1-klik **Simpan Multi-Batch** ke Kas) + **Indikator Anti-Duplikat** (badge font-bold `✓ Dicatat di Kas (Rp XXX)` bila setoran Ranting sudah pernah diinput di Kas + peringatan toast `toast.warning` jika memilih item yang sudah tercatat) + tab **Preset Mutasi Rutin** (8 templat transaksi cepat) + **Rincian Collapse/Expand per Ranting (Accordion Chevron `>`/`v`)**: setiap baris Ranting di Rekap & Modal Explorer dapat dibuka-tutup untuk menampilkan **Breakdown transaksi kas terdaftar** dan **Link Pintasan Langsung 🔗 "Lihat Daftar Nama Anggota & Pembayaran Lunas di Halaman UKT"** (`/admin/ukt?dojo=<dojoId>`); mode Buku = grup kegiatan **bisa dilipat** (**default dilipat** saat muat/ganti filter; terbuka jika filter Kegiatan spesifik); KPI berwarna per periode + **Semua tanggal**; Tambah / **Tambah massal** (kolom **Tanggal per baris**, **Tanggal default**, tempel Excel append multi-batch tanggal berbeda, tanggal ID, pre-check bulan terkunci, counter baris, chevron daftar, **paste Excel maks 500**, **Lokasi** buku untuk cabang/admin, reset form tiap buka) / **Ubah+Hapus manual** (**Kegiatan** pilih dari daftar atau ketik baru) / **Pindah+Hapus lokasi** (cabang-only: 1 baris, **centang multi** pindah/hapus, **Pindah kegiatan** grup + **modal konfirmasi hapus dilengkapi pratinjau daftar rincian transaksi terpilih** + **tombol Salin WA transaksi terpilih di samping centang toolbar floating**) / **Tambah item** & **Hapus kegiatan 🗑️** di header grup kegiatan (`canWrite`, prefill Kegiatan, auto-buka grup setelah simpan, modal konfirmasi hapus massal `deleteKasByKegiatan` + `POST /api/admin/kas/delete-kegiatan` menyertakan pencocokan `getKasBaseKegiatan`, kueri *bulk batch* `deleteMany`/`updateMany` cepat tanpa timeout transaksi Prisma 5000ms, & *Type-to-Confirm* case-insensitive bila item > 5 & tombol *Salin WA Rincian*); layout laptop sticky header/KPI/toolbar + HP ringkasan kolaps; **perilaku scroll diselaraskan dengan UKT** (`overflow-x-auto overflow-y-clip` pada mode biasa tanpa scrollbar ganda internal; mode fullscreen flex-col viewport); impor CSV; Cetak kop logo INKAI; tutup buku bulanan (cabang); rekon Belum rekon/Cocok rekening; auto-sync background disesuaikan agar Kas murni dikelola secara manual oleh pengurus tanpa override otomatis; kwitansi non-iuran; Prisma `KasEntry` Integer rupiah; schema `prisma db push` |
| Kwitansi Pembayaran | Nav **Kwitansi** `/admin/kwitansi` + **Arsip** `/admin/kwitansi/arsip` di bawah Keuangan; logo INKAI + kop Cabang Surabaya + cap LUNAS; mode Per Penerima vs Rangkuman; **tab Pengeluaran Event**: mendukung **Input Massal** (`KwitansiBulkNotaItemDialog`, shortcut `Alt+Shift+A`, input multi-baris tabel + parser tempel Excel TSV/CSV `Deskripsi [TAB] Jumlah [TAB] Harga [TAB] Petugas` + pratinjau kalkulasi live); edit arsip via `?no=` memuat data kwitansi beserta **100% rincian item barang/penerima**; simpan arsip memperbarui data eksisting berdasarkan nomor dokumen (in-place update tanpa duplikat) + **pilihan Singkronisasi Jurnal Kas** (**Kas Masuk (+)** / **Kas Keluar (-)** / **Tidak Catat di Kas**) yang otomatis mencatat, memperbarui, atau menghapus entri terkait di `/admin/kas`; **sinkronisasi tanggal otomatis**: tanggal transaksi di Kas (`KasEntry.txnDate`) selalu diselaraskan secara presisi dengan tanggal kwitansi (`parseToYmd(tanggal)`) di Arsip; **auto-sync status badge & tanggal saat memuat Arsip Kwitansi**; **tabel Arsip menampilkan badge Status Kas** (Kas Masuk / Kas Keluar / Belum Sync) + **modal pilihan arah kas 1-klik pada tombol 🏛️ Kas**; **export CSV arsip memuat kolom Status Kas**; **hapus kwitansi di Arsip otomatis menghapus entri Kas terkait** (`DELETE /api/admin/kas?sourceType=kwitansi&sourceId=`); pembersihan duplikat otomatis di Arsip; **field Nominal/Harga format otomatis titik pemisah ribuan** (mis. `100.000`); export CSV rekap; nomor KW/NP; TTD; `openHtmlPrintWindow`; grant + preset bendahara + Kas |
| UKT | Sidebar grup **UKT (Ujian Kenaikan Tingkat)** → **Pendaftaran** (`/admin/ukt`) + **Arsip UKT** (`/admin/ukt/arsip`); **navigasi URL kanonikal & alur resolusi**: `/admin/ukt` (root default entry point yang mendeteksi term berjalan & meluncurkan smart redirect ke periode aktif terkini) vs `/admin/ukt?semester=...&year=...&period=...` (halaman operasional pendaftaran aktif terikat UUID periode; auto-redirect ke `/admin/ukt/arsip` jika periode di-lock/archived) vs `/admin/ukt/arsip` (halaman read-only riwayat & hasil ujian periode selesai); **mode pendaftaran baru** (`create=1`); **biaya dinamis vs fee snapshot** (pendaftaran pakai template dinamis, arsip terkunci dengan snapshot finansial persisten); **registrants-first** (peserta terdaftar; Belum Daftar via suggest+hydrate); foto peserta di-hydrate dari `Member.photoUrl` Prisma (fallback `User.photoUrl`, Prisma menang atas string kosong/stale dari Inkai); **shell streaming**: header + semester/tahun tampil dulu, KPI/tabel Suspense menyusul; periode aktif, **sort kolom & multi-column sort (urutan bertingkat)** (header kolom mendukung Shift+klik & Multi-Sort Pill Bar, mis. Ranting A-Z + Kyu Lama 10-1 + Nama; badge nomor urut sort 1, 2, 3 di header), **filter Ranting & Kyu spesifik di toolbar** (dropdown `Semua Kyu` / Putih Kyu 10, Kuning Kyu 8, dsb.), **Centang Khusus per Kyu & Ranting** (dropdown header checkbox `[ ] ˅`: centang cepat peserta Kyu 10, Kyu 8, atau per Dojo AIRLANGGA, dsb.), multi-select ranting, **filter Gabungan multi-ranting**, **Scan Express Kyu** (`UktQrKyuScannerModal`: tombol ⚡ **Scan Express Kyu** di toolbar sticky untuk mengunggah Kyu Baru instan lewat alat scanner barcode USB/Bluetooth maupun kamera HP live; rekomendasi sabuk otomatis + preset 1-klik Kyu 9 s/d DAN 1 + lompat +1/+2/+3 Kyu + konfirmasi beeping audio + **Riwayat Scan & Update Sesi Ini** realtime dengan konfirmasi timestamp, total peserta diproses, dan tombol edit ulang cepat), bayar→verifikasi cabang, sabuk target, nota, **Laporan/Rekapan** (hub tab Peserta / **Rekap Matrix** (rekapan pendaftaran peserta ujian per ranting × sabuk Putih/Kuning/Hijau/Biru/Cokelat/Jumlah + cetak PDF Kop Resmi TTD pejabat + WA + CSV Excel) / Hasil Ujian / Administrasi — cabang; ranting/Pengprov tanpa Administrasi), **hari-H**, **setoran + rekonsiliasi** (tabel di bawah KPI dilengkapi 5 kolom matriks sabuk `PUTIH | KUNING | HIJAU | BIRU | COKELAT` sebelum `PESERTA` + indikator **centang hijau `✓ Lunas`** di Keterangan bila lunas), **arsip**, wizard (ujian/pejabat/snapshot biaya); **HP:** timer tetap; aksi toolbar → **Lainnya**; jadwal/setoran/KPI/alur → **Ringkasan**; cari sticky di bawah topbar+TermNav (`top-[6.75rem]` / `2xl`); saran cabang tampil **ranting** + tombol **Daftar UKT** di dropdown (hilang jika sudah terdaftar; `uktEventId` di suggest); **kolom Aksi** **Daftar UKT** (shared `EventQuickRegisterButtons`; Waiver cabang tetap); HP **Daftar UKT** dekat nama; **kolom Nama+checkbox sticky** (satu scroller; HP vertikal scroll halaman via `overflow-y-clip` pada wrapper tabel); **Centang Khusus** (dropdown header checkbox: per Sabuk/Kyu 10, Kuning, Hijau, Biru, Coklat, Belum Isi Kyu Baru); **Bulk Action Kyu Baru** (floating bar: manual target Kyu Baru, ⚡ **Set Standar INKAI** Kyu 10 ➔ Kyu 8, 🧹 **Bersihkan Kyu Baru**); ranting subtitle HP + kolom `sm+`; **kolom Tanggal daftar** (sort; tampil di mode ringkas); kolom default ringkas; **toolbar atas sticky**; **ikon Printer Cetak Nota** di toolbar sticky **tabel** (semua breakpoint, termasuk mode full); modal Cetak Nota **tetap terbuka** setelah Print; **ikon Maximize/Minimize full halaman in-app** (`fixed inset-0`, Esc; header Refresh+Laporan/Rekapan; tanpa auto-refresh saat maximize; FAB Tanya INKAI disembunyikan); **Kyu Lama+Baru Select Radix** (cabang/administrator) di **tabel biasa dan full** (Kyu Lama = snapshot `update_kyu`, Kyu Baru = alur Selesai; Esc pada dropdown tidak menutup full); saat full: tampilan Lengkap, tanpa kolom Dokumen; **penyamaan toolbar UKT**: Cabang, Pengprov (`ADMIN_PROVINCE`), dan Ranting mendapatkan akses ke tombol toolbar periode (**Hari-H**, **Laporan/Rekapan**, **Laporan WA**, **Salin/WA Undangan**, **Cetak Nota**, dan dropdown **Lainnya** berisi Biaya Sabuk & Syarat UKT) serta tombol **Tambah Anggota** dan filter **Select Ranting**; **ekspor CSV & Excel (.xlsx) Peserta**: tab Peserta menyediakan tombol **Excel (.xlsx)** presisi sesuai format formulir cabang (Kop logo INKAI, 10 kolom berborder tipis, alignment presisi, lebar kolom disesuaikan) + tombol pilihan **Ukuran Kertas (A4 / F4 Folio)** untuk Excel & Print; **ranting: Daftar/Batal (sebelum Menunggu Ujian)/Bayar + toolbar Laporan/Rekapan (Peserta+Hasil scoped) + Laporan WA (rincian setor), Salin/WA Undangan & Cetak Nota**; cabang: **edit NIA inline** (set_nia) + **Hapus tagihan** terpisah dari hapus pendaftaran + **Hapus pendaftaran termasuk lunas**; **Pengprov: edit NIA inline** (tanpa aksi cabang limited); **undangan portal** `/undangan/ukt/[periodId]` + **roster publik** `/ukt`; **penyelarasan URL period**: auto-detect semester/tahun dari judul event (mis. period ID Semester I yang diakses lewat URL `semester=II` akan otomatis disinkronkan ke semester/tahun kanonis event tanpa berkedip/redirect-loop); **loader registrasi Prisma**: filter status registrasi Prisma disamakan (`notIn: ["CANCELLED", "REJECTED"]`) sehingga peserta berstatus `PAID`/`CONFIRMED`/`REGISTERED` tetap dihitung dan dimuat penuh tanpa salah terdeteksi sebagai periode kosong |
| Latihan Bersama | Nav grup **Pendaftaran** (`/admin/latber`) + **Arsip** (`/admin/latber/arsip`); exact-match sidebar; registrants-first + **Tambah Peserta** (stub tamu); **kolom Tanggal daftar** + badge **Tamu**; status **Belum Bayar / Menunggu Verifikasi / Lunas / Tunai** (`isLatberPaidStatus` = lunas\|tunai); aksi **Tunai** (`mark_cash`, Payment `CASH`, ranting+cabang) + cabang **Verifikasi** (`mark_paid` → Lunas); **Tambah keanggotaan** promote admin-only; periode fee/komisi; `periodLocked`; **cari + Daftar Latber di dropdown** (`suggest?latberEventId=`; tombol hilang jika sudah terdaftar aktif) **+ kolom Aksi Daftar Latber** (shared); **Cetak Nota** (`filterLatberApprovedRows` / `isLatberNotaRow`, CASHBACK × semua baris nota) multi-ranting + kolom **Status + Hadir ☐ + Biaya**; **Laporan WA** salin clipboard (selaras UKT); Hapus lunas void kas + kredit LATBER; Rekap/Laporan WA/Undangan; publik `/latber` **Cetak daftar** modal tetap terbuka setelah cetak |
...
| Sekretariat | Grup **Sekretariat** (`/admin/sekretaris`): **Dashboard Sekretariat** (`/admin/sekretaris`), **Surat & Persuratan** (`/admin/sekretaris/surat`), **Generator Surat** (`/admin/sekretaris/generator`), **Dokumen & SK** (`/admin/sekretaris/dokumen`), & **Notulen Rapat** (`/admin/sekretaris/rapat`); **Urutan Sidebar Kanonikal**: Grup Sekretariat diletakkan langsung di bawah **Keanggotaan** (sebelum Keuangan/Kas & Iuran Anggota) untuk menyatukan area administrasi utama keorganisasian; **Ikon Navigasi Lengkap**: Ikon `ScrollText` untuk grup Sekretariat serta ikon individual untuk seluruh item anaknya (`LayoutDashboard` untuk Dashboard, `Mail` untuk Surat & Persuratan, `FileEdit` untuk Generator Surat, `FolderArchive` untuk Dokumen & SK, `Notebook` untuk Notulen Rapat); 100% mendukung dual mode toggle terang & gelap (`dark:`); penomoran otomatis `[No]/INKAI-KOTA.SBY/[Kode]/[Bulan-Romawi]/[Tahun]`, generator cetak A4/F4 presisi 1:1 Kop FORKI (sebelah kiri header kop; logo resmi FORKI terbaru tertanam persisten via `LOGO_FORKI_BASE64` & `public/logo-forki.png`) & INKAI (sebelah kanan header kop; tertanam terintegrasi Base64 persisten), **Stempel Resmi INKAI Kota Surabaya** (stempel keorganisasian resmi berlogo sabuk INKAI Kota Surabaya versi terbaru terpasang otomatis pada tanda tangan digital/sistem via `STEMPEL_INKAI_BASE64` & `public/stempel-inkai.png`), templat **Surat Tugas**, **Surat Undangan**, **Surat Keterangan**, **Rekomendasi**, **SK Pengurus**, dan **Tab Resume Rapat / Notulensi** + **Impor Otomatis Notulensi Rapat Tersimpan**, **Editor Visual WYSIWYG Langsung In-Place & Style Toolbar Aktif Realtime** (pelacakan *active selection & cursor state* secara otomatis: saat kursor/blok teks berada di posisi kata dengan format seperti kata berhuruf tebal, miring, garis bawah, coret, perataan kiri/tengah/kanan/justify, atau daftar berbutir/bernomor, ikon tombol toolbar seperti **B**, *I*, <u>U</u>, dsb. otomatis menyala aktif dengan warna merah kustom), **Tata Letak Rapi Footer Ranting**: daftar ranting/dojo resmi di bagian bawah cetakan surat menggunakan layout Flexbox terpisah (`.footer-ranting-label` & `.footer-ranting-list`) dengan nomor tebal tersusun rapi tanpa ada teks yang tergeser miring/menimpa di bawah label `RTG :`, **Toolbar Toolkit Lengkap**, **Pratinjau Live PDF Cetak Kertas Utuh Tanpa Terpotong** (margin kertas A4/F4 presisi 16mm di layar + bingkai responsif proporsional dengan kontrol zoom +/- & mode layar penuh 100% utuh tanpa teks terpotong di tepi), TTD Digital/Manual/Draw, QR Code verifikasi publik (`/v/surat/[id]`), delivery ke portal & notifikasi anggota (`/dashboard/dokumen`), kearsipan SK pengurus dengan alert kadaluarsa, & notulensi rapat dengan action items & salin format WA; **Navigasi Sidebar Dinamis Single-Active**: perbaikan penentuan link aktif di `SidebarNavGroup` menggunakan algoritma *dynamic longest-prefix matching* pada link bersaudara sehingga ketika berada di sub-halaman seperti Generator Surat (`/admin/sekretaris/generator`), hanya menu spesifik tersebut yang tersorot merah aktif |
| Pengaturan | User digabung ke **Ranting & User**; tombol **QR Dojo / Cetak Poster QR** (`DojoQrModal`) per ranting (poster A4 resmi ber-kop INKAI untuk dipasang di dojo; string ID/payload mentah disembunyikan agar UI bersih); cabang edit data ranting + **email/password** PIC di form Ubah Data; panel Akun: **Jadikan admin ranting** (email anggota existing → dual-role) + **centang hak akses** (edit profil, CRUD, menu sidebar); **Pengaturan Cabang** mendukung **Jadikan admin cabang** dari akun existing (mis. ketua cabang) tanpa akun baru + badge **Admin + Anggota / Admin saja**; admin ranting: form **Ubah Data** lengkap (multi-ranting) + **email/password** di **Akun Saya**; multi-akun (Akun), kebijakan, **Pengaturan UKT (syarat daftar)**, peran (**preset**), geofencing (**pratinjau peta**), akun; **arsip cabang: Pulihkan + Hapus permanen** (ditolak jika masih ada anggota / cabang SURABAYA) |
...
|------|----------------|---------|--------|----------|
| Profil & akun | Edit profil sendiri | Edit **nama**, **dokumen** & **identitas** (NIK/TTL/JK/alamat/kontak) di **detail**; tidak edit akun login orang lain | Edit nama/dokumen/identitas (inline + detail); lihat ranting & anggota di bawahnya | Lihat cabang, ranting & anggota di bawahnya |
| Kyu / DAN | Tidak edit sendiri | Tidak edit | **Edit Kyu (UKT & anggota)** | Tidak edit (hanya lihat) |
| Event (UKT, Gashuku, pertandingan) | Lihat & **daftar UKT/Latber mandiri** (+ konfirmasi bayar); daftar event non-UKT | Daftarkan anggota ranting; **Terima/Konfirmasi/Tolak** mandiri; Bayar UKT/Latber; **Batal UKT** hanya sebelum **Menunggu Ujian** | **Buat event** + periode Latber; lihat pendaftar; Verifikasi bayar UKT/Latber; **Hapus UKT** termasuk lunas (Menunggu Ujian+) | Lihat event & pendaftar; **edit NIA** di UKT |
| NIA | Lihat sendiri | Tidak assign | **Assign NIA** | **Assign NIA** |
...
**Administrator pusat**, **cabang**, dan **ranting**: dapat mengedit **identitas lengkap** (NIK, tempat/tgl lahir, JK, alamat) dan **kontak** (email login, telepon) anggota di sheet detail Kelola Anggota (`set_identity` / `set_contact`) sesuai cakupan wilayahnya; deteksi duplikat NIK/TTL sama seperti create.
...
| `Member` | NIA, NIK, nama, sabuk, status, dokumen, foto kanonis (`photoUrl`), dojo |
...
| `AppSetting` | Setting aplikasi (mis. CASHBACK ranting / `komisiRanting`) |
| `AuditLog` | Log audit |
| `SuratEntry` | Surat Masuk & Keluar, auto-numbering, QR Code verifikasi, scope, delivery anggota |
| `ArsipDokumen` | Kearsipan berkas digital, SK pengurus, AD/ART, LPJ, tanggal kadaluarsa |
| `NotulenRapat` | Risalah rapat pengurus, pimpinan, peserta, agenda, keputusan, action items |
...
### 9.1 Keanggotaan
1. Calon anggota daftar via `/login?tab=daftar` — urutan **Dojo → Identitas lengkap wajib** (nama, JK, tempat/tgl lahir, alamat, **NIK 16 digit**, telepon; **NIA tetap opsional**) → **Sabuk** (**No. MSH opsional** hanya Hitam/DAN) → **Akun**. **Cek duplikat email** (debounce UI + `POST /api/auth/register/check` + gate `409` di register) sebelum simpan. Setelah sukses daftar mandiri: **buka WhatsApp ranting** (nomor dari `GET /api/dojos` `phoneNumber`, fallback kontak cabang) dengan pesan verifikasi terisi. **Daftar mandiri** persist identitas lengkap ke Prisma lokal (`persistRegisterLocal` selaras `createAdminMember`) agar sheet Identitas admin tidak kosong. **Tambah Anggota** oleh ranting/cabang: NIK/NIA boleh kosong; **No. MSH opsional** (khusus Hitam/DAN, section Sabuk) disimpan ke Prisma saat create / daftar mandiri; **`currentRank` disinkronkan ke Prisma** saat create (`createAdminMember`, register mandiri, bulk) — tabel admin baca Prisma, bukan default Putih. Bila JWT Inkai expired saat create: retry `INKAI_SERVICE_TOKEN` bila ada, else **buat Member Active di Prisma** (portal tetap bisa pakai). **Input Massal**: tabel NIA, Nama, **Tempat & Tgl Lahir** digabung, JK (teks), Alamat, Kyu, Ranting + **paste 6 kolom tanpa NIA** (Nama…Ranting) didukung via `member-bulk-paste`; **isi semua ranting** & **isi semua Kyu/DAN**; paste Excel/CSV (format lama dengan NIK/Telepon tetap didukung); simpan per chunk dengan **progress %**; maks 50 (`POST /api/admin/members/bulk-create`). Field teks identitas **huruf besar**; **tanggal lahir** bisa paste (mis. `28 Februari 2011` / `Surabaya, 28 Maret 2015`).
...
4. **Deteksi duplikat** sebelum simpan: **keras** jika NIK, NIA, atau nama tepat + tanggal lahir sama (cakupan Cabang Surabaya); **lunak** jika nama mirip; **email** sudah punya akun (`User`). Blok `POST /api/admin/members` & `POST /api/auth/register` (409); UI peringatan di form tambah anggota & daftar publik. **`set_identity` (pusat)** memakai aturan duplikat yang sama + lepas NIK arsip bila hanya bentrok nomor.
...
7. Cabang **dan Pengprov** dapat mengisi atau mengubah **NIA** anggota dari kolom NIA inline maupun sheet detail di `/admin/anggota` (Pengprov juga inline di tabel UKT); **cabang** tetap **mengedit sabuk** dan **memindahkan ranting** anggota (kolom Dojo inline di `/admin/anggota`, `set_dojo`). Saat **set NIA**, password default diset = NIA **hanya** jika akun belum login / masih password default NIA lama (tidak menimpa password kuat). Ranting & cabang dapat **mengedit nama** (`set_name`), **dokumen** Akte/BPJS (`set_documents`), **foto profil** (`set_photo`, klik avatar → modal unggah drop/paste/pilih file + crop 1:1, kompres ~100 KB; simpan `Member.photoUrl` + mirror `User.photoUrl` bila ada akun), dan **No. MSH** untuk sabuk Hitam/DAN (`set_msh`); **ranting hanya lewat sheet detail** (bukan inline tabel); **cabang** tetap boleh inline Nama/Dokumen/NIA; **Pengprov** hanya assign NIA (tidak edit Kyu). Create admin & **Input Massal** yang membawa NIA kini menyinkronkan `member.nia` ke Prisma lokal sebelum auto-provision login. Perubahan MSH (mandiri, pengajuan, atau admin) **memberitahu admin ranting & cabang**. Ajuan pindah dari anggota tetap lewat verifikasi `DOJO_TRANSFER`.
...
12. **Login anggota:** identifier email atau NIA (varian titik dinormalisasi ke NIA tersimpan); rate limit per-IP + per-identifier di NextAuth authorize; email tanpa NIA tetap bisa login. **Auto-provision:** anggota ber-NIA tanpa `User` otomatis dibuatkan akun (password default = NIA) saat create / `set_nia` / bulk-create; backfill `scripts/backfill-nia-login-accounts.ts`; retry admin `POST /api/admin/members/provision-login`. Password kuat tidak ditimpa. Banner profil/dashboard jika password masih = NIA.
13. **Audit identitas di Kelola Anggota:** KPI/filter `account=missing` (ber-NIA tanpa akun) dan `dup=nia_nik` (bentrok NIA/NIK setelah normalisasi, termasuk vs arsip); tidak auto-merge.
14. **Koreksi identitas:** admin ranting (anggota rantingnya), pengurus cabang, dan administrator nasional dapat mengubah NIK, tempat/tgl lahir, JK, alamat (`set_identity`) serta email login & telepon (`set_contact`) dari sheet detail `/admin/anggota`; email hanya jika sudah ada akun User; telepon tanpa akun tetap tersimpan via Inkai.
### 9.2 Iuran
...
5. **Ranting** — aksi baris: **Daftar UKT**, **Terima**/**Tolak** (mandiri), **Batal UKT** (hanya **Belum Bayar** & **Menunggu Verifikasi** — setelah cabang verifikasi / **Menunggu Ujian+** hanya cabang), **Bayar UKT** (= ajukan ke cabang → **Menunggu Verifikasi**; untuk jalur daftar ranting). Setelah daftar ranting: **Belum Bayar**. **Bayar UKT** / **Verifikasi** tetap tampil bila terdaftar APPROVED meski `billingId` belum tertaut (ensure billing saat klik). **Hapus tagihan** (cabang, hanya bila tagihan **lunas** / **Menunggu Ujian+**; tidak di Belum Bayar / Menunggu Verifikasi): pendaftaran tetap → tagihan PENDING baru, status **Belum Bayar**, ranting **Bayar UKT** lagi (salah hapus/refund); **Hapus** pendaftaran cabang = hapus peserta + tagihan (keluar daftar). Notifikasi otomatis daftar/batal/Bayar/Terima.
5b. **Matriks aksi UKT per wilayah:**

| Aksi UKT | Ranting | Cabang | Pengprov |
|---|---|---|---|
| Buat/arsip periode | Tidak | Ya | Tidak |
| Daftar / Terima–Tolak / Bayar | Ya | Ya | Tidak |
| Verifikasi → Menunggu Ujian | Tidak | Ya | Tidak |
| Kyu Baru / hasil ujian | Tidak | Ya | Tidak |
| Batal sebelum lunas | Ya | Ya | Tidak |
| Batal/Hapus Menunggu Ujian+ | Tidak | Ya | Tidak |
| Edit NIA di UKT | Tidak | Ya | Ya |

6. **Cabang** **memverifikasi pembayaran** → **Menunggu Ujian**, lalu hasil ujian `LULUS` / `GAGAL` / `MENGULANG`.
7. Alur cabang setelah bayar: **Verifikasi** → **Menunggu Ujian** → isi **Kyu Baru** (tersedia juga modal **Scan QR / Barcode — Express Kyu Baru** `UktQrKyuScannerModal` berlayar luas & responsif `sm:max-w-3xl md:max-w-4xl lg:max-w-5xl` untuk kemudahan scan barcode/pencarian cepat di semua perangkat) → otomatis **Lulus** + **Selesai**. Kolom **Kyu Baru** hanya tampil jika `examResult === LULUS` (bukan dari kategori tagihan, bukan sisi kanan snapshot `|| Putih` palsu). Snapshot tanpa pemisah = Kyu Lama. Apply Kyu Baru menulis Prisma `Member.currentRank` + `EventRegistration.registeredRank` (`lama || baru`) + Inkai (selaras `set_rank`).
...
9. Status **Selesai** bila sudah lunas + lulus + sabuk target terisi; sabuk resmi anggota diperbarui + riwayat. Dual snapshot mengunci **Kyu Lama** di tabel UKT meski `currentRank` sudah naik; koreksi sabuk di `/admin/anggota` (`set_rank`) tidak menulis snapshot UKT.
10. **Cetak Nota** (`isUktNotaRow`): semua peserta **terdaftar** yang punya tagihan — termasuk **Belum Bayar**, Menunggu Verifikasi, dan lunas; **tanpa kode unik** (+1…999). Subtotal A = **tarif snapshot sabuk** (`buildNotaBeltLines` → `fallbackFees[belt]` dari `resolveNotaBeltGroupFromKyu`: Kyu Lama, fallback Kyu Baru; satu sabuk = satu baris meski `billingAmount` tabel beda; `LAINNYA` tetap dari billing); preview menampilkan ringkasan *Termasuk X Belum Bayar*. Nomor nota memuat semester (`UKT/SBY/{RANTING}/I|II/{tahun}`); modal **daftar centang ranting** (satu / beberapa / semua via Pilih semua) — bila >1 tercentang: `RANTING : GABUNGAN ({nama…})` + nomor `UKT/SBY/GABUNGAN/{smt}/{tahun}` + agregat peserta/sabuk/komisi; filter ranting cocok `dojoId` dengan fallback nama. Pejabat (Bidang Ujian / Bendahara) dari **period-meta per periode**, fallback **Pengaturan → Kebijakan**. Tab Peserta tetap semua terdaftar. Modal **tetap terbuka** setelah Print (footer Tutup+Cetak; guard `printBusy` saat dialog print browser — selaras Latber).
10a. **Laporan WA ranting:** daftar peserta terdaftar (`isUktWaRosterRow`) diurut **Kyu Lama** (Kyu 10→1, lalu Dan 1→10, lalu nama) + **rincian sabuk/A/B/C/TOTAL** dari **seluruh roster** (tarif snapshot via `buildNotaBeltLines` + `resolveNotaBeltGroupFromKyu`: **Kyu Lama lalu Kyu Baru** bila lama kosong — selaras baris peserta; termasuk tanpa tagihan — bisa beda jumlah sabuk vs Cetak Nota yang masih `isUktNotaRow`) + **C = jumlah peserta daftar WA × CASHBACK** (`komisiRanting`) + `_Termasuk N Belum Bayar_` = count `!isUktBillingPaid` di roster + pecahan Sudah/Belum lunas (`uktWaNetOfNotaRows` tanpa re-filter nota) + **rekening Bendahara Cabang** (`resolveUktWaBendaharaPayment`: profil `org.branch.profile` bila nomor terisi, else fallback Mandiri `LATBER_PAYMENT`; tanpa instruksi Latber Rp45rb; jarak enter antar blok rincian). WA cabang: **ringkas** — baris pertama tautan roster publik (`SITE_URL/ukt`); `*TOTAL SEMUA*` + pecahan lunas **paling atas**, lalu Pelaksanaan + *Jumlah UKT* + pecahan rupiah + tempat/lokasi + *N Ranting* (**semua ranting cabang**, termasuk `_0 peserta_` tanpa sufiks Lunas) + jumlah kyu; **tanpa** rekening; atau **rinci per ranting** (sama + rekening; ranting 0 → tautan lalu `*Ranting/Dojo*` lalu `*TOTAL SEMUA: 0*` lalu teks singkat `_0 peserta_`); **picker cabang multi-centang** ranting (Pilih semua / Hapus semua — selaras Cetak Nota): semua tercentang = ringkas (`buildUktCabangWaReportText`); satu = rinci; beberapa = gabungan rinci per ranting (`buildUktSelectedDojosWaReportText`, urut alfabetis, dipisah baris kosong); picker dialog tetap buka meski belum ada peserta; tombol **Salin laporan** (clipboard); rekap sabuk **Kyu Lama**.
10b. **Biaya sabuk & CASHBACK** di-**snapshot** ke period-meta saat buat/simpan periode; UI Atur Biaya default menyimpan ke snapshot periode (`updateGlobal: false`). Template global (`RankFeeTemplate` + setting `komisiRanting`) hanya diubah bila tanpa periode atau opsi “juga update global” dicentang.
10b2. **Hub Laporan/Rekapan UKT** (tombol **Laporan/Rekapan** di toolbar, Pendaftaran + Arsip): modal ber-tab `UktReportsHubModal` — **Peserta** (export daftar: ranting/CSV/Print; floating Export multi-select juga membuka tab ini), **Hasil Ujian** (rekap Pengda Excel/Print + pejabat/penguji/TTD), **Administrasi** (Perincian setor Pengprov — **cabang saja**; ranting tidak melihat tab ini). Default tab **Hasil Ujian** jika sudah ada Kyu Baru, selain itu **Peserta**. Tombol **Unduh PDF** di hub & Cetak Nota dihapus (Save as PDF lewat dialog Print browser). Seluruh dokumen/laporan cetak UKT (Peserta Print HTML/CSV/PDF, Hasil Ujian Print HTML/Excel, Public Roster Print) diurutkan secara hirarkis menurut **Ranting (A–Z)** → **Sabuk/Kyu 10–1** (Kyu 10→1, Dan 1→10) → **Nama (A–Z)**, dengan alignment sel cetak peserta `vertical-align: middle` untuk semua sel dan `text-align: center` khusus kolom **No, NIA, No. R (nomor urut per ranting), Kyu/Kyu Baru, dan Jenis Kelamin**.
10b2b. **Tab Administrasi** (cabang): Bagian I = peserta **lunas** × tarif **Pengprov** (`pengprovBeltFees` di period-meta, default 150/150/150/155/195 — **terpisah** dari `beltFees` Nota); abaikan filter ranting UI (agregat semua ranting); Bagian II buku + Biaya Penguji manual; Print.
10b3. **Tab Hasil Ujian** (cabang **dan** ranting scoped): Excel 2 sheet format Pengda (`SMT. I/II` + `LEMBAR TTD`) via `POST /api/admin/ukt/rekap-hasil`; Print HTML A4 landscape 2 halaman (kop logo INKAI kiri); hanya peserta yang sudah punya **Kyu Baru**; Kyu Lama dari snapshot daftar; urut ranting + Kyu 10→1; pejabat TTD + daftar **penguji** (bukan pelatih) dari period-meta/template/org; CATATAN sabuk `SABUK X = N` rapat; nama penguji di kolom kanan CATATAN; gambar TTD opsional dari URL upload `ukt-ttd`.
10b4. **Pejabat & TTD**: period-meta (nama + title DAN/MSH + `memberId` pejabat/penguji + `*SignUrl`); autocomplete `GET /api/admin/ukt/ttd-suggest`; refresh pangkat live `POST /api/admin/ukt/ttd-titles` saat simpan/cetak (jika ada memberId); pad TTD **full viewport** (muat TTD lama, tolak kosong, coba landscape); template `ukt.ttd-template`.
...
11b. **Rekonsiliasi setoran** (kartu di UI **cabang**): tabel Ranting / Peserta / Lunas / Total / Status setor via `buildUktDepositReconciliation` — **semua ranting** di daftar cabang (termasuk **0 peserta**: Total Rp 0, Status **—**, Keterangan `0 peserta`, tanpa Terima/Reset); ranting berpeserta: kolom **Keterangan** = `Belum Bayar: n, Menunggu Ujian: m`; **Total tagihan** net (Subtotal A kotor − CASHBACK × lunas) selaras TOTAL NOTA; status setoran (diterima/reset) **hanya diubah cabang** — `PATCH /api/admin/ukt/deposit` menolak non-cabang (`canEditKyuBaru`) fail-closed; ranting tidak menandai setor dari halaman UKT. **Tulis Prisma-first** (`AppSetting` key `ukt-deposit:{eventId}:{dojoId}`) + best-effort Inkai PUT; baca deposit merge Prisma; UI update `depositMap` lokal dari response.
...
11d. **Fokus periode aktif:** resolusi mengutamakan non-arsip; judul kanonis `UKT Semester {I|II}-{tahun}`; buat periode baru mengarsipkan term yang sudah tutup; sidebar **UKT → Pendaftaran / Arsip UKT** (bukan dropdown campuran); anggota hanya melihat periode aktif. Arsipkan dari Pendaftaran mengarahkan ke Arsip; buka arsip mengembalikan ke Pendaftaran. **Pendaftaran & Arsip UKT** = **registrants-first** (tanpa pool 250/500 anggota): `allRows` = peserta terdaftar; **Belum Daftar** via cari nama → `GET /api/admin/ukt/suggest?uktEventId=` + hydrate `GET /api/admin/ukt/members?memberId=&periodId=` (stub lalu **Daftar UKT** di dropdown pencarian, **kolom Aksi**, atau baris tabel HP; tombol **hilang** jika sudah registrasi aktif); CANCELLED/REJECTED tetap boleh daftar ulang. Tolak/batal (`CANCELLED`/`REJECTED`) tidak di-ghost sebagai Belum Bayar.
11e. **Kunci periode di API** (bukan cuma UI): `assertUktPeriodMutable` (`ukt-period-meta-store.ts`) dipanggil di `register`, `registrations/[id]` (PATCH/DELETE — `eventId` **selalu dari DB/Inkai**, hint klien hanya untuk lookup `memberId+eventId` + deteksi mismatch `SECURITY_UKT_EVENT_ID_MISMATCH`; unresolved → tolak), `deposit`, `fees` (bila `eventId`), `waiver`, `exam-day`, `validateUktRegistrationEligibility`, dan **billing UKT** (`PATCH /api/admin/billing/[id]` via `registrationId` → event) — periode arsip/terkunci menolak mutasi di server. `loadUktPeriodMeta` fallback Prisma `AppSetting` `ukt-period-meta:{eventId}` bila GET Inkai gagal (jangan anggap periode terbuka palsu saat JWT expired). `period-meta` PATCH (toggle arsip/kunci itu sendiri) sengaja **tidak** digate agar cabang tetap bisa membuka kunci. **DELETE** resolve `eventId` Prisma-first (by id, lalu `memberId`+hint `eventId`, lalu Inkai GET); bila Inkai detail 404, dojo/nama diisi dari Prisma (gate ranting tetap fail-closed). Setelah Inkai DELETE ok/**404**, `forceDeleteRegistrationInDb` untuk id URL **dan** id Prisma hasil lookup (baris fallback `forceRegisterUktInDb` / daftar mandiri tidak tertinggal). UI hapus mengirim `eventId` periode aktif sebagai hint saja.
11f. **Refresh tabel UKT read-only:** `fetchUktTableRefreshSnapshot` (dipakai `GET /api/admin/ukt/table`) menggabungkan snapshot Inkai + **selalu** fallback Prisma registrasi (cabang & ranting) — tidak mengosongkan tabel saat Inkai detail gagal; UI menolak apply snapshot kosong jika baris terdaftar sudah ada; tanpa side-effect write saat refresh.
11g. **Hardening `registrations/[id]` (P0):** DELETE tidak lagi mempercayai `billingId` dari query klien (anti-IDOR) — hanya tagihan yang terverifikasi tertaut `registrationId` yang dihapus; mismatch dicatat `SECURITY_UKT_BILLING_ID_MISMATCH`. Scope ranting (`ADMIN_DOJO`) di `accept/reject_self_registration`, `submit_for_verification`, DELETE, dan **`register`** kini **fail-closed**: allowlist kosong atau dojo peserta tidak diketahui/tidak cocok → 403. **Force-batal tagihan lunas (`canForcePaid`) cabang-only** — ranting DELETE pada **Menunggu Ujian+** → 403. `mark_paid` menolak daftar mandiri PENDING (wajib **Terima** ranting dulu); UI cabang Verifikasi memakai `canCabangVerifyUktPayment`. `deposit` PATCH kini **cabang-only** (`canEditKyuBaru`); jalur ranting `SUBMITTED` dihapus. Rate limit (`rateLimitAsync`/`rateLimitResponse`, ~20–30/60dtk per user) dipasang di `register`, `registrations/[id]` PATCH & DELETE, `deposit`, `fees`, `waiver`, `exam-day`, `period-meta`.
11h. **Load Pendaftaran UKT:** satu fetch berat `fetchUktDashboardData` (tanpa light-resolve redirect ganda); URL sync hanya via `resolveUktAdminCanonicalRedirect` (tidak strip `period` saat API gagal / canonical null); filter event `isUktAdminEventTitle` (exclude Latber); `fetchUktEventsCached` cache key `ukt-events-list-v5` (revalidate 600s, **tidak cache list kosong**); **selalu merge Prisma `Event`** (+ meta `AppSetting`) agar periode tidak hilang setelah blip API; **registrants-first** termasuk seed Prisma regs; **slim-when-empty**; fees snapshot; dojos Prisma scoped; Inkai `retries:0`/8s; `dojoGroups` lazy; dashboard SSR lewat client re-export; refresh snapshot tahan kosong; ranting allowlist kosong = fail-closed.
12. Dashboard anggota: kartu **Status UKT** (daftar mandiri + konfirmasi bayar + status; **tanpa nominal**); admin cabang: export, Laporan WA, hari-H, setoran, arsip, waiver, wizard. **Ranting** membatalkan pendaftaran UKT + tagihan hanya sebelum **Menunggu Ujian**; **cabang** dapat **Hapus** termasuk lunas (refund luar biasa).
13. Toolbar cabang: **Buat Periode**, Hari-H, **Laporan/Rekapan** (hub Peserta/Hasil/Administrasi), Laporan WA (**salin clipboard**, dialog pilih ranting), **Salin Undangan** / **WA Undangan**, Cetak Nota (Print saja), Biaya Sabuk, Arsip (tombol terpisah). Toolbar ranting: **Laporan/Rekapan** (Peserta+Hasil scoped) + **Laporan WA** (salin) + **Salin/WA Undangan** + **Cetak Nota** (manual). Sama di **Arsip UKT**. Roster publik: `/ukt`.
...
14b. **Roster publik** `/ukt`: tanpa login; periode non-arsip; **timer batas pendaftaran** (payload `registrationCloseAt`); KPI + cari filter (nama/NIA/ranting) + ringkasan HP; **kolom Tanggal daftar**; **sort kolom** + **centang baris** + **Cetak daftar** (modal pilih ranting, kertas A4/F4, modal tetap terbuka saat Print; HTML `ukt-roster-print-html.ts`); poll senyap ~30s (sama `/latber`); `GET /api/public/ukt/registrants`; `robots: noindex`.

### 9.3b Latihan Bersama
1. **Cabang** membuat / **edit** periode Latihan Bersama aktif (nama + buka/tutup + jadwal/lokasi); event kanonis `Latihan Bersama — {nama}` (judul lama `Latber —` tetap dikenali), kategori **Pendaftaran Latihan Bersama**; snapshot `latber-period-meta:{eventId}` (fee **45.000** dikunci, CASHBACK ranting **5.000** / `komisiRanting`); buat periode baru **mengarsipkan** periode lama non-arsip; PATCH konten diblok saat arsip/kunci.
2. URL admin: **Pendaftaran** `/admin/latber?period=<eventId>` dan **Arsip Latihan Bersama** `/admin/latber/arsip?...`; cabang **Arsipkan** manual atau **Buka kembali** dari arsip; `/admin/latber/arsip` memuat periode terarsip/tutup dengan fallback ke periode tersedia sehingga data peserta & nota/laporan selalu dapat diakses di kedua halaman. Kartu kosong (`!selectedPeriodId`) menampilkan navigasi timbal-balik kontekstual. **Load periode:** Inkai `/v1/events` + **fallback Prisma `Event`** (selaras `/latber` publik) bila API gagal/kosong; meta missing dari `AppSetting` `latber-period-meta:`; peserta tetap registrants-first Prisma.
...
4. **Terima mandiri** (belum lapor bayar) → tagihan **`PENDING` (Belum Bayar)**; ranting **Bayar** → `WAITING_VERIFICATION`. **Konfirmasi** (anggota sudah **Konfirmasi sudah bayar** → status **Menunggu Konfirmasi Ranting**) → langsung `WAITING_VERIFICATION`; cabang **Verifikasi** → **Lunas** (`mark_paid` idempotent bila sudah PAID); ranting/cabang **Tunai** → **Tunai** (`mark_cash` + Payment `CASH` + kas + kredit absensi, tanpa antri verifikasi); UI admin **optimistic `setRows` + sync props**.
5. **Ranting** — Terima/Konfirmasi/Tolak mandiri, **Daftar** / **Tambah Peserta**, **Bayar**, **Tunai**, **Tambah keanggotaan** (promote), **Batal**; **Cabang** — Verifikasi lunas, Tunai, Hapus (termasuk arsip); mutasi diblok `periodLocked`; **DELETE** tidak gated arsip + `voidKasFromBilling` + hapus kredit LATBER.
6. Nota/rekap/WA ranting: peserta **`filterLatberApprovedRows`** (`isLatberNotaRow` — Belum Bayar + Menunggu Verifikasi + Lunas/Tunai; bukan mandiri PENDING/ditolak); `grandTotal = (n × 45.000) − (n × 5.000)` dari **semua baris nota**; **Cetak Nota** kolom No·NIA·Nama·Sabuk·**Status**·**Hadir ☐**·Biaya + *Termasuk N Belum Bayar*; **Laporan WA** salin clipboard; Rekap/Undangan.
7. **Link undangan:** Salin/WA Undangan memakai URL pendek **`/latber`** (periode aktif) atau `/latber?period=…` (arsip); `/undangan/latber/[periodId]` tetap redirect (kompatibilitas tautan lama); teks WA menyertakan jadwal/lokasi/biaya; snapshot `latber-invite:{periodId}` / sync lama boleh tetap ada.
...
10. **Kelola Anggota** — dropdown pencarian + **kolom Aksi**: **Daftar UKT** / **Daftar Latber** ke periode aktif (`active-registration-periods`); tombol hanya jika belum registrasi aktif (tanpa teks Sudah UKT/Latber); ACTIVE/PENDING saja.
11. **Kredit kehadiran:** peserta **Lunas atau Tunai** mendapat 1 hari absensi pada tanggal jadwal (`eventAt`, `method=LATBER`); ditulis saat Verifikasi/`mark_cash` (hari H lewat) + cron `GET /api/cron/latber-attendance`; batal/hapus menghapus kredit LATBER.
12. **Walk-in publik `/latber`:** tanpa login; timer; KPI (Peserta/Belum Bayar/Menunggu Verifikasi/Lunas+Tunai) + ranting + grafik; cari/scan; **Tambah Peserta** (`POST /api/public/latber/add-guest`, rate ketat) + **Tambah Anggota** (register penuh; bentrok nama tamu → 409 `LATBER_GUEST_EXISTS`); **Cetak** daftar peserta (pilih ranting, kolom ☐ sebelum No, kertas A4/F4, portrait/landscape, wrap tanpa potong); Bayar QRIS+Mandiri → Verifikasi admin → Lunas; badge Tunai mirror; **tanpa** Tunai/promote di publik; Batal WA; `/admin/latber` tanpa sesi → `/latber`.
...
1. **Aturan Distribusi Setoran UKT vs Latber & Status Lunas**:
   - **Pemisahan Verifikasi Pembayaran & Buku Kas**: Verifikasi pembayaran peserta pada modul UKT (`/admin/ukt`) dan Latihan Bersama (`/admin/latber`) **tidak lagi otomatis memposting/mengirimkan transaksi ke Buku Kas** (`/admin/kas`), sehingga pencatatan kas tetap terpisah dan terkontrol penuh oleh Bendahara/Admin Cabang.
   ...
   - **Struktur Kolom Modal (7 Kolom)**: `No`, `Ranting / Dojo` (dengan badge kategori `[UKT + Latber]` / `[Latber]`), `UKT`, `CASHBACK UKT`, `Latber`, `CASHBACK Latber`, `Total Masuk`.
   - **Transparansi Keuangan (Gross vs Net)**: Ringkasan KPI memperlihatkan Total Setoran Kotor UKT & Latber, Total CASHBACK Ranting, serta **Setoran Masuk Net Cabang** (Setoran Kotor − Total CASHBACK Ranting) untuk memastikan akurasi saldo kas yang diterima cabang.
...
│ │ (4 UKT, 11 Latber)   │ │ (CASHBACK: Rp 491.000) │ │ (CASHBACK: Rp 450.000) │ │ (Setoran Kotor − CASHBACK Ranting) │ │
│ └──────────────────────┘ └──────────────────────┘ └──────────────────────┘ └──────────────────────────────────┘ │
│ Periode: Sabtu, 1 Agustus 2026 – Selasa, 25 Agustus 2026 · Status: Lunas            [ 🔍 Cari ranting...      ] │
│                                                                             * Klik baris untuk memfilter di kas │
├────┬─────────────────────────────────┬───────────────┬──────────────┬───────────────┬───────────────┬───────────┤
│ No │ Ranting / Dojo                  │   UKT (Lunas) │ CASHBACK UKT │Latber (Lunas) │ CASHBACK Latber │ Total Masuk│
├────┼─────────────────────────────────┼───────────────┼──────────────┼───────────────┼───────────────┼───────────┤
...
3. **Penjelasan Perbedaan Wireframe Mockup (.md) vs Data Aplikasi Nyata (`/admin/kas`)**:
   - **Wireframe (.md)**: Merupakan *mockup ilustratif / standar rancangan visual UI* yang digunakan untuk menentukan tata letak 7 kolom, posisi KPI card, tombol pencarian ranting, badge jenis setoran, serta simulasi pengisian CASHBACK jika terdapat 15+ ranting.
   - **Modal Aplikasi Nyata (`/admin/kas`)**: Menampilkan data secara **dinamis dan real-time dari database kas cabang** sesuai rentang tanggal periode yang dipilih pengguna. Sebagai contoh, pada transaksi real periode 1–25 Agustus 2026:
     ...
     - Angka CASHBACK pada kas bernilai `Rp 0` (atau `-`) apabila catatan kas yang diinput merupakan transaksi net/bersih.
...
- Admin `/admin/absensi`: filter Cabang & Ranting dojo; tabel Progress (klik → detail riwayat & koreksi), Harian, Belum hadir; modal Koreksi Absensi (edit ranting/dojo lokasi check-in, edit tanggal/waktu check-in, serta hapus absensi barcode/manual via `PATCH`/`DELETE /api/admin/absensi`); export CSV harian/belum/progress; geofencing di Pengaturan.
...
### 9.5 Sekretariat & Generator Surat
- **Generator Surat** (`/admin/sekretaris/generator`): Generator surat resmi organisasi dengan WYSIWYG Visual Editor & Style Toolbar (B/I/U/S, penataan teks, warna font/highlight, font family/size, list, hapus format) dilengkapi deteksi status tombol aktif (merah) sesuai kursor; cetak A4/F4 presisi.
- **Struktur Grid Layout Presisi & Interactive Table Panitia / Personel**: Memperbaiki hierarki penutupan elemen HTML `div` container editor utama (`lg:col-span-6`) sehingga seksi **Interactive Table Panitia / Personel** dan **Pejabat Tanda Tangan Digital & Stempel** tetap berada rapi di dalam card form parameter (bukan melimpah ke luar grid track `grid-cols-12`), memastikan input nama/jabatan personel dan canvas TTD tampil luas, presisi, serta tidak berantakan/tumpang tindih di semua ukuran layar.
- **Nomor Surat Direct Edit**: Hasil generasi nomor surat otomatis (misal `001/INKAI-KOTA.SBY/ST/IX/2026`) langsung ditempatkan di dalam input field **Nomor Surat** sebagai nilainya sehingga pengguna dapat mengedit atau menyesuaikan format nomor secara langsung di dalam kotak input, disertai tombol **`Auto Gen`** untuk meregenerasi nomor urut otomatis kapan saja.
- **Canvas TTD Digital Interactive Extra Luas** (`SignatureCanvasModal`): Modul tanda tangan digital interaktif untuk **Ketua** dan **Sekretaris**. Dialog modal diperlebar (`max-w-3xl sm:max-w-4xl lg:max-w-5xl`) dengan area pad gambar ekstra luas (`h-64 sm:h-80 md:h-[340px]`) untuk memberikan kenyamanan leluasa saat menggoreskan tanda tangan (mouse, touchscreen, stylus tablet). Mendukung resolusi tinggi 2×, pilihan warna tinta resmi (Biru Tua/Hitam/Biru Basah/Merah), tebal pen (2px–6px), unggah file PNG, reset canvas, dan **penyimpanan permanen `localStorage`** (`INKAI_TTD_KETUA` & `INKAI_TTD_SEKRETARIS`). Gambar TTD tersimpan secara otomatis dan dapat dipakai seterusnya pada surat keluar, kwitansi, nota, resume rapat, dll.
- **Kop Surat & Stempel Resmi**: Menggunakan logo FORKI resmi (`LOGO_FORKI_BASE64`) & INKAI (`LOGO_INKAI_BASE64`) pada kop header; Stempel INKAI Kota Surabaya (`STEMPEL_INKAI_BASE64`) terpasang presisi di antara TTD Ketua dan Sekretaris.
- **Ranting Footer List**: Daftar Ranting Aktif Cabang Surabaya di bagian footer `surat-print-html.ts` menggunakan layout 7 kolom presisi CSS Grid (`display: grid; grid-template-columns: repeat(7, minmax(0, 1fr));`) sehingga seluruh nama ranting terurut sejajar secara vertikal dari nomor urut 1 sampai 28+ (misal 2. AL-IRSYAD, 9. FORTRESS, 16. MANYAR, 23. SIMOMULYO lurus tegak dalam 1 kolom).
- **Salin Format WA Dinamis & Kirim Direct**: Tombol **Salin WA** dan **Kirim ke WA** (`handleCopyWaText` & `handleOpenWaDirect` pada `SuratPrintModal.tsx`) mengonversi dokumen ke format WhatsApp Markdown (judul kategori dinamis, perihal, nomor, tanggal, tujuan, konversi HTML editor ke teks bersih WA `*bold*`/`_italic_`/`•`, serta tabel personel) sekaligus opsi 1-klik buka aplikasi WhatsApp Web/App (`https://api.whatsapp.com/send`) dan tombol **Unduh HTML** untuk backup dokumen.
- **Fitur Ahli & Produktivitas**: Tombol **`Tukar Posisi Pejabat`** (swap nama, jabatan, dan TTD Ketua ⇄ Sekretaris dalam 1-klik), **Toggle Stempel INKAI** (opsi menampilkan/menyembunyikan stempel resmi), **Penyimpanan Judul & Jabatan `localStorage`** (`INKAI_SURAT_KETUA_NAME`, dll.), serta chip **Preset Paragraf Standard** (`+ Pembuka`, `+ Penutup`) pada toolbar WYSIWYG editor.
...
| PostgREST / Data API | Prisma-only | App **tidak** pakai Supabase client di browser untuk CRUD; hardening SQL [`prisma/sql/postgrest-rls-deny-all.sql`](prisma/sql/postgrest-rls-deny-all.sql) — RLS + revoke `anon`/`authenticated`; wajib apply ke **DB produksi** (`DATABASE_URL` Vercel, ref `ztrryuhhdoqdglajukuw`); opsional matikan Data API di dashboard |
| Edge firewall (Vercel WAF) | Tidak aktif (Hobby) | Portal publik — **jangan** Trusted IPs produksi; DDoS platform Vercel sudah ada; custom WAF / Attack Mode butuh Pro — hanya saat insiden abuse |
...
| OG image publik | Kartu pratinjau tautan dinamis (`src/lib/og-image.tsx`, 1200×630, opsional `subtitle`) via konvensi `opengraph-image` di grup `(public)` (default + Beranda) & tiap tab (tutorial/sejarah/struktur/dojo/kegiatan/apresiasi/kontak/artikel/**latber**); artikel `[slug]` pakai foto. Root layout tak lagi memakai `/logo-inkai.png` sebagai `og:image` |
...
| Admin anggota / iuran / UKT | Lengkap | Iuran: **rekening koran** per anggota + Sheet pengaturan/mutasi/bayar + pengecualian event/UKT; anggota: pencarian filter tabel + klik baris detail; **sabuk tersimpan ke Prisma** saat create/`set_rank`; nonaktif/aktif/hapus arsip + **edit sabuk (cabang)**; **pusat edit identitas/kontak di sheet detail**; UKT pakai gate iuran+dokumen+absensi, hasil ujian, rekap ranting, nota = semua terdaftar + nominal tabel (`isUktNotaRow`); WA setor tetap filter pembayaran |
...
| Nav admin | Campuran | Top-level: Iuran; grup **Keuangan** (Kas + Kwitansi + Arsip); Event, Absensi; **UKT** + **Latihan Bersama** sebagai grup (Pendaftaran + Arsip); grup: Keanggotaan / Konten / Sistem + badge unread pesan |
| Kas | Aktif (jurnal lokal) | `/admin/kas` Prisma `KasEntry`/`KasPeriodLock`; mode **Laporan Detail** (default, flat + inline edit semua sourceType + badge sumber `manual`/`otomatis` dengan tautan audit + centang multi-select & hapus massal baris manual + Maximize full halaman; saat full: Refresh + periode + Cetak di header) / **Buku** (grup lipat; Ubah semua sourceType, Hapus manual); filter periode awal–akhir; filter **arah mutasi** (Semua/Masuk saja/Keluar saja); **tombol Reset filter**; **badge interaktif Belum rekon** pada KPI (1-klik filter belum rekon); Ubah/Hapus/Pindah lokasi (1 baris + centang batch + Pindah kegiatan + **modal konfirmasi hapus dilengkapi rincian/pratinjau daftar transaksi terpilih** + **tombol icon Salin WA transaksi terpilih di samping centang toolbar**); **Tombol Salin WA** (format terstruktur & dikategorikan: saldo awal, masuk, keluar, surplus, saldo akhir, rincian `• [MASUK]` & `• [KELUAR]`); **Tambah item** di header grup kegiatan; **Tambah massal multi-tanggal**; sticky laptop / ringkasan kolaps HP; auto dari verifikasi bayar; manual/massal/impor CSV; cetak kop INKAI; tutup buku; rekon; grant `/admin/kas`; bukan backfill Excel |
| Kwitansi Pembayaran | Wireframe | `/admin/kwitansi` + `/admin/kwitansi/arsip` (dummy); mode A centang; draft sessionStorage; cetak 1× helper bersama; watermark DRAFT; `Member.signatureUrl` (+ SQL `prisma/sql/member-signature.sql` produksi); PaymentReceipt **fase 2** |
...
| Nominal UKT & Latber | Flat (Tanpa kode unik) | Frontend & backend tidak menulis `uniqueTail`; `postKasFromUktPaid` & `postKasFromLatberPaid` memotong ekor unik ke ribuan (`amount - (amount % 1000)`); `allocateLatberUniqueTail` mengembalikan `uniqueTail: null` & flat fee; verifikasi tagihan Latber (`billing/[id]`) otomatis mencatat **CASHBACK Ranting Rp 5.000** ke Kas Ranting; label kegiatan Latber otomatis menyertakan nama Ranting (`Bayar Latber Persiapan UKT-[NAMA_RANTING]`); tombol Aksi Latber diubah menjadi **TF** & **Tunai** (proses konfirmasi langsung tanpa WA); script `backfill-kas-flat-fee.ts` menormalisasi data historis `Billing`, `KasEntry`, dan menyinkronkan CASHBACK Latber yang belum tercatat. |
...
| Roster publik UKT | Aktif | `/ukt` + `GET /api/public/ukt/registrants`; KPI/cari/ringkasan HP; poll ~30s; noindex; **periode tetap** jika load peserta gagal (`loadError`); cache hanya payload sukses; **kolom Tanggal daftar**; **sort kolom**; **centang baris**; **Cetak daftar** (modal ranting, modal tetap terbuka saat Print) |
| Refresh tabel UKT | Aktif | `GET /api/admin/ukt/table` snapshot + hydrate identitas Prisma (Tempat/TTL/JK/Alamat/Foto) + `registeredAt`; full halaman in-app Maximize (header Refresh/Laporan-Rekapan); **Kyu Lama+Baru Select Radix** cabang/administrator di tabel biasa & full |
...
| Modul Latber (MVP) | Aktif | Admin `/admin/latber` + arsip; suggest/hydrate; **Tambah Peserta**/promote; status **Tunai** (`mark_cash`); nota Status+Hadir; `periodLocked`; Rekap/WA; kredit absensi Lunas\|Tunai |
...
| Undangan portal Latber | Aktif (redirect + URL pendek) | Salin/WA → `/latber` (aktif) atau `/latber?period=…` (arsip); `/undangan/latber/[periodId]` → redirect; OG `/latber` |
| Latber walk-in publik | Aktif | `/latber` + add-guest/register/confirm; **Tambah Peserta** + **Cetak** roster pilih ranting; badge Tamu/Tunai; Tambah Anggota; Bayar QRIS+Mandiri; tanpa Tunai/promote publik |
...
| Setoran UKT | Aktif | Status setor (SUBMITTED/RECEIVED) **hanya diubah cabang** (`canEditKyuBaru`, fail-closed); jalur ranting dihapus; **rekonsiliasi** peserta/lunas/tagihan vs status setor; tulis **Prisma-first** (`ukt-deposit:…`) + best-effort Inkai |
...
| Tarif setor Pengprov UKT | Aktif | `pengprovBeltFees` di period-meta (default 150/150/150/155/195); Laporan UKT cabang; terpisah dari Nota |
| Jadwal ujian UKT | Aktif | `examAt` + `examLocation` di period-meta; wizard + kartu jadwal + kartu anggota |
| Pejabat UKT per periode | Aktif | Nota: `bidangUjianName` / `bendaharaCabangName` di meta; Hasil Ujian: pejabat Pengda/MSH/Ketua Cabang/Koordinator + `pengujiNames` + URL TTD; template `ukt.ttd-template`; suggest akun Hitam/DAN |
...
| Sync cloud ↔ lokal | Fase 1 CLI | One-way **cloud → lokal** saja: `npm run db:local:dump-from-prod` (`scripts/dump-prod-to-local.ts`); `PROD_DATABASE_URL` read-only + `assert-local-database` :5433; subset + sanitize PII; password lokal `inkai-local`. **Bukan** sync dua arah; **bukan** `/api/admin/sync/*`; login tetap lewat inkai-backend |
| Cadangan produksi (ops) | Runbook | (1) Supabase Dashboard → project produksi portal **`ztrryuhhdoqdglajukuw`** (bukan salinan `inkai-db`/`mzmdh…`) → Database → Backups / PITR sesuai plan; (2) opsional `pg_dump` berkala ke storage terenkripsi di luar repo; (3) restore drill ke project/staging terpisah — verifikasi `count(Member)` + login backend; **jangan** restore langsung menimpa produksi; (4) `db:local:dump-from-prod` hanya untuk debug lokal, **bukan** pengganti backup |
...
| Drift skema Prisma | Proses | Setiap ubah `prisma/schema.prisma` wajib `prisma/sql/*.sql` (ADD IF NOT EXISTS) + apply ke **DB yang sama dengan `DATABASE_URL` app (Vercel)** sebelum/bersamaan deploy field baru — **produksi portal = Supabase ref `ztrryuhhdoqdglajukuw`** (bukan `inkai-db`/`mzmdhkwleufeiyaspmns` yang adalah salinan lama); `db:check-drift` vs DB app + job CI opsional `PROD_DATABASE_URL`; `memberPhotoSelect()` probe `information_schema` ter-cache (skip select bila kolom hilang); `withPrismaFallback` → `degraded:true` + toast UKT identitas; JWT refresh / impersonasi tidak select `Member.photoUrl`; roster `/ukt` satu query lewat probe; soft-update `update`+`select` + `prismaUserFacingError`; toast hard-logout admin **hanya** `SESSION_EXPIRED_MESSAGE` (bukan setiap Inkai 401) |
| Inkai JWT stale / mutasi admin | Diperkuat | `inkaiFetchWithServiceRetry`: JWT near-exp → service token dulu; auth fail → retry service; **jangan forward 401** (`portalStatusFromInkai` → 502); `shouldApplyInkaiPrismaFallback` → Prisma 200 untuk `set_nia`/`set_name`/`set_dojo`/`set_identity`/`set_contact`/`approve`/`reject` + bulk approve, verifikasi lokal, UKT quick-reg; `set_rank` / UKT `update_kyu` Prisma fallback; login soft-check `exp` + clear cookie stale; `INKAI_SERVICE_TOKEN` wajib produksi |
...
| Firewall / IP lock (ops) | Ditolak untuk produksi | **Jangan** Trusted IPs Vercel (`inkai-sby`, `inkai-ecosystem`) — anggota/publik/Latber walk-in butuh akses dari IP dinamis; **jangan** Supabase Network Restrictions ke IP rumah (Vercel Hobby tanpa egress statis); preview `*.vercel.app` sudah SSO (`all_except_custom_domains`) |
| Supabase PostgREST (ops) | Runbook | Advisor `rls_disabled_in_public` pada tabel sensitif jika Data API masih nyala; apply [`postgrest-rls-deny-all.sql`](prisma/sql/postgrest-rls-deny-all.sql) ke DB yang sama dengan `DATABASE_URL` app (**bukan** salinan `inkai-db`/`mzmdh…` kecuali dev); dashboard opsional: Settings → API → matikan Data API |
| Vercel WAF edge (ops) | Deferred (Hobby) | Tim `inkais-projects` plan **Hobby** — custom firewall rules / Attack Mode butuh **Pro**; rate limit aplikasi sudah di [`src/proxy.ts`](src/proxy.ts) + endpoint; aktifkan WAF edge hanya setelah upgrade + insiden (rate-limit `/api/auth/*`, `/v1/auth/login`) |
...
/api/admin/members/[id]     Detail + aksi (approve/NIA/set_name/set_msh/set_rank/set_dojo/set_dues/set_dues_exemption/dokumen/set_photo/set_identity/set_contact/reset_password/nonaktif/hapus/restore/merge); mutasi Inkai via `inkaiFetchWithServiceRetry` (bukan forward 401); Prisma fallback 200 bila JWT/auth/503/5xx untuk `set_nia`/`set_name`/`set_dojo`/`set_identity`/`set_contact`/`approve`/`reject` (lib `member-registration-actions`); overlay GET Prisma menang status/ranting/nama/NIA/kontak; `set_identity`/`set_contact` hanya pusat; `set_photo` selalu simpan `Member.photoUrl` dan mirror `User.photoUrl` bila ada akun; set_nia auto-provision jika belum berakun
/api/admin/members/bulk     Bulk nonaktif / approve (Prisma fallback registrasi) / hapus-arsip (ARSIPKAN) / purge arsip (HAPUS) / restore
/api/admin/members/archived Daftar arsip soft-delete
/api/admin/billing/[id]     Edit tagihan, **submit_for_verification** (ranting→Menunggu Verifikasi), verifikasi/tandai lunas, **hapus** (ranting/cabang; force lunas = cabang; fallback Prisma bila API gagal); **UKT:** setelah hapus, buat tagihan PENDING baru + reset registrasi lunas→APPROVED (status Belum Bayar)
...
/api/admin/kas                 GET buku kas scoped (`from`/`to`, `scopeType`/`scopeId` untuk cabang); POST Tambah/massal (sourceType manual/kwitansi)
/api/admin/kas/[id]            PATCH rekon/edit manual; DELETE baris manual
/api/admin/kas/[id]/transfer   POST pindah lokasi baris manual ke buku lain (cabang-only)
/api/admin/kas/transfer-kegiatan POST pindah semua baris manual per kegiatan ke buku lain (cabang-only)
/api/admin/kas/transfer-batch  POST pindah beberapa baris manual by ids (cabang-only, maks 100)
/api/admin/kas/delete-batch    POST hapus beberapa baris manual by ids (maks 100)
/api/admin/kas/lock            POST tutup/buka buku bulan (cabang)
/api/admin/kas/import          POST impor CSV/TSV maks 500 baris manual
/api/admin/kas/scopes          GET daftar buku kas cabang/ranting yang boleh dibuka user
/api/admin/kas/ukt-deposit     GET status setor UKT (`depositMap`) untuk rekap Kas: resolusi periode dari `from`/`to` (tengah rentang); cabang semua ranting, ranting scoped allowlist; read-only
/api/admin/kwitansi/member-suggest  GET saran anggota scoped wilayah (nama/NIA/MSH) + `signatureUrl` untuk kwitansi/nota
/api/admin/members/[id]/signature  GET/PATCH arsip TTD `Member.signatureUrl` (scoped)
/api/admin/absensi             GET payload absensi admin (harian/semester/progress) untuk client refetch filter
/api/admin/ukt/registrations/[id]  Update/hapus pendaftaran UKT (`submit_for_verification` / `mark_paid` / Kyu; cabang force hapus: API lalu fallback Prisma shared DB); kunci periode (`assertUktPeriodMutable`) di PATCH & DELETE; DELETE resolve `eventId` Prisma+Inkai (hint `?eventId=` bukan sumber); setelah Inkai ok/404 bersihkan Prisma (id URL + lookup)
...
/api/admin/ukt/suggest      GET autocomplete nama/NIA (q≥2, maks 64 char); ADMIN_DOJO di-scope Prisma ke ranting allowlist, role lain via Inkai `/v1/members`; hydrate `dojoName` dari Prisma; opsional `uktEventId`/`latberEventId` → flag `registeredUkt`/`registeredLatber` (status aktif `notIn CANCELLED/REJECTED`)
/api/admin/ukt/deposit      PATCH status setoran ranting — **cabang-only** (`canEditKyuBaru`); kunci periode berlaku; **Prisma-first** `AppSetting` + best-effort Inkai PUT (tahan JWT expired)
...
/api/admin/ukt/rekap-hasil  POST Excel rekap hasil ujian Pengda (2 sheet); payload pejabat/penguji/sign URL; cabang+ranting; rate limit; tidak digate arsip/kunci
/api/admin/ukt/ttd-template GET/PATCH template TTD reusable (`ukt.ttd-template`); cabang-only; Zod + rate limit
/api/admin/ukt/ttd-suggest  GET autocomplete pejabat/penguji (akun + Hitam/DAN, q≥2) + `signatureUrl`; cabang-only; Prisma; rate limit
/api/admin/ukt/ttd-titles   POST batch refresh pangkat/MSH dari Member (`memberIds`); cabang-only; rate limit
/api/admin/ukt/*            Periode, waiver, nota, hasil ujian, fees (snapshot/global; kunci periode berlaku), Kyu, exam-day, period-meta (pejabat TTD/penguji/sign), invite (siapkan snapshot), hapus pendaftaran + tagihan terkait; sync undangan publik `ukt-invite:{id}`
/api/admin/latber/period    POST buat / PATCH ubah periode Latber (edit aktif: nama/jadwal/lokasi; fee dikunci 45.000; + `archived`/`locked` arsip); buat periode arsipkan semua periode lama non-arsip; cabang-only `canCreateEventsByWilayah`; sync `latber-invite:{id}`
...
/api/admin/latber/registrations/[id]  PATCH terima/tolak mandiri, submit verifikasi, **mark_paid** (cabang → Lunas), **mark_cash** (ranting+cabang → Tunai + Payment CASH + kas); DELETE batal + void kas + tagihan + kredit LATBER (tanpa gate arsip)
/api/admin/latber/add-guest  POST stub Member PENDING + daftar Latber (`latber-guest` flag); `canRegisterMembersToEvents`; allowlist ranting
/api/admin/latber/promote-membership  POST lengkapi identitas + Active + opsional NIA login; cabang/ranting scoped; hapus flag tamu
/api/admin/latber/rekap      POST Excel rekap peserta+setor (ExcelJS); cabang+ranting; rate limit; tidak digate arsip/kunci
/api/admin/latber/suggest   GET autocomplete nama/NIA (scoped); opsional `latberEventId` → flag `registeredLatber` (status aktif `notIn CANCELLED/REJECTED`); hydrate `dojoName` Prisma
/api/admin/latber/members   GET hydrate 1 anggota + `?periodId=` baris Belum Daftar (`latberRow`)
/api/admin/latber/invite     POST refresh snapshot undangan publik Latber; `url` = `/latber` (aktif) atau `/latber?period=` (arsip)
/api/public/latber/period    GET periode aktif + fee flat 45.000 + ranting + `paymentEnabled` + `payment` (LATBER_PAYMENT / fallback kebijakan)
/api/public/latber/registrants GET tabel peserta publik (NIA, amount flat, status, `createdAt`, flag tamu)
/api/public/ukt/registrants GET roster UKT publik `{ period, registrants }` (cache ~10s; noindex page `/ukt`; `createdAt` tanggal daftar)
...
/api/member/profile          GET sabuk kartu (no-store) + PATCH profil lengkap (identitas, foto, dokumen; email/NIA/sabuk/MSH 1×); foto dual-write `Member.photoUrl` + `User.photoUrl`
...
/api/dojos                  Daftar dojo publik (id, nama, cabang, phoneNumber untuk WA verifikasi daftar)
/api/auth/register/check    Cek duplikat NIK/NIA/nama+TTL + email (`emailTaken`) sebelum daftar
...
| Tanggal | Keterangan |
|---------|------------|
| 12 September 2026 | **Posisi Kartu Anggota Ber-QR di Atas Dashboard & UKT Improvements:** Memindahkan `MemberCard` (Kartu Anggota digital berisi QR/barcode) ke posisi paling atas beranda portal anggota (`/dashboard`, `src/app/dashboard/page.tsx`) tepat di bawah `DashboardHomeHeader`; Kolom Kyu Lama di tabel `/admin/ukt` diset read-only (`Badge`) untuk semua role (`UktDashboard.tsx`); `handleKyuUpdate` & API `applyKyuBaruToMember` (`route.ts`) mengunci `decoded.kyuLama` agar Kyu Lama tidak berubah saat Kyu Baru diisi; Menambahkan Card KPI **Rekap Perubahan Kyu (Kyu Lama ➔ Kyu Baru)** di dashboard UKT dengan badge `+2 tingkat` / `+3 tingkat 🔥`, tombol filter cepat **⚡ Naik 2+ Tingkat** & **🔥 Naik 3+ Tingkat**, pill transisi interaktif, **Filter Chip Aktif** di atas tabel (`Filter Perubahan Kyu: [X]`), serta pengelompokan visual `<SelectGroup>` bertajuk **SABUK KYU (WARNA)** vs **SABUK HITAM / DAN** pada dropdown Kyu Baru & Toolbar; inventaris §5/§6/§9.3/§15 |
| 31 Agustus 2026 | **Daftar + Input Massal:** cek duplikat email (register/check + UI); WA verifikasi post-daftar ke ranting; persist identitas Prisma saat daftar mandiri; paste Input Massal 6 kolom tanpa NIA; `GET /api/dojos` sertakan `phoneNumber`; inventaris §9.1/§13/§15 |
| 31 Agustus 2026 | **UKT Verifikasi tanpa billingId:** `canCabangVerifyUktPayment` / `canRantingSubmitUktPayment` tidak lagi mensyaratkan `billingId` untuk APPROVED; `mark_paid` ensure billing + approve; fallback billing orphan di `fetchUktDashboardData`; inventaris §9.3/§15 |
| 5 September 2026 | **14-inch screen layout + Edit icon (openDetail fix) + Cabang edit permissions + Quick-Reg fix:** Global table density (`table.tsx` `py-1.5 px-2 text-xs`), compact button/input heights (`h-7`/`h-8`), Pencil Edit button di kolom Aksi `MembersTable.tsx` memanggil `openDetail(m)` untuk memuat sheet, `canEditMemberIdentity` untuk Cabang (`ADMIN_BRANCH`), service token retry & Prisma event stub auto-sync untuk UKT (`ukt-register.ts`) & Latber (`latber-register.ts`); inventaris §6/§9.3/§10/§11/§15 |
| 31 Agustus 2026 | **UKT Pilih Laporan WA multi-ranting:** dialog cabang centang multi-ranting; semua = ringkas; beberapa = gabungan rinci; `buildUktSelectedDojosWaReportText`; inventaris §9.3.10a/§15 |
| 31 Agustus 2026 | **UKT Laporan WA tautan roster:** baris pertama `SITE_URL/ukt` di salinan WA cabang/ranting; ranting 0: `*Ranting/Dojo*` di atas `*TOTAL SEMUA*`; inventaris §9.3.10a/§15 |
| 30 Agustus 2026 | **UKT Kyu Baru hanya jika LULUS:** `resolveUktRankColumns` buang sisi kanan snapshot (`\|\| Putih` palsu) kecuali `examResult === LULUS`; roster/cetak/admin selaras; inventaris §9.3/§15 |
| 30 Agustus 2026 | **UKT Kyu Baru kosong + sabuk Prisma + setoran/WA 0 peserta:** `resolveUktRankColumns` tidak isi Baru dari kategori; decode tanpa pemisah = Lama; dual snapshot kunci Lama; `applyKyuBaruToMember` tulis Prisma `currentRank` + `registeredRank`; Status setoran & Laporan WA tampilkan ranting 0 (`_0 peserta_`); WA cabang `*TOTAL SEMUA*` di atas; inventaris §9.3/§15 |
| 6 September 2026 | **UKT Multi-Column Sorting (urutan bertingkat):** Tabel `/admin/ukt` mendukung pengurutan bertingkat multi-kolom (contoh: Ranting A-Z lalu Kyu Lama 10→1). Header kolom menampilkan badge nomor urut sort (1, 2, 3), mendukung Shift+klik serta Multi-Sort Pill Bar (+ Tambah Sort / Reset) untuk menambah atau mengubah arah sort secara presisi; `compareRankBuckets` di `table-sort.ts` & `SortableTableHead.tsx`; inventaris §6/§15 |
| 30 Agustus 2026 | **Roster publik `/ukt` sort + centang + cetak:** header sort ikon; kolom centang sticky; toolbar **Cetak** (modal pilih ranting A4/F4, modal tetap terbuka saat Print); `ukt-roster-print-html.ts` + `ukt-public-roster-sort.ts`; inventaris §6/§9.3.14b/§11/§15 |
| 30 Agustus 2026 | **UKT Cetak Nota dekat tabel:** ikon Printer di toolbar sticky tabel (HP+cabang/ranting); `UktPrintModal` tetap terbuka setelah Print (footer Tutup+Cetak); inventaris §6/§9.3.10/§15 |
| 30 Agustus 2026 | **Quick-reg parity anggota/UKT/Latber:** shared `EventQuickRegisterButtons` + `registerMemberToEvent`; dropdown + **kolom Aksi** Daftar UKT/Latber (tombol hilang bila terdaftar); `buildMemberEventRegistrationMap`; `active-registration-periods` fallback Prisma; inventaris §6/§9.3.11d/§9.3b/§15 |
| 30 Agustus 2026 | **Latber nota + WA + cetak publik:** Cetak Nota = `filterLatberApprovedRows` (CASHBACK × semua baris, *Termasuk N Belum Bayar*); Laporan WA salin clipboard (bukan `wa.me`); `/latber` modal cetak daftar tetap terbuka; fix TS `server.ts` deploy; inventaris §6/§9.3b/§15 |
| 30 Agustus 2026 | **Label UI Komisi → CASHBACK:** nota/WA UKT & Latber, rekap Kas, jurnal kas ranting baru; `aggregateKasByDojo` kenali deskripsi `komisi` (lama) + `cashback` (baru); field `komisiRanting` & key setting tetap; inventaris §6/§9/§15 |
| 30 Agustus 2026 | **Fix JWT Kelola Anggota (cabang):** Prisma fallback `set_nia`/`set_name`/`set_dojo`/`set_identity`/`set_contact`/`approve`/`reject` + bulk approve; GET detail overlay Prisma (status/ranting/nama/NIA/kontak); verifikasi + UKT quick-reg auth fail → lokal; merge Inkai `inkaiFetchWithServiceRetry`; inventaris §11/§13/§15 |
| 27 Agustus 2026 | **UKT WA sabuk: Kyu Lama \|\| Kyu Baru:** `resolveNotaBeltGroupFromKyu` selaras daftar peserta WA; inventaris §9.3.10a/§15 |
| 27 Agustus 2026 | **UKT WA A/sabuk = roster penuh:** ranting+cabang hitung snapshot semua peserta daftar (bukan hanya `isUktNotaRow`); `uktWaNetOfNotaRows` tanpa re-filter; inventaris §9.3.10a/§15 |
| 27 Agustus 2026 | **UKT Nota/WA satu baris per sabuk:** `buildNotaBeltLines` pakai tarif snapshot Kyu Lama (bukan pecah per `billingAmount`); inventaris §9.3.10/§15 |
...
| 27 Agustus 2026 | **UKT Laporan WA salin + cetak Latber:** Laporan WA `/admin/ukt` salin clipboard (dialog pilih ranting tetap); format ringkas Pelaksanaan + tempat/lokasi + Lunas/Belum lunas; `/latber` Cetak daftar kolom ☐ sebelum No, kertas A4/F4, portrait/landscape, wrap tanpa potong; inventaris §6/§9.3/§9.3b/§15 |
...
| 26 Agustus 2026 | **Fix UKT kosong pasca deploy:** `fetchUktEventsCached` kompatibel dengan cache lama berbentuk array, cache key event UKT naik ke `ukt-events-list-v4`, dan tes regresi memastikan `/admin/ukt` tetap auto-resolve ke periode aktif II-2026 |
| 26 Agustus 2026 | **Fix blink `/admin/ukt` + empty `/admin/latber`:** hentikan redirect kanonikal yang strip `period` (loader Suspense berkedip); `fetchUktEventsCached` bedakan gagal vs kosong; filter UKT exclude Latber; dashboard client SSR (tanpa double loader); Latber `events?limit=200` + `dbError` + detail period URL + auto-arsip aman; tes `ukt-admin-period-resolve` |
...
| 17 Agustus 2026 | **Sticky kolom Nama + fix Daftar Latber:** `/admin/ukt`, `/ukt`, `/latber` — kolom Nama menempel saat scroll horizontal; `/admin/latber` — tombol Daftar Belum Daftar label Mendaftar… + fetch timeout (hindari stuck disabled) |
| 17 Agustus 2026 | **UKT admin HP + timer publik:** `/admin/ukt` HP — timer tetap; aksi → Lainnya; jadwal/setoran/KPI/alur → Ringkasan; cari sticky di atas tabel; kolom default ringkas; timer batas daftar juga di `/ukt` + `/latber` (payload `registrationCloseAt`) |
...
| 12 Agustus 2026 | Fix NIA anggota: create / bulk-create kini menyinkronkan NIA ke Prisma lokal sebelum auto-provision login; cabang dapat edit NIA inline di kolom anggota + tetap dari sheet detail |
| 12 Agustus 2026 | UKT admin: cabang bisa edit NIA inline di tabel (re-use API `set_nia` anggota) |
...
| 12 Agustus 2026 | **Latber hardening + Kelola Anggota quick-reg:** `InkaiConfirmDialog`; `LatberSearchBar` suggest/hydrate; nota print terisolasi + org profile; arsip manual/auto idempotent + `periodLocked` UI; terima mandiri → billing PENDING; meta self-reg admin; notifikasi + rate limit member; fix `LatberStatusCard` confirm (`eventId`); quick-reg UKT/Latber di `/admin/anggota`; inventaris §6/§9.3b/§11/§13/§15 |
| 12 Agustus 2026 | **Fix Latber arsip UX:** sidebar nav exact-match `/admin/latber` vs arsip; tombol **Hapus** tampil di arsip; DELETE registrations tanpa `assertLatberPeriodMutable`; inventaris §6/§9.3b/§13/§15 |
...
| 14 Agustus 2026 | **Cetak Nota UKT Gabungan:** dropdown modal — satu ranting atau **Gabungan** (multi-ranting / semua cabang); agregat Subtotal A − komisi; nomor `UKT/SBY/GABUNGAN/…`; selaras WA rinci, bukan tab Administrasi Pengprov; inventaris §9.3/§15 |
...
| 16 Agustus 2026 | **Latber edit periode + tarif flat:** dialog **Edit Periode** aktif (cabang); fee dikunci Rp45.000 / komisi Rp5.000; walk-in `/latber` tanpa `uniqueTail`; UI Biaya selalu 45.000; script `backfill-latber-flat-fee.ts` (deploy → `--allow-remote --apply`); inventaris §4/§9.3b/§11/§13/§15 |
| 16 Agustus 2026 | **Link Latber mudah dibaca + fix Verifikasi:** Salin/WA → `/latber` (+ teks jadwal/lokasi/biaya); OG `/latber` "Latihan Bersama"; `mark_paid` idempotent; admin optimistic `setRows` + sync props; inventaris §4/§9.3b/§10/§11/§13/§15 |
...
| 16 Agustus 2026 | **Kwitansi Pembayaran (wireframe):** sidebar + grant ADMIN_DOJO + soft-backfill; `/admin/kwitansi` jenis Iuran/Prestasi/Pengeluaran/Lainnya; mode A+B; Nota Pengeluaran tanpa pratinjau dobel; Cetak/PDF; `Member.signatureUrl` + `SignaturePadField` + UKT prefill; inventaris §6/§11/§13/§15 |
| 16 Agustus 2026 | **Kwitansi polish:** mode A centang filter jumlah/daftar/cetak; Periode/Nama; tab preserve; grup nav Pembuatan+Arsip wireframe; member picker z-index; TTD jelas; cetak padat + watermark DRAFT; draft sessionStorage + nomor lokal; `openHtmlPrintWindow` 1× (UKT/Latber/Anggota); inventaris §6/§11/§15 |
...
| 17 Agustus 2026 | **Fix logout sidebar + `/ukt` kosong palsu:** P2022 `Member.photoUrl` pada JWT refresh (~30s) → `JWTSessionError` → redirect `/login`; claims refresh slim (tanpa `Member.photoUrl`) + fail-open; roster publik pisah periode vs peserta (`loadError`), retry tanpa kolom, UI bedakan 5xx vs tidak ada periode; inventaris §4/§11/§15 |
...
| 19 Agustus 2026 | **Grup Keuangan + Kas:** sidebar Kas/Kwitansi/Arsip; jurnal Prisma `KasEntry`; auto iuran/UKT/Latber (komisi dua buku, setor dual); Tambah/massal/impor/cetak kop INKAI; tutup buku; rekon; grant `/admin/kas`; `db push` lokal; inventaris §6/§11/§13/§15 |
| 19 Agustus 2026 | **Kas dialog + periode:** dialog Tambah/massal lebih lebar; paste tanggal ID (`Selasa, 27 Januari 2026`); filter periode awal–akhir + saldo bawa; teks ruang lingkup ranting vs cabang; inventaris §6/§11/§13/§15 |
| 19 Agustus 2026 | **Kas Ubah manual:** tombol Ubah di kolom Aksi (PATCH baris Tambah/massal); otomatis tanpa edit; bulan terkunci menyembunyikan Ubah/Hapus; inventaris §6/§15 |
| 20 Agustus 2026 | **Kas collapse + UX ahli:** judul grup dilipat (baris Kegiatan kosong tetap tampil); chevron Kegiatan bersama di massal; KPI warna/caption periode; Semua tanggal; label Belum rekon/Cocok rekening; inventaris §6/§15 |
| 20 Agustus 2026 | **Kas pindah lokasi:** cabang/administrator bisa membuka buku ranting dari dropdown dan memindah baris manual ke buku lain (ranting↔cabang); route `scopeType/scopeId`, `POST /api/admin/kas/[id]/transfer`, audit `KAS_TRANSFER`; inventaris §6/§13/§15 |
| 21 Agustus 2026 | **Kas pindah kegiatan:** tombol di judul grup memindah semua baris manual kegiatan itu ke buku lain; `POST /api/admin/kas/transfer-kegiatan`, audit `KAS_TRANSFER_KEGIATAN`; baris otomatis tetap; inventaris §6/§13/§15 |
| 21 Agustus 2026 | **Kas centang batch + layout:** centang multi-pilih baris manual + floating Pindah lokasi; `POST /api/admin/kas/transfer-batch` audit `KAS_TRANSFER_BATCH`; laptop sticky header/KPI/toolbar, HP ringkasan kolaps; inventaris §6/§13/§15 |
| 21 Agustus 2026 | **Kas lokasi tambah + paste massal:** cabang/admin pilih Lokasi buku di Tambah/Tambah massal; tempel TSV Excel (ket+nominal) ke massal via `parseKasMassPaste`; inventaris §6/§15 |
| 21 Agustus 2026 | **Kas hapus batch + massal 500:** floating Hapus + `POST /api/admin/kas/delete-batch` audit `KAS_DELETE_BATCH`; reset form Tambah massal; `KAS_MAX_BATCH` 500; inventaris §6/§13/§15 |
| 21 Agustus 2026 | **Kas Kegiatan combobox:** field Kegiatan di Tambah/Ubah + Kegiatan bersama massal = input + datalist dari `kegiatanOptions` (pilih lama atau ketik baru); inventaris §6/§15 |
| 21 Agustus 2026 | **Cetak Nota UKT = nominal tabel:** `isUktNotaRow` (semua terdaftar inkl. Belum Bayar); Subtotal A = sum `billingAmount` per Kyu Lama; ringkasan Belum Bayar; WA setor tetap `isUktPaymentDocumentRow`; inventaris §9.3.10/10a/§15 |
...
| 22 Agustus 2026 | **Kas massal multi-tanggal:** kolom Tanggal per baris + Tanggal default; tempel Excel append (batch lama tidak hilang); parse tanggal ID; `mergeMassPasteRows`; pre-check bulan terkunci; counter baris; inventaris §6/§11/§15 |
...
| 22 Agustus 2026 | **Kas cabang — impor/reconcile PDF Excel:** lib `kas-import-cleanup` + `kas-pdf-parse` (fill-down tanggal, skip total grup); scripts `kas:pdf-to-raw`, `kas:prepare-import`, `kas:verify-tsv`, `kas:import`, `kas:reconcile`; artefak `data/kas/cabang-sby-clean.tsv` (~297 baris dari PDF); verifikasi saldo periode 27 Jan–25 Jul vs target Rp 7.045.700; produksi: `prisma db push` schema KasEntry sudah sync; reconcile `--apply` hapus 13 duplikat manual (12 Latber 5 Apr + 1 ANDI IRAWAN); inventaris §11/§13/§15 |
| 22 Agustus 2026 | **Kas cabang web3 + restore:** export portal v3 saldo **-Rp 247.125** (286 baris); koreksi nominal Kai-Zen/Griya Amerta → **Rp 107.700**; Latber 5 Apr **14 baris IN** tetap (reconcile hanya hapus duplikat); script `kas:restore-cabang` + `kas:verify-tsv --diff-db`; diff TSV vs DB tidak aman bulk impor (butuh Excel asli); `kas:restore`; inventaris §11/§15 |
| 23 Agustus 2026 | **Kas Laporan Detail + edit inline:** toggle **Laporan Detail** (default) / **Buku** di `/admin/kas`; flat 7 kolom + judul LAPORAN KEUANGAN DETAIL + badge Saldo akhir; klik sel manual → PATCH Tanggal/Keterangan/Masuk/Keluar/Kegiatan (`KasInlineCell`); baris otomatis & bulan terkunci read-only; inventaris §6/§11/§15 |
| 23 Agustus 2026 | **Kas tabel full halaman:** ikon Maximize2/Minimize2 di header Laporan Detail & Buku; panel `fixed inset-0 z-50` (in-app, bukan Fullscreen API); Esc keluar (saat edit sel, Esc hanya batalkan edit); inventaris §6/§15 |
| 23 Agustus 2026 | **Kas full halaman — periode + cetak:** header full menampilkan Dari–Sampai (`KasDateField`) + ikon Printer kiri Saldo/Minimize; sync state filter toolbar; mode Buku sama; inventaris §6/§15 |
| 23 Agustus 2026 | **Kas edit inline baris otomatis:** `updateManualKas` + `canInlineEdit` izinkan semua `sourceType` (bulan terbuka); Hapus/transfer tetap manual-only; subtitle “Klik sel untuk ubah”; Buku tombol Ubah ikut; inventaris §6/§15 |
...
| 25 Agustus 2026 | **Fix Total Tagihan Rekonsiliasi UKT:** `buildUktDepositReconciliation` kini menghitung total tagihan net disetorkan ke cabang (Subtotal A kotor − komisi ranting peserta lunas) dan menerima parameter `komisiRanting` dari state UI/periode sehingga nominal pada tabel Status Setoran UKT (`/admin/ukt`) selaras 100% dengan TOTAL pada NOTA PEMBAYARAN UJIAN KENAIKAN TINGKAT; inventaris §9.3.11b/§15 |
| 25 Agustus 2026 | **Fix Rekapitulasi Setoran Kas UKT Net, Komisi UKT & Komisi Latber All Ranting:** `aggregateKasByDojo` (`src/lib/kas.ts`) & `postKasFromUktPaid` (`src/lib/kas-hooks.ts`) kini memprioritaskan pengelompokan transaksi UKT yang memuat deskripsi/kegiatan UKT (seperti "Latber Persiapan UKT" / "UKT Persiapan") sehingga Ranting **MANYAR**, **CAKRA KOARMATIM**, dan ranting lainnya terisi lengkap pada kolom **UKT (Lunas)** & **Komisi UKT** serta Komisi Latber pada modal **Rekapitulasi Setoran Masuk Per Ranting** (`/admin/kas`), 100% utuh dan selaras di seluruh 7 kolom modal kas; inventaris §9.3c/§15 |
| 27 Agustus 2026 | **UKT/Anggota HP + quick-reg + kepadatan 14" + Kas status setor:** sticky Nama/checkbox (satu scroller); ranting HP + saran cabang; cari sticky offset topbar; quick-reg tampil ranting + sembunyikan Daftar jika sudah terdaftar; Dojo semua breakpoint; shell admin rapat sampai `2xl`; Rekap Kas kolom Status setor UKT (`GET /api/admin/kas/ukt-deposit`); inventaris §6/§13/§15 |
| 27 Agustus 2026 | **UKT Keterangan setoran + scroll HP:** `gapLabel` = `Belum Bayar: n, Menunggu Ujian: m` (bukan duplikat Status setor); tabel non-maximize `overflow-y-clip` agar scroll vertikal ke `<main>`; inventaris §9.3.11b/§15 |
| 27 Agustus 2026 | **Fix ubah Sabuk/Kyu Lama terpental `/admin`:** Radix Select + `stopPropagation` sel Sabuk/Dojo di Kelola Anggota; Kyu Lama UKT editable di tabel biasa (bukan hanya Maximize); Esc select tidak menutup full; `canEditKyuBaru` ADMINISTRATOR tetap; inventaris §6/§11/§15 |
| 27 Agustus 2026 | **Fix sesi berakhir sistemik:** `inkaiFetchWithServiceRetry` + `portalStatusFromInkai` (Inkai 401≠portal 401); members mutasi + `update_kyu` Prisma fallback; hard-logout hanya `SESSION_EXPIRED_MESSAGE`; login soft-check JWT exp; inventaris §11/§13/§15 |
| 5 September 2026 | **Akses Edit Identitas Anggota Admin Ranting:** `canEditMemberIdentity` (`src/lib/wilayah-rbac.ts`) kini mengizinkan Admin Ranting (`ADMIN_DOJO`) memperbarui NIK, Tempat/Tgl Lahir, JK, Alamat, Email, dan Telepon anggota di sheet detail `/admin/anggota` via tombol **Simpan identitas** / **Simpan kontak** (tetap ter-scope wilayah via `buildMemberFilter`); unit test `tests/wilayah-rbac.test.ts`; inventaris §6/§7.3/§9.1/§15 |
| 5 September 2026 | **Desain Ultrathin Seamless, Sidebar Density & Spreadsheet Ultra-Density 14 Inci Global:** Merapatkan seluruh admin (`/admin/*`) ke tingkat kepadatan spreadsheet (tinggi baris ~24-28px, font 11-12px, cell padding 3px); merapatkan **Sidebar Admin** (`AppSidebar`, `SidebarNavLink`, `SidebarNavGroup`): lebar ramping `w-56 xl:w-60`, padding `py-1.5 px-2.5`, font 12px, icon 14px, logo 28px; kolom **AKSI** diubah menjadi **tombol ikon presisi** dengan tooltip (`Wallet`, `Trash2`, `CheckCircle2`, `Banknote`, `X`), menghemat lebar dari >220px ke ~60-80px; jarak antar-seksi rapat (`space-y-1.5`, `mb-1`); KPI status micro-cards; paginasi `sticky bottom-0` backdrop blur; kapasitas tabel meningkat drastis menjadi **12–15+ baris data sekaligus** di layar laptop 14 inci; inventaris §6/§11/§15 |
...
| 6 September 2026 | **Keyboard Shortcuts Tambah & Simpan Kwitansi/Nota (`/admin/kwitansi`):** Menambahkan pintasan keyboard `Alt + A` / `Alt + N` untuk membuka modal **+ Tambah Penerima / Rincian**, `Enter` / `Ctrl + Enter` di dalam modal untuk mengeksekusi **Simpan**, dan `Alt + S` me-trigger **Simpan ke Arsip**; dilengkapi label indikator visual pada tombol (`Alt+A`, `Alt+S`, `Enter`); inventaris §6/§9/§15 |
| 6 September 2026 | **Input Massal Penerima Kwitansi & Tempel Excel (`/admin/kwitansi`):** Komponen `KwitansiBulkPenerimaDialog` (`src/components/admin/kwitansi/KwitansiBulkPenerimaDialog.tsx`) dengan tombol **`📊 Input Massal`** (`Alt + Shift + A`); mendukung copy-paste multi-baris langsung dari Excel/Spreadsheet/TSV/CSV/Pipe (`|`), pengaturan nilai default (Sebagai & Nominal) untuk baris kosong, dan tabel grid interaktif multi-baris; inventaris §6/§9/§15 |
| 6 September 2026 | **Restorasi Scroll Halaman Alami Kas (`/admin/kas`):** Menghapus CSS pembatas `overflow: hidden` pada `.admin-surface:has([data-kas-fill])` di `src/app/globals.css` dan `overflow-y-clip` pada container tabel `KasLedgerClient.tsx`; seluruh halaman Kas (Header, KPI Cards, Toolbar Filter, dan Tabel hingga baris paling bawah) kini dapat digulung secara alami dan mulus di layar laptop, desktop, tablet, maupun HP; inventaris §6/§9/§15 |
| 6 September 2026 | **Baris Total Rekapitulasi Ranting & Sabuk UKT (`/admin/ukt`):** Menambahkan baris **`JUMLAH / TOTAL`** pada bagian bawah tabel status setoran & rekonsiliasi UKT per ranting di `UktDashboard.tsx`; menampilkan penjumlahan total sabuk Putih, Kuning, Hijau, Biru, Cokelat, Hitam, total Peserta, Lunas, dan Total Tagihan (Rp) dari seluruh ranting terdaftar; inventaris §6/§11/§15 |
| 6 September 2026 | **Salin Excel & Word Dual Format, Export CSV & Cetak PDF Tabel Setoran UKT (`/admin/ukt`):** Menambahkan opsi ekspor ganda (**`Salin Excel & Word`**, **`Export CSV`**, **`Cetak PDF`**) beserta ikon **`Copy`**, **`Download`**, dan **`Printer`** pada header kartu dan header tabel status setoran & rekonsiliasi UKT (`depositRecon`) di `UktDashboard.tsx` & `UktMatrixRecapPanel.tsx`; memanfaatkan API `navigator.clipboard.write([new ClipboardItem(...)])` menyalin MIME type ganda (`text/html` tabel HTML ber-border & background + `text/plain` TSV) secara bersamaan sehingga saat ditempel (`Ctrl + V`) di Microsoft Word / Google Docs langsung menjadi **Tabel Word Asli Bergaris Rapi** dan saat ditempel di Microsoft Excel / Google Sheets terisi sel demi sel tanpa berantakan; inventaris §6/§11/§15 |
| 7 September 2026 | **Sinkronisasi Kyu Baru UKT (`/admin/ukt` ↔ `/dashboard/prestasi?tab=Sabuk`):** Menghapus restriksi pembatalan `kyuBaru` pada `resolveUktRankColumns` (`src/lib/belt.ts`) agar target Kyu Baru yang diset admin tetap presisten sebelum/setelah Lulus; memperbarui `EventList` (`src/app/dashboard/prestasi/page.tsx`) dengan mendekode `registeredRank` (snapshot Kyu Lama → Kyu Baru) alih-alih mengandalkan nama kategori statis backend (`category?.name`), sehingga Kyu Baru di `/admin/ukt` selalu 100% sinkron real-time dengan tampilan di portal anggota `/dashboard/prestasi?tab=Sabuk` dan kartu status UKT; inventaris §5/§6/§9.3/§15 |
| 7 September 2026 | **Nama Lengkap di Header Dashboard Prestasi (`/dashboard/prestasi`):** Menambahkan dukungan prop `subtitle` pada `MemberPageHeader` (`src/components/member/MemberPageHeader.tsx`) dan menampilkan identitas/nama lengkap anggota (mis. `24.31411 · ABISEKHA RAJASANAGARA D BUFFON`) secara rapi dan presisi di bawah judul header **Prestasi & Sabuk**; inventaris §5/§15 |
| 7 September 2026 | **Pengecualian Iuran Aktif (`allowEventWithoutDues`) & Penyesuaian Dashboard (`/dashboard` & `/dashboard/iuran`):** Ketika pengurus mengaktifkan opsi *"Tidak wajib lunas iuran untuk daftar event/UKT"* (`allowEventWithoutDues = true`) pada anggota (seperti Abisekha Rajasanagara D Buffon di `/admin/anggota`), dashboard anggota (`/dashboard`) secara otomatis menyembunyikan peringatan/banner merah tunggakan iuran, menampilkan status *"Pengecualian iuran aktif (bebas untuk UKT/Event)"* pada checklist keanggotaan (`MembershipChecklist`), menampilkan banner hijau pemberitahuan Pengecualian Iuran Aktif pada `/dashboard` dan `/dashboard/iuran`, serta me-reset angka aksi cepat iuran menjadi 0; inventaris §5/§6/§9/§15 |
| 7 September 2026 | **Klarifikasi Label "Belum Bayar UKT" Portal Anggota (`/dashboard/prestasi?tab=Sabuk`):** Mengubah label status `belum_bayar` pada kartu UKT dari *"Belum Bayar"* menjadi **"Belum Bayar UKT"** (`src/lib/ukt.ts` & `UktStatusCard.tsx`) serta memperjelas teks petunjuk menjadi *"Koordinasikan pembayaran biaya pendaftaran UKT dengan ketua ranting (nota)"* agar anggota tidak bingung membedakan antara **Biaya Ujian (UKT)** dan **Iuran Bulanan Keanggotaan**; inventaris §5/§9.3/§15 |
| 7 September 2026 | **Fix Sinkronisasi Status Pembayaran/Kelulusan UKT (`/admin/ukt` ↔ `/dashboard/prestasi`):** Memperbaiki bug di `getMemberUktStatus` (`src/lib/member-ukt-status.ts`) yang sebelumnya memintas (*fast-path*) registrasi ber-status PENDING tanpa mengecek tabel `Billing` Prisma dan `examSettings`. Akibatnya, anggota yang sudah dilunaskan/diluluskan di `/admin/ukt` tetap tampil *"Belum Bayar"* di portal anggota. Kini status pembayaran dan hasil ujian selalu dibaca secara akurat sehingga status di `/admin/ukt` dan portal anggota selaras 100% (Lunas / Menunggu Ujian / Lulus Ujian / Selesai); inventaris §5/§6/§9.3/§15 |
| 7 September 2026 | **Prioritas Status Lulus Ujian & Teks Kelulusan Sabuk (`/dashboard/prestasi?tab=Sabuk`):** Meng-update `resolveUktDisplayStatus` (`src/lib/ukt.ts`) agar peserta dengan `examResult === "LULUS"` atau pencapaian sabuk `memberCurrentRank` yang cocok dengan `kyuBaru` langsung berstatus **Lulus Ujian / Selesai** (tidak terjebak di `Belum Bayar UKT`); meng-update `UktStatusCard.tsx` sehingga kartu status kelulusan menampilkan teks hijau: *"Anda naik / lulus UKT dari sabuk Hijau (Kyu 6) → Menjadi: Biru (Kyu 5)"*; inventaris §5/§9.3/§15 |
| 7 September 2026 | **Penyelarasan Agregasi Setoran UKT & Komisi Ranting Kas (`src/lib/kas.ts` & `src/lib/kas-hooks.ts`):** Memperbaiki fungsi `aggregateKasByDojo` agar nominal `amountIn` setoran UKT pada transaksi kas langsung tercatat utuh ke `totalUkt` tanpa pemotongan komisi ganda/asumsi lama (karena nilai jurnal kas sudah berupa nominal setoran masuk); menyelaraskan `postKasFromUktPaid` dengan `DEFAULT_KOMISI_RANTING` (Rp 100.000) sehingga angka pada modal Rekapitulasi Kas dan Pendaftaran UKT selaras; inventaris §6/§9.3.11b/§15 |
| 8 September 2026 | **Fix Inisialisasi Query String Filter Tanggal Kas (`src/components/admin/kas/KasLedgerClient.tsx`):** Memperbaiki inisialisasi state `fromYmd`, `toYmd`, `kegiatan`, `source`, dan `recon` agar membaca query string URL (`searchParams.get("from")`, `searchParams.get("to")`) secara langsung pada mount pertama, sehingga filter tanggal kustom dari tautan (seperti `?from=2026-08-23&to=2026-09-08`) tidak lagi ter-reset otomatis ke awal bulan berjalan (`2026-09-01`); inventaris §6/§9/§15 |
| 8 September 2026 | **Pengelompokan Induk Kegiatan Kas Multiranting & Badge Jumlah Item (`src/lib/kas.ts` & `KasLedgerClient.tsx`):** Meng-update `getKasBaseKegiatan`, `groupKasTable`, dan `KasGroupHeader` dengan properti `count` serta menampilkan badge indikator **`n item`** (mis. `12 item`, `131 item`) secara langsung pada header baris grup terlipat, memadukan seluruh transaksi sejenis (seperti `Bayar Latber Persiapan UKT` dan `Bayar UKT Semester II-2026` dari seluruh ranting & lintas tanggal) di bawah **1 baris header induk utama**; inventaris §6/§9/§15 |
| 8 September 2026 | **Fitur Set Kategori / Kegiatan Massal Item Dicentang (`src/components/admin/kas/KasLedgerClient.tsx` & `POST /api/admin/kas/batch-kegiatan`):** Menambahkan tombol **`🏷️ Set Kategori (n)`** pada bilah aksi melayang saat transaksi dicentang (`selectedIds`), dilengkapi modal dialog `batchKegiatanOpen` yang mendukung pengisian nama kategori baru (mis. `DONASI SS LINDA`) atau memilih kategori existing, secara serentak mengupdate kolom `kegiatan` seluruh baris terpilih dan menggabungkannya ke dalam 1 kategori; inventaris §6/§9/§15 |
| 9 September 2026 | **Fix Terpental ke Arsip saat Buat Periode UKT Baru (`/admin/ukt` ↔ `/admin/ukt/arsip`):** Memperbaiki logika server redirect (`src/app/admin/ukt/page.tsx`) agar saat periode diarsip dan admin berpindah ke layar pendaftaran/pembuatan periode baru, server tidak menendang (redirect) admin kembali ke `/admin/ukt/arsip`, melainkan langsung menampilkan formulir pendaftaran bersih (`/admin/ukt?create=1`); menambahkan Tab Switcher **[📋 Pendaftaran]** | **[📦 Arsip UKT]** dan tombol cepat **`+ Buat Periode`** di toolbar `UktTermNav` serta tombol **`+ Buat Periode Baru`** pada empty state kartu Arsip (`UktDashboard.tsx`); inventaris §6/§9/§15 |
| 9 September 2026 | **Fix Positioning Toolbar Floating Centang & Tombol Kelompokkan Kegiatan Kas (`src/components/admin/kas/KasLedgerClient.tsx`):** Mengubah CSS container toolbar melayang dari `absolute bottom-3 left-3` menjadi `fixed bottom-4 left-1/2 -translate-x-1/2 z-50` agar bilah aksi melayang saat transaksi dicentang (`selectedIds`) selalu tampak mengapung di bagian bawah tengah layar pengguna (viewport) tanpa terbenam di dasar tabel; serta menambahkan tombol **`Kelompokkan kegiatan (n)`** dan **`Pindah lokasi`** di mode tampilan Laporan Detail maupun mode Buku; inventaris §6/§9/§15 |
| 9 September 2026 | **Presisi Nominal Kas Cabang Latber Net Rp 40.000 (`src/lib/kas-store.ts`):** Memperbaiki `syncMissingLatberKasForScope` agar entri transaksi Latber di Buku Kas Cabang selalu dicatat presisi sebesar **Rp 40.000** net (dari total biaya Latber Rp 45.000 dikurangi cashback ranting Rp 5.000), serta secara otomatis memperbarui (*auto-reconcile*) entri Latber di Kas Cabang yang sempat membawa nominal gabungan bundel pendaftaran UKT; inventaris §6/§9/§15 |
| 13 September 2026 | **Keterangan Saldo Kas Berjalan Per Filter Tanggal (`src/components/admin/kas/KasLedgerClient.tsx`):** Menambahkan penjelasan saldo kas berjalan per tanggal di bawah pesan empty state tabel kas ("Tidak ada mutasi yang cocok...") dan sub-label tanggal `(per dd mmm yyyy)` pada badge Saldo Akhir ketika mutasi dalam rentang tanggal bernilai 0; memperjelas ke pengguna/pengurus bahwa nominal saldo tersebut merupakan akumulasi saldo berjalan dari transaksi sebelumnya; inventaris §6/§9/§15 |
| 13 September 2026 | **Auto-Sync Komisi UKT Ranting ke Kas Ranting (`src/lib/kas-store.ts` & `src/lib/kas-hooks.ts`):** Meng-update `syncMissingUktKasForScope` dan `postKasFromUktPaid` agar transaksi pembayaran UKT anggota ranting secara otomatis disinkronkan ke Buku Kas Ranting (`scopeType: "dojo"`) sebagai entri `Komisi ranting — [Nama Peserta]` (Rp 50.000/peserta), sehingga tabel Kas Ranting kini menampilkan peserta UKT bersanding dengan peserta Latber; inventaris §6/§9/§15 |
| 13 September 2026 | **Integrasi Otomatis Pembayaran Event / Kegiatan Umum ke Kas (`src/lib/kas-hooks.ts`, `billing/[id]/route.ts`, `KasLedgerClient.tsx`):** Menambahkan `postKasFromEventPaid`, tipe `KasSourceType = "event"`, dan klasifikasi `classifyBillingForKas` untuk tagihan Event Umum (Gashuku, Kejuaraan, Seminar, Pertandingan, dll.); sehingga saat tagihan event dilunasi, transaksi secara otomatis tercatat di Kas Cabang & Ranting dengan kategori kegiatan sesuai judul event, serta menambahkan opsi filter `Event / Kegiatan` pada dropdown Sumber Kas; inventaris §6/§9/§15 |
| 13 September 2026 | **Tampilan Saldo Berjalan pada Baris Grup Terlipat / Kolaps Kas (`src/lib/kas.ts` & `KasLedgerClient.tsx`):** Meng-update `KasGroupHeader` dan `groupKasTable` untuk menghitung `lastSaldo` (saldo berjalan pada item terakhir grup kegiatan) serta menampilkan nilai saldo tersebut di kolom Saldo pada baris header grup yang sedang terlipat/kolaps; sehingga alur perhitungan saldo berjalan di Mode Buku Kas tetap berlanjut dan mudah dibaca tanpa ada kolom kosong; inventaris §6/§9/§15 |
| 13 September 2026 | **Penyelarasan Cetak PDF Laporan Kas Kosong (`src/lib/kas-print-html.ts`):** Meng-update template HTML cetak `buildKasPrintHtml` agar ketika tabel hasil cetak tidak memuat mutasi pada rentang filter yang dipilih, dokumen cetak PDF menampilkan penjelasan saldo berjalan per periode tanggal yang sama dengan tampilan layar; inventaris §6/§9/§15 |
| 13 September 2026 | **Fix Nama Scope Ranting & Attribution Rekap Kas (`src/lib/kas-store.ts` & `src/lib/kas.ts`):** Memperbaiki `listKasScopes` agar menyertakan nama Ranting presisi (mis. `Ranting GADING`) pada scope admin non-superadmin, serta meng-update `extractDojoNameFromKasRow` agar saat berada pada single dojo scope, seluruh mutasi kas ranting dialokasikan presisi ke ranting tersebut tanpa terpotong menjadi nama generik `RANTING`; inventaris §6/§9/§15 |
| 13 September 2026 | **Dukungan Hapus Batch Kas Tanpa Batas 100 Baris (`src/lib/security/schemas.ts`, `src/lib/kas-store.ts`, `KasLedgerClient.tsx`):** Memperbarui `kasDeleteBatchSchema`, `deleteManualKasByIds`, dan `handleBatchDelete` untuk menaikkan batas hapus transaksi massal sekaligus hingga 5.000 baris (sebelumnya dibatasi 100 baris); sehingga aksi centang banyak/semua baris (mis. 132 baris) dapat dieksekusi sekali klik tanpa muncul error penghalau "Maksimal 100 baris per penghapusan"; inventaris §6/§9/§15 |
| 14 September 2026 | **Fix Preservasi Nama Kegiatan Custom Kas (`src/lib/kas.ts`):** Memperbaiki `getKasBaseKegiatan` agar hanya mengelompokkan transaksi pembayaran pendaftaran Latber yang diawali `Bayar Latber` / `Latber` / `Latihan Bersama` atau `sourceType === "latber"`; mencegah transaksi manual/custom dengan kata "LATBER" di judul (seperti `HONOR PELATIH LATBER 1 PERSIAPAN KEJURKOT WALIKOTA`) agar tidak lagi dipaksa (*override*) kembali ke grup `Bayar Latber UKT`; inventaris §6/§9/§15 |
| 14 September 2026 | **Non-Otomatisasi Pengurutan Tanggal & Kontinuitas Saldo Kas (`src/lib/kas.ts` & `src/lib/kas-store.ts`):** Meng-update `sortKasEntries`, `groupKasTable`, dan `listKasEntries` agar pengurutan baris transaksi kas dipertahankan murni berdasarkan `sortOrder` dan waktu penginputan (`createdAt: "asc"`), tanpa mengacak/mengubah urutan baris berdasarkan tanggal (`txnDate`); mempertahankan perhitungan saldo kas berjalan global yang kontinu dan tidak terputus/negatif; inventaris §6/§9/§15 |
| 14 September 2026 | **Fitur Drag & Drop Reordering Baris & Grup Kegiatan Kas (`src/lib/kas-store.ts`, `POST /api/admin/kas/reorder`, `KasLedgerClient.tsx`):** Menambahkan field `sortOrder` pada model `KasEntry` (`prisma/schema.prisma`), endpoint API `reorder`, serta handle geser `GripVertical` di UI tabel Kas baik untuk baris individu maupun baris header **Grup Kegiatan**; memindahkan seluruh item anggota kegiatan secara utuh dalam satu blok (termasuk grup terlipat/kolaps) ketika header kegiatan digeser (drag & drop), dengan update saldo berjalan global dan penginputan real-time tanpa ketergantungan pada abjad/tanggal; inventaris §6/§9/§15 |
| 15 September 2026 | **Grafik Keuangan Interaktif & Analisis SWOT Otomatis pada Cetak Laporan Kas (`src/components/admin/kas/KasChartSwotPanel.tsx`, `KasLedgerClient.tsx`, `src/lib/kas-print-html.ts`):** Menambahkan seksi grafik interaktif (`KasChartSwotPanel`) tepat di bawah kartu KPI pada halaman Kas `/admin/kas` yang mendukung **Filter Multi-Select Kegiatan** (dapat mencentang/memilih beberapa kegiatan spesifik seperti UKT, Latber, Iuran, Operasional), **3 Mode Visualisasi Grafik** (Tren per Bulan, Distribusi Kegiatan, dan Arus Kas Bersih Net Surplus/Defisit), serta **Mesin Analisis SWOT Keuangan Otomatis** (perhitungan dinamis 4-kuadran *Strengths*, *Weaknesses*, *Opportunities*, *Threats* + Rekomendasi Strategis berdasarkan rasio kas & transaksi belum rekon); saat tombol **Cetak** diklik, dokumen cetak A4/F4 Landscape memuat Kop Surat INKAI, Ringkasan Visual Grafik Kegiatan Terpilih, Matriks 4-Kuadran SWOT Keuangan, Rekomendasi Strategis, dan Tabel Mutasi Detail; inventaris §6/§9/§15 |
| 15 September 2026 | **Fitur Massal Cocok Rekening & Kalkulator Subtotal Terpilih di Bilah Melayang Kas (`POST /api/admin/kas/batch-recon`, `KasLedgerClient.tsx`):** Menambahkan tombol **`✓ Cocok rekening (n)`** (tandai 1-klik seluruh transaksi dicentang sebagai *matched* / cocok bank) dan **`🔓 Buka rekon (n)`** (kembalikan ke *open* / belum rekon) pada bilah melayang (*floating action bar*), serta menambahkan **Badge Subtotal Terpilih Instant** (`selectedSubtotals`) yang menghitung jumlah item, total masuk, total keluar, dan net saldo secara live dari item-item yang dicentang; inventaris §6/§9/§15 |
| 20 September 2026 | **Fitur Sekretaris & Kearsipan Organisasi (`/admin/sekretaris` & `/v/surat/[id]`):** Implementasi lengkap modul Sekretariat yang mencakup **Dashboard Sekretariat** (`/admin/sekretaris`), **Surat Masuk & Keluar** (`/admin/sekretaris/surat`) dengan penomoran otomatis (`[No]/INKAI-KOTA.SBY/[Kode]/[Bulan-Romawi]/[Tahun]`), **Generator Surat PDF A4/F4** (`/admin/sekretaris/generator`) dengan WYSIWYG Rich Text Editor Toolbar (Font Family, Font Size, Bold/Italic/Underline, Align, Table Editor), **Mode TTD Digital / Manual / Draw**, **QR Code Verifikasi Keabsahan Surat Publik** (`/v/surat/[id]`), **Delivery Surat ke Portal & Inbox Anggota** (`/dashboard/dokumen`), **Repository Kearsipan Dokumen & SK Organisasi** (`/admin/sekretaris/dokumen`) dengan alert SK kadaluarsa, **Notulensi & Risalah Rapat** (`/admin/sekretaris/rapat`) dengan action items & salin format WA, serta matriks scope wilayah (Pengprov, Cabang, Dojo); inventaris §6/§8/§9.14/§13/§15 |
