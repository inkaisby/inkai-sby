...
| Inkai JWT stale / mutasi admin | Diperkuat | `inkaiFetchWithServiceRetry`: JWT near-exp → service token dulu; auth fail → retry service; **jangan forward 401** (`portalStatusFromInkai` → 502); `set_rank` / UKT `update_kyu` Prisma fallback 200; login soft-check `exp` + clear cookie stale; `INKAI_SERVICE_TOKEN` wajib produksi |
