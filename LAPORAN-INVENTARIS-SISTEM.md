...
| 5 September 2026 | **Desain Ultrathin Seamless & Kepadatan Layout 14 Inci Global:** Implementasi desain ultrathin seamless seluruh admin (`/admin/*`); border hairline tipis (`border-border/35-40`), custom scrollbar ultrathin 5px, topbar & sticky header rapat (`top-11 lg:top-12`), header setoran UKT 100% utuh di bawah topbar, KPI cards compact, hairline border kolom sticky `Nama` (`src/lib/admin-table-sticky.ts`), paginasi `sticky bottom-0` backdrop blur, dan sheet detail `max-h-[100dvh]` inner scroll; kapasitas tabel meningkat menjadi 8–12+ baris data sekaligus pada laptop 14 inci; inventaris §6/§11/§15 |
