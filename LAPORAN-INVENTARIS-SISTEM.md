...
| `/latber` | **Pendaftaran Latber walk-in (publik):** info login NIA + Pesan; **sticky bar** (timer compact HP + cari + **filter centang multi-ranting** + **Salin WA** format Cormat + Scan/Tambah/Ringkasan); KPI status mengikuti filter (cari/saran/register tetap cabang penuh); KPI per ranting klik = toggle filter + grafik batang; cari/scan QR kartu; modal Tambah Anggota (= tab Daftar); tabel (NIA, **Tanggal daftar**, biaya flat **Rp 45.000**); **kolom Nama sticky** saat scroll horizontal (HP); **badge Status berwarna** (mirror admin: lunas hijau, belum bayar/verifikasi amber); **Bayar** = QRIS percobaan + Mandiri `1400024546344` a.n. HABIBUR RAHMAN (salin rekening/nominal 45.000) + **Sudah bayar** → Menunggu Verifikasi; batal WA; `/admin/latber` tanpa sesi → sini; link undangan pendek **`/latber`** + `opengraph-image` / metadata "Latihan Bersama" |
...
| UKT | Sidebar grup **UKT (Ujian Kenaikan Tingkat)** → **Pendaftaran** (`/admin/ukt`) + **Arsip UKT** (`/admin/ukt/arsip`); **registrants-first** (peserta terdaftar; Belum Daftar via suggest+hydrate); foto peserta di-hydrate dari `Member.photoUrl` Prisma (fallback `User.photoUrl`, Prisma menang atas string kosong/stale dari Inkai); **shell streaming**: header + semester/tahun tampil dulu, KPI/tabel Suspense menyusul; periode aktif, **sort kolom**, multi-select ranting, **filter Gabungan multi-ranting**, bayar→verifikasi cabang, sabuk target, nota, **Laporan/Rekapan** (hub tab Peserta / Hasil Ujian / Administrasi — cabang; ranting tanpa Administrasi), **hari-H**, **setoran + rekonsiliasi**, **arsip**, wizard (ujian/pejabat/snapshot biaya); **HP:** timer tetap; aksi toolbar → **Lainnya**; jadwal/setoran/KPI/alur → **Ringkasan**; cari sticky menempel di atas tabel; **kolom Nama Lengkap sticky** saat scroll horizontal; **kolom Tanggal daftar** (sort; tampil di mode ringkas); kolom default ringkas; **toolbar atas sticky**; **ranting: Daftar/Batal (sebelum Menunggu Ujian)/Bayar + toolbar Laporan/Rekapan (Peserta+Hasil scoped) + Laporan WA (rincian setor), Salin/WA Undangan & Cetak Nota**; cabang: **edit NIA inline** (set_nia) + **Hapus tagihan** terpisah dari hapus pendaftaran + **Hapus pendaftaran termasuk lunas**; **Pengprov: edit NIA inline** (tanpa aksi cabang lain); **undangan portal** `/undangan/ukt/[periodId]` + **roster publik** `/ukt` |
| Latihan Bersama | Nav grup **Pendaftaran** (`/admin/latber`) + **Arsip Latihan Bersama** (`/admin/latber/arsip`); sidebar **exact-match** Pendaftaran vs Arsip (mirror UKT); **registrants-first** (`LatberSearchBar` suggest+hydrate Belum Daftar + **Daftar** instan optimis update state `setRows` agar tidak stuck `Mendaftar…`); **kolom Tanggal daftar**; periode bebas tanggal (cabang **Buat/Edit Periode** dengan field **Tarif tetap Peserta** & **Komisi Ranting**); **Arsipkan/Buka arsip** + auto-arsip idempotent saat tutup daftar; **UI gate `periodLocked`** (banner + disable mutasi; **Hapus** lunas tetap di arsip); modal konfirmasi **InkaiConfirmDialog** (Batal/Hapus/Arsip); tarif flat default **Rp 45.000** (tanpa kode unik) + komisi ranting default **Rp 5.000** (keduanya terkonfigurasi per periode via `resolveLatberPeriodFees`); **Cetak Nota** diperlebar (`max-w-4xl`) dengan **filter centang multi-ranting (GABUNGAN)**, field input penyesuaian komisi ranting per peserta, dan preview kertas A4 ber-kop/logo/bendahara; **Daftar/Terima/Konfirmasi/Tolak/Bayar/Verifikasi/Batal**; cabang **Hapus pendaftaran** (termasuk arsip); toolbar **Rekap** (Excel/PDF/Print) + **Laporan WA** (ranting rinci setor, cabang ringkas/pilih ranting) + **Salin/WA Undangan** (URL pendek `/latber`); meta self-reg di admin (`memberPaymentConfirmedAt`); notifikasi ranting/cabang + anggota; rate limit member register/confirm-payment |
...
2. URL admin: **Pendaftaran** `/admin/latber?period=<eventId>` dan **Arsip Latihan Bersama** `/admin/latber/arsip?...`; cabang **Arsipkan** manual; auto-arsip idempotent bila pendaftaran tutup.
...
4. **Terima mandiri** (belum lapor bayar) → tagihan **`PENDING` (Belum Bayar)**; ranting **Bayar** → `WAITING_VERIFICATION`. **Konfirmasi** (anggota sudah **Konfirmasi sudah bayar** → status **Menunggu Konfirmasi Ranting**) → langsung `WAITING_VERIFICATION`; cabang **Verifikasi** → lunas (`mark_paid` idempotent bila sudah PAID); UI admin **optimistic `setRows` + sync props** agar aksi/status langsung berubah tanpa refresh manual.
5. **Ranting** — Terima/Konfirmasi/Tolak mandiri, **Daftar Latihan Bersama**, **Bayar** (ajukan verifikasi), **Batal**; **Cabang** — Verifikasi lunas, **Hapus pendaftaran** (termasuk periode arsip); mutasi daftar/status diblok saat arsip/kunci (`assertLatberPeriodMutable` + UI `periodLocked`); **DELETE** pendaftaran tidak gated arsip (cleanup admin).
6. Nota/rekap: `grandTotal = (n × 45.000) − (n × 5.000)`; **Cetak Nota** via iframe terisolasi + kop org (`getBranchOrgProfile`); toolbar **Rekap** (Excel/PDF/Print peserta disetujui + setor) + **Laporan WA** (ranting rinci / cabang ringkas atau pilih ranting) + **Salin/WA Undangan**.
...
12. **Walk-in publik `/latber`:** tanpa login; **timer batas pendaftaran** (komponen sama admin UKT); KPI status (Peserta/Belum Bayar/Menunggu Verifikasi/Lunas) + KPI per ranting + grafik batang stacked; cari/scan QR kartu `/v/{NIA\|UUID}`; daftar langsung **Belum Bayar** dengan **tarif flat Rp 45.000** (`uniqueTail` tidak lagi dialokasi; `amount`/`baseFeeAmount` = 45.000); tabel **Tanggal daftar**; Salin WA Cormat menyertakan tanggal pelaksanaan WIB sebelum countdown; modal Tambah Anggota = `POST /api/auth/register` (+`memberId`) lalu auto-daftar; **Bayar** = QRIS Livin percobaan (`/images/latber-qris-trial.png`, 1 transaksi / kedaluwarsa 16 Agu 2026 10:26 WIB) + rekening Mandiri `1400024546344` HABIBUR RAHMAN (`LATBER_PAYMENT`) + salin nominal 45.000; **Sudah bayar** → `WAITING_VERIFICATION`; bendahara admin **Verifikasi** → Lunas; Batal = WA `081331053100`; `/admin/latber` tanpa cookie sesi → `/latber`. Backfill periode aktif: `scripts/backfill-latber-flat-fee.ts` (deploy kode dulu, lalu `--allow-remote --apply` di produksi).
13. v1 **tanpa:** Kyu/hari-H, laporan Pengprov, fee per sabuk, halaman Pengaturan Latber penuh, Midtrans live.
...
| Refresh tabel UKT | Aktif | `GET /api/admin/ukt/table` snapshot + hydrate identitas Prisma (Tempat/TTL/JK/Alamat/Foto) + `registeredAt` |
...
| Latber walk-in publik | Aktif | `/latber` + `/api/public/latber/*`; tarif flat 45.000 (tanpa uniqueTail); KPI status+ranting+grafik; **kolom Nama sticky** saat scroll horizontal; **kolom Tanggal daftar**; Salin WA Cormat + tanggal pelaksanaan WIB; Bayar QRIS percobaan + transfer Mandiri; Sudah bayar → verifikasi admin; batal WA; poll senyap ~30s |
...
/api/admin/latber/registrations/[id]  PATCH terima/tolak mandiri (terima tanpa lapor bayar → billing PENDING; konfirmasi bayar → WAITING_VERIFICATION), submit verifikasi, mark_paid (idempotent jika sudah PAID); notif anggota+cabang; DELETE batal + tagihan (tanpa gate arsip)
...
/api/public/latber/registrants GET tabel peserta publik (NIA, amount flat, status, `createdAt`)
