...
| Absensi | Aktif | Streaming UI; check-in GPS multi-lokasi (auto geofence 150m + fallback 500m + override "Bukan di sini?"); **kamera live Scan Kode QR Ranting** (`BarcodeDetector` + WebRTC video feed); **Scan Kode QR Ranting mendukung scan QR dari dojo/ranting/tempat lain (multi-ranting)** — melegitimasi fisik & bypass geofence lintas ranting; modal **Barcode Dojo Saya**; biometrik HP opsional (WebAuthn); QR collapsible; riwayat; 1×/hari |
