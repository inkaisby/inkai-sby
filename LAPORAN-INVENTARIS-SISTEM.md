...
| Kas | Grup **Keuangan**: **Kas** `/admin/kas` + **Kwitansi** `/admin/kwitansi` + **Arsip** `/admin/kwitansi/arsip`; buku kas 7 kolom (No/Tanggal/Keterangan/Masuk/Keluar/Saldo/Kegiatan) baris datar + judul grup kegiatan (bukan rowspan) **bisa dilipat**; KPI berwarna per periode + **Semua tanggal**; Tambah / Tambah massal (chevron daftar, **paste Excel maks 500**, **Lokasi** buku untuk cabang/admin, reset form tiap buka) / **Ubah+Hapus manual** (**Kegiatan** pilih dari daftar atau ketik baru) / **Pindah+Hapus lokasi** (cabang-only: 1 baris, **centang multi** pindah/hapus, **Pindah kegiatan** grup) / **Tambah item** di header grup kegiatan (`canWrite`, prefill Kegiatan); layout laptop sticky header/KPI/toolbar + HP ringkasan kolaps; impor CSV; Cetak kop logo INKAI; tutup buku bulanan (cabang); rekon Belum rekon/Cocok rekening; otomatis setelah verifikasi iuran/UKT/Latber (komisi Latber dua buku); kwitansi non-iuran; Prisma `KasEntry` Integer rupiah; schema `prisma db push` |
