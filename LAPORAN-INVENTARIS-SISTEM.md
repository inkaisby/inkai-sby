...
14b. **Roster publik** `/ukt`: tanpa login; periode non-arsip; **timer batas pendaftaran** (payload `registrationCloseAt`); KPI + cari filter (nama/NIA/ranting) + ringkasan HP; poll senyap ~30s (sama `/latber`); `GET /api/public/ukt/registrants`; `robots: noindex`.
...
| Roster publik UKT | Aktif | `/ukt` + `GET /api/public/ukt/registrants`; KPI/cari/ringkasan HP; poll ~30s; noindex; **periode tetap** jika load peserta gagal (`loadError`); cache hanya payload sukses |
| Refresh tabel UKT | Aktif | `GET /api/admin/ukt/table` snapshot + hydrate identitas Prisma (Tempat/TTL/JK/Alamat/Foto) |
...
| Latber walk-in publik | Aktif | `/latber` + `/api/public/latber/*`; tarif flat 45.000 (tanpa uniqueTail); KPI status+ranting+grafik; **kolom Nama sticky** saat scroll horizontal; Bayar QRIS percobaan + transfer Mandiri; Sudah bayar → verifikasi admin; batal WA; poll senyap ~30s |
...
/api/public/latber/registrants GET tabel peserta publik (NIA, amount flat, status)
/api/public/ukt/registrants GET roster UKT publik `{ period, registrants }` (cache ~10s; noindex page `/ukt`)
/api/admin/ukt/table        GET refresh snapshot tabel + identitas Prisma (Tempat/TTL/JK/Alamat/Foto)
