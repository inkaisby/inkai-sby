...
4. **Terima mandiri** → tagihan **`PENDING` (Belum Bayar)**; ranting **Bayar** → `WAITING_VERIFICATION`; cabang **Verifikasi** → lunas (selaras UKT).
5. **Ranting** — Terima/Tolak mandiri, **Daftar Latihan Bersama**, **Bayar** (ajukan verifikasi), **Batal**; **Cabang** — Verifikasi lunas, **Hapus pendaftaran** (termasuk periode arsip); mutasi daftar/status diblok saat arsip/kunci (`assertLatberPeriodMutable` + UI `periodLocked`); **DELETE** pendaftaran tidak gated arsip (cleanup admin).
6. Nota/rekap: `grandTotal = (n × 45.000) − (n × 5.000)`; **Cetak Nota** via iframe terisolasi + kop org (`getBranchOrgProfile`); toolbar **Salin/WA Undangan**.
...
| Modul Latber (MVP) | Aktif | Admin `/admin/latber` + arsip; suggest/hydrate; arsip manual+auto; `InkaiConfirmDialog`; nota print terisolasi; `periodLocked` UI |
...
/api/admin/latber/registrations/[id]  PATCH terima/tolak mandiri (terima → billing PENDING), submit verifikasi, mark_paid; notif anggota; DELETE batal + tagihan (tanpa gate arsip)
