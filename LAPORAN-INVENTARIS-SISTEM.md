...
| Kwitansi Pembayaran | Wireframe `/admin/kwitansi` (setelah Iuran); jenis Iuran/Prestasi/Pengeluaran/Lainnya; mode A+B (kecuali Pengeluaran); Daftar Penerima / Nota Pengeluaran; Cetak/PDF kwitansi landscape + daftar kop INKAI + nota portrait; `Member.signatureUrl` shared UKT; arsip `PaymentReceipt` **fase 2**; grant + soft-backfill ADMIN_DOJO + preset bendahara |
...
1. **Cabang** membuat periode Latihan Bersama (nama + buka/tutup + jadwal/lokasi); event kanonis `Latihan Bersama — {nama}` (judul lama `Latber —` tetap dikenali), kategori **Pendaftaran Latihan Bersama**; snapshot `latber-period-meta:{eventId}` (fee **45.000**, komisi ranting **5.000**); buat periode baru **mengarsipkan** periode lama non-arsip.
...
12. **Walk-in publik `/latber`:** tanpa login; KPI status (Peserta/Belum Bayar/Menunggu Verifikasi/Lunas) + KPI per ranting + grafik batang stacked; cari/scan QR kartu `/v/{NIA\|UUID}`; daftar langsung **Belum Bayar** + `Billing.uniqueTail` (3 digit NIA / cadangan 1–999; `amount=45000+tail`; rekap/nota tetap 45.000); modal Tambah Anggota = `POST /api/auth/register` (+`memberId`) lalu auto-daftar; **Bayar** = QRIS Livin percobaan (`/images/latber-qris-trial.png`, 1 transaksi / kedaluwarsa 16 Agu 2026 10:26 WIB) + rekening Mandiri `1400024546344` HABIBUR RAHMAN (`LATBER_PAYMENT`) + salin nominal; **Sudah bayar** → `WAITING_VERIFICATION`; bendahara admin **Verifikasi** → Lunas; Batal = WA `081331053100`; `/admin/latber` tanpa cookie sesi → `/latber`.
...
| Nav admin | Campuran | Top-level: Iuran, **Kwitansi Pembayaran**, Event, Absensi; **UKT** + **Latihan Bersama** sebagai grup (Pendaftaran + Arsip); grup: Keanggotaan / Konten / Sistem + badge unread pesan |
| Kwitansi Pembayaran | Wireframe | `/admin/kwitansi` — cetak/PDF aktif; state client; `Member.signatureUrl`; arsip PaymentReceipt + notifikasi/dashboard **fase 2** |
...
| Latber walk-in publik | Aktif | `/latber` + `/api/public/latber/*`; uniqueTail; KPI status+ranting+grafik; Bayar QRIS percobaan + transfer Mandiri; Sudah bayar → verifikasi admin; batal WA |
...
/api/admin/latber/period    POST buat / PATCH ubah periode Latber (+ `archived`/`locked` arsip); buat periode arsipkan semua periode lama non-arsip; cabang-only `canCreateEventsByWilayah`; sync `latber-invite:{id}`
...
/api/public/latber/period    GET periode aktif + fee + ranting + `paymentEnabled` + `payment` (LATBER_PAYMENT / fallback kebijakan)
/api/public/latber/registrants GET tabel peserta publik (NIA, amount unik, status)
