...
2. URL admin: **Pendaftaran** `/admin/latber?period=<eventId>` dan **Arsip Latihan Bersama** `/admin/latber/arsip?...`; cabang **Arsipkan** manual atau **Buka kembali** dari arsip; timer pendaftaran (`registrationCloseAt`) tutup pendaftaran publik tanpa menyembunyikan periode dari `/admin/latber` (badge/banner Pendaftaran Ditutup); auto-arsip idempotent 24 jam setelah waktu pelaksanaan event (`eventAt`/`endDate`). Kartu kosong (`!selectedPeriodId`) menampilkan navigasi timbal-balik kontekstual (halaman pendaftaran `[Lihat Arsip]`, halaman arsip `[Kembali ke Pendaftaran Latber Aktif]`).
