...
| 9 September 2026 | **Centang Multi-Select Semua Jenis Transaksi di Mode Buku (`src/components/admin/kas/KasLedgerClient.tsx`):** Membuka akses kotak centang (*checkbox*) di mode **Buku Kas** untuk seluruh jenis transaksi otomatis (seperti `latber`, `ukt`, `iuran`, `kwitansi`) selama bulan buku belum dikunci (`!monthLocked`), sehingga pengguna dapat mencentang transaksi non-manual secara bersamaan dan menggunakan tombol **`Kelompokkan kegiatan (n)`** pada bilah melayang untuk memindahkan kategori kegiatannya; inventaris §6/§9/§15 |
