...
| `/ukt` | **Roster peserta UKT publik** (tanpa login, `noindex`): periode non-arsip term berjalan; kolom No/Foto/NIA/Nama/Kyu Lama/Kyu Baru/Ranting/Status; foto dari `Member.photoUrl` Prisma via `memberPhotoSelect` (fallback `User.photoUrl`; skip kolom bila drift); periode dipisah dari load peserta (`loadError` ≠ “tidak ada periode”); KPI + cari filter-only + ringkasan HP; poll senyap ~30s; **bukan** daftar UKT |
...
14b. **Roster publik** `/ukt`: tanpa login; periode non-arsip; KPI + cari filter (nama/NIA/ranting) + ringkasan HP; poll senyap ~30s (sama `/latber`); `GET /api/public/ukt/registrants`; `robots: noindex`.
