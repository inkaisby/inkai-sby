...
| `/ukt` | **Roster peserta UKT publik** (tanpa login, `noindex`): periode non-arsip term berjalan; kolom No/Foto/NIA/Nama/Kyu Lama/Kyu Baru/Ranting/Status; **kolom Nama sticky** saat scroll horizontal (HP); foto dari `Member.photoUrl` Prisma via `memberPhotoSelect` (fallback `User.photoUrl`; skip kolom bila drift); periode dipisah dari load peserta (`loadError` ≠ “tidak ada periode”); **timer batas pendaftaran** (`UktFloatingCountdown`, di atas cari — bukan di Ringkasan); KPI + cari filter-only + ringkasan HP; poll senyap ~30s; **bukan** daftar UKT |
