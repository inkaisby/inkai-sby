...
| Kas | Grup **Keuangan**: **Kas** `/admin/kas` + **Kwitansi** `/admin/kwitansi` + **Arsip** `/admin/kwitansi/arsip`; buku kas 7 kolom (No/Tanggal/Keterangan/Masuk/Keluar/Saldo/Kegiatan) baris datar + judul grup kegiatan (bukan rowspan); KPI + saldo bawa; filter **periode awal–akhir**; Tambah / Tambah massal / **Ubah+Hapus manual**; impor CSV; Cetak kop logo INKAI; tutup buku bulanan (cabang); rekon Belum/Cocok; otomatis setelah verifikasi iuran/UKT/Latber (komisi Latber dua buku); kwitansi non-iuran; Prisma `KasEntry` Integer rupiah; schema `prisma db push` |
