...
| 5 September 2026 | **14-inch screen layout + Edit icon + Cabang edit permissions + Quick-Reg fix:** Global table density (`table.tsx` `py-1.5 px-2 text-xs`), compact button/input heights (`h-7`/`h-8`), Pencil Edit button di kolom Aksi `MembersTable.tsx`, `canEditMemberIdentity` untuk Cabang (`ADMIN_BRANCH`), service token retry & Prisma event stub auto-sync untuk UKT (`ukt-register.ts`) & Latber (`latber-register.ts`); inventaris §6/§9.3/§10/§11/§15 |
