...
10. **Cetak Nota** (`isUktNotaRow`): semua peserta **terdaftar** yang punya tagihan — termasuk **Belum Bayar**, Menunggu Verifikasi, dan lunas; **tanpa kode unik** (+1…999). Subtotal A = **jumlah kolom Rp tabel** (`billingAmount` aktual), dikelompokkan dari **Kyu Lama** (satu sabuk nominal beda → pecah baris); preview menampilkan ringkasan *Termasuk X Belum Bayar*. Nomor nota memuat semester (`UKT/SBY/{RANTING}/I|II/{tahun}`); modal **daftar centang ranting** (satu / beberapa / semua via Pilih semua) — bila >1 tercentang: `RANTING : GABUNGAN ({nama…})` + nomor `UKT/SBY/GABUNGAN/{smt}/{tahun}` + agregat peserta/sabuk/komisi; filter ranting cocok `dojoId` dengan fallback nama. Pejabat (Bidang Ujian / Bendahara) dari **period-meta per periode**, fallback **Pengaturan → Kebijakan**. Tab Peserta tetap semua terdaftar.
