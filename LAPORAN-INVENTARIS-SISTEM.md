...
**Administrator pusat** (`ADMINISTRATOR` / `ADMIN_PUSAT`): override edit **identitas lengkap** (NIK, tempat/tgl lahir, JK, alamat) dan **kontak** (email login, telepon) anggota di sheet detail Kelola Anggota (`set_identity` / `set_contact`); deteksi duplikat NIK/TTL sama seperti create; ranting/cabang/pengprov tidak mendapat aksi ini.
...
14. **Koreksi identitas pusat:** administrator nasional dapat mengubah NIK, tempat/tgl lahir, JK, alamat (`set_identity`) serta email login & telepon (`set_contact`) dari sheet detail `/admin/anggota`; email hanya jika sudah ada akun User; telepon tanpa akun tetap tersimpan via Inkai.
### 9.2 Iuran
