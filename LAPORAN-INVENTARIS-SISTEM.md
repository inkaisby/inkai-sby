...
10a. **Laporan WA ranting:** daftar peserta + **rincian pembayaran** (Subtotal A sabuk − Komisi Ranting = total disetor ke cabang) hanya peserta **tagihan pembayaran** (`isUktPaymentDocumentRow`: `WAITING_VERIFICATION` atau lunas — **bukan** Belum Bayar). **Nota ≠ WA setor.** WA cabang bisa memilih: **ringkas** (Pelaksanaan + tempat/lokasi + TOTAL + *N Ranting* dengan Lunas/Belum lunas + jumlah kyu) atau **rinci per ranting** (peserta + rincian setor) tanpa ganti akun; tombol **Salin laporan** (clipboard, tidak buka `wa.me`); rekap sabuk mengikuti **Kyu Lama**.
...
| Refresh tabel UKT | Aktif | `GET /api/admin/ukt/table` snapshot + hydrate identitas Prisma (Tempat/TTL/JK/Alamat/Foto) + `registeredAt`; full halaman in-app Maximize (header Refresh/Laporan-Rekapan, Kyu Lama+Baru select cabang) |
