...
| 6 September 2026 | **Salin ke Excel, Export CSV & Cetak PDF Tabel Setoran UKT (`/admin/ukt`):** Menambahkan 3 opsi ekspor lengkap (**`Salin ke Excel`**, **`Export CSV`**, **`Cetak PDF`**) beserta ikon **`Copy`**, **`Download`**, dan **`Printer`** pada header kartu dan header tabel status setoran & rekonsiliasi UKT (`depositRecon`) di `UktDashboard.tsx`; memformat data tabel lengkap (Headers, Sabuk Putih-Cokelat, Total Peserta, Total Tagihan, Status Setor, Keterangan Lunas & Baris Total) dalam format Tab-Separated Values (TSV) ke clipboard (siap paste `Ctrl + V` di Excel), unduhan file `.csv`, dan cetak PDF Kop Resmi TTD pejabat via `openUktMatrixPrint`; inventaris §6/§11/§15 |
