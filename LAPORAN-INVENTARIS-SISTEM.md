...
| UKT | Nav grup **Pendaftaran** (`/admin/ukt`) + **Arsip UKT** (`/admin/ukt/arsip`); **registrants-first** (peserta terdaftar; Belum Daftar via suggest+hydrate); **shell streaming**: header + semester/tahun tampil dulu, KPI/tabel Suspense menyusul; periode aktif, **sort kolom**, multi-select ranting, **filter Gabungan multi-ranting**, bayar→verifikasi cabang, sabuk target, nota, **Buat Laporan UKT** (cabang→Pengprov), **export**, **Rekap Hasil Ujian** (Excel Pengda, cabang+ranting), **hari-H**, **setoran + rekonsiliasi**, **arsip**, wizard (ujian/pejabat/snapshot biaya); **toolbar atas sticky**; **ranting: Daftar/Batal/Bayar + toolbar Rekap Hasil Ujian (scoped) + Laporan WA (rincian setor), Salin/WA Undangan & Cetak Nota**; cabang: **edit NIA inline** (set_nia) + **Hapus tagihan** terpisah dari hapus pendaftaran; **undangan portal** `/undangan/ukt/[periodId]` |
...
10b3. **Rekap Hasil Ujian** (toolbar cabang **dan** ranting): Excel 2 sheet format Pengda (`SMT. I/II` + `LEMBAR TTD`); hanya peserta yang sudah punya **Kyu Baru**; Kyu Lama dari snapshot daftar (bukan `currentRank`); urut ranting + Kyu 10→1; ranting hanya data rantingnya; tombol tampil bila ada hasil; tanggal/nama file dari `examAt`; `POST /api/admin/ukt/rekap-hasil`.
...
13. Toolbar cabang: **Buat Periode**, Hari-H, Export, **Rekap Hasil Ujian**, Laporan WA, **Salin Undangan** / **WA Undangan**, Cetak Nota, **Buat Laporan UKT**, Biaya Sabuk, Arsip (tombol terpisah). Toolbar ranting: **Rekap Hasil Ujian** (scoped) + **Laporan WA** + **Salin/WA Undangan** + **Cetak Nota** (manual).
...
| Export rekap UKT | Aktif | Daftar peserta (formulir): Print/Save as PDF/CSV + pilih ranting + validasi + pratinjau; **Rekap Hasil Ujian** Excel Pengda (2 sheet, cabang+ranting, hanya yang punya Kyu Baru) |
