...
| Kas | Grup **Keuangan**: **Kas** `/admin/kas` + **Kwitansi** `/admin/kwitansi` + **Arsip** `/admin/kwitansi/arsip`; buku kas 7 kolom (No/Tanggal/Keterangan/Masuk/Keluar/Saldo/Kegiatan) baris datar + judul grup kegiatan (bukan rowspan) **bisa dilipat**; KPI berwarna per periode + **Semua tanggal**; Tambah / Tambah massal (chevron daftar) / **Ubah+Hapus manual** / **Pindah lokasi manual** (cabang-only, ranting↔cabang); impor CSV; Cetak kop logo INKAI; tutup buku bulanan (cabang); rekon Belum rekon/Cocok rekening; otomatis setelah verifikasi iuran/UKT/Latber (komisi Latber dua buku); kwitansi non-iuran; Prisma `KasEntry` Integer rupiah; schema `prisma db push` |
...
| Kas | Aktif (jurnal lokal) | `/admin/kas` Prisma `KasEntry`/`KasPeriodLock`; filter periode awal–akhir; Ubah/Hapus/Pindah lokasi baris manual (cabang-only); auto dari verifikasi bayar; manual/massal/impor CSV; cetak kop INKAI; tutup buku; rekon; grant `/admin/kas`; bukan backfill Excel |
