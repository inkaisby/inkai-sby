...
11h. **Load Pendaftaran UKT:** URL canonical di-sync lewat `resolveUktAdminPeriodId` (events + period-meta saja) **sebelum** fetch berat; **registrants-first** (skip pool members + billing global + attendance dump 800); refresh snapshot membawa self-reg meta + append peserta baru; ranting allowlist kosong = fail-closed.
