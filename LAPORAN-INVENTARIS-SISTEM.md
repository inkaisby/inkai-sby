...
| Latihan Bersama | Nav grup **Pendaftaran** (`/admin/latber`) + **Arsip Latihan Bersama** (`/admin/latber/arsip`); sidebar **exact-match** Pendaftaran vs Arsip (mirror UKT); **registrants-first** (`LatberSearchBar` suggest+hydrate Belum Daftar + **Daftar** — label **Mendaftar…** + fetch timeout agar tombol tidak stuck disabled); **kolom Tanggal daftar**; periode bebas tanggal (cabang **Buat/Edit Periode**); **Arsipkan/Buka arsip** + auto-arsip idempotent saat tutup daftar; **UI gate `periodLocked`** (banner + disable mutasi; **Hapus** lunas tetap di arsip); modal konfirmasi **InkaiConfirmDialog** (Batal/Hapus/Arsip); biaya flat **Rp 45.000** (tanpa kode unik) + komisi ranting **Rp 5.000**; **Cetak Nota** terisolasi (kop/logo/bendahara, `printLatberNotaDocument`); **Daftar/Terima/Konfirmasi/Tolak/Bayar/Verifikasi/Batal**; cabang **Hapus pendaftaran** (termasuk arsip); toolbar **Rekap** (Excel/PDF/Print) + **Laporan WA** (ranting rinci setor, cabang ringkas/pilih ranting) + **Salin/WA Undangan** (URL pendek `/latber`); meta self-reg di admin (`memberPaymentConfirmedAt`); notifikasi ranting/cabang + anggota; rate limit member register/confirm-payment |
