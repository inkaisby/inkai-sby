...
| 9 September 2026 | **Centang Multi-Select Semua Jenis Transaksi di Mode Buku (`src/components/admin/kas/KasLedgerClient.tsx`):** Membuka akses kotak centang (*checkbox*) di mode **Buku Kas** untuk seluruh jenis transaksi (termasuk `latber`, `ukt`, `iuran`, `kwitansi`) selama bulan buku belum dikunci (`!monthLocked`), serta menyelaraskan fungsi **Pindahkan** (`transferManualKasByIds`, `transferManualKasByKegiatan`), **Kelompokkan kegiatan**, dan **Hapus** agar bekerja secara seragam pada seluruh jenis transaksi; inventaris §6/§9/§15 |
